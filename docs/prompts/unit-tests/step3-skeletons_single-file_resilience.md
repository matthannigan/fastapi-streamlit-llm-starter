...
Our testing philosophy treats the entire `resilience` infrastructure as a single **Unit Under Test (UUT)**. Every test you design must treat the UUT as a black box, interacting with it exclusively through its public API.
...
2.  **No Internal Mocking**: Your test plans **must not** rely on mocking any internal part of `resilience`. The docstrings should only reference fixtures for dependencies that are **external** to the component. The mocks and fixtures available for testing the `circuit_breaker` are located at:
      - `backend/tests/unit/conftest.py`
      - `backend/tests/unit/resilience/conftest.py`
      - `backend/tests/unit/resilience/circuit_breaker/conftest.py`
...
Generate skeleton tests in `backend/tests/unit/resilience/circuit_breaker/test_*.py`.
