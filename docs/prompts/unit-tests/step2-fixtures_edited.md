...
Your goal is to create high-quality, reusable Pytest fixtures for the **external dependencies** of the `backend/app/infrastructure/cache/encryption.py` module. These fixtures will be used to test the module's behavior while isolating it from systems outside its component boundary.
...
1.  **Analyze Dependencies**: Review its public contract at `backend/contracts/infrastructure/cache/encryption.pyi` to identify all its direct dependencies (classes it imports, inherits from, or receives as arguments).
2.  **Define the Boundary**: A dependency is "external" if it is located **outside** the `backend/app/infrastructure/cache/` directory. Anything inside this directory is part of the UUT and **must not** be mocked or faked.
3.  **Review Existing Fixtures**: Check `backend/tests/unit/conftest.py` and `backend/tests/unit/cache/conftest.py` to see if a suitable fixture already exists.
...
5.  **Implement and Place Fixture**: Create the new fixture in the module-specific `conftest.py` at `backend/tests/unit/cache/encryption/conftest.py`.
...
2.  **Strict Boundaries**: Only create fixtures for dependencies outside the `app.infrastructure.cache` namespace.
...
You can reference `backend/tests/unit/auth/conftest.py` as an example implementation for the `auth` module defined by the public contract at `backend/app/infrastructure/security/auth.py`.
