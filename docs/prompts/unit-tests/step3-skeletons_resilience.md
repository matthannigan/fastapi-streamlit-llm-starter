...
Our testing philosophy treats the entire `resilience` as a single **Unit Under Test (UUT)**. Every test you design must treat the UUT as a black box, interacting with it exclusively through its public API.

  * **Source of Truth**: The public contract defined in `backend/contracts/infrastructure/resilience/[module].pyi` and its corresponding production docstrings.
...
2.  **No Internal Mocking**: Your test plans **must not** rely on mocking any internal part of the `resilience`. The docstrings should only reference fixtures for dependencies that are **external** to the component. The mocks and fixtures available for testing the `[module]` are located at:
      - `backend/tests/unit/conftest.py`
      - `backend/tests/unit/resilience/conftest.py`
      - `backend/tests/unit/resilience/[module]/conftest.py`
...
For each module, generate one or more skeleton test files in `backend/tests/unit/infrastructure/resilience/[module]/`.
