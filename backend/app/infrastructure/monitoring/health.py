...
class HealthChecker:
    ...
    def _determine_overall_status(components: List[ComponentStatus]) -> HealthStatus:
        """
        Aggregate individual component health statuses into overall system health status.

        Implements worst-case aggregation logic where overall system health reflects
        the most severe individual component status to ensure system health accurately
        represents operational capability.

        Args:
            components: List of ComponentStatus objects from individual health checks.
                       Each component contains status, message, timing, and metadata.

        Returns:
            HealthStatus representing aggregated system health:
            - UNHEALTHY: If any component is UNHEALTHY (critical failure)
            - DEGRADED: If any component is DEGRADED and none are UNHEALTHY (partial functionality)
            - HEALTHY: Only if all components are HEALTHY (full operational capability)

        Behavior:
            - Applies strict worst-case aggregation for conservative health reporting
            - Prioritizes UNHEALTHY status over DEGRADED for critical failure visibility
            - Requires all components to be HEALTHY for overall HEALTHY status
            - Handles empty component list by returning HEALTHY (no registered components)
            - Does not consider component response times in status aggregation
            - Focuses purely on functional health status for operational decisions

        Examples:
            >>> # All healthy components
            >>> components = [
            ...     ComponentStatus("db", HealthStatus.HEALTHY, "OK"),
            ...     ComponentStatus("cache", HealthStatus.HEALTHY, "OK")
            ... ]
            >>> status = HealthChecker._determine_overall_status(components)
            >>> assert status == HealthStatus.HEALTHY
            >>>
            >>> # Mixed health with degraded component
            >>> components = [
            ...     ComponentStatus("db", HealthStatus.HEALTHY, "OK"),
            ...     ComponentStatus("cache", HealthStatus.DEGRADED, "Memory only")
            ... ]
            ...
            ...     ComponentStatus("db", HealthStatus.HEALTHY, "OK"),
            ...     ComponentStatus("cache", HealthStatus.DEGRADED, "Memory only"),
            ...     ComponentStatus("ai", HealthStatus.UNHEALTHY, "API unreachable")
            ... ]
            >>> status = HealthChecker._determine_overall_status(components)
            >>> assert status == HealthStatus.UNHEALTHY  # Worst case wins
        """
