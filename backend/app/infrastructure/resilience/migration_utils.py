...
class LegacyConfigAnalyzer:
            ...
            if legacy_var in env_vars:
                ...
                if legacy_var in [
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Invalid integer value for {legacy_var}: {value}"
                        )
                        continue
    ...
    def recommend_preset(
        ...
        if not legacy_config:
            return MigrationRecommendation(
                ...
                migration_steps=[
                    "Set RESILIENCE_PRESET=simple",
                    "Remove any legacy environment variables",
                ],
            )
    ...
    def _generate_custom_overrides(
        self, legacy_config: Dict[str, Any], preset_name: str
        ...
        if "circuit_breaker_threshold" in legacy_config:
            if (
                legacy_config["circuit_breaker_threshold"]
                != preset.circuit_breaker_threshold
            ):
                overrides["circuit_breaker_threshold"] = legacy_config[
    ...
    def _generate_migration_warnings(self, legacy_config: Dict[str, Any]) -> List[str]:
        ...
        if retry_attempts and retry_attempts > 7:
            warnings.append(
                f"High retry attempts ({retry_attempts}) may cause significant latency"
            )

        circuit_threshold = legacy_config.get("circuit_breaker_threshold")
        if circuit_threshold and circuit_threshold < 3:
            warnings.append(
                f"Low circuit breaker threshold ({circuit_threshold}) may cause frequent failures"
            )
        ...
        if retry_attempts and circuit_threshold and retry_attempts > circuit_threshold:
            warnings.append(
                "Retry attempts exceed circuit breaker threshold - circuit may open before retries complete"
            )

        # Check for aggressive strategies in what might be production
        if legacy_config.get("default_strategy") == "aggressive":
            warnings.append(
                "Aggressive strategy detected - ensure this is appropriate for your environment"
            )

        return warnings

    def _generate_migration_steps(
        self, preset_name: str, custom_overrides: Optional[Dict[str, Any]]
    ) -> List[str]:
        """Generate step-by-step migration instructions."""
        steps = [
        ...
        if custom_overrides:
            ...
            steps.append(
                "3. No custom overrides needed - preset defaults are sufficient"
            )
    ...
    def _generate_reasoning(
        self, legacy_config: Dict[str, Any], preset_name: str, score: float
    ) -> str:
        ...
        if preset_name == "development":
            reasoning_parts.append(
                "Your configuration shows patterns typical of development environments: "
                "fast failure recovery and aggressive retry strategies."
            )
        elif preset_name == "production":
            reasoning_parts.append(
                "Your configuration emphasizes reliability and fault tolerance, "
                "indicating production-grade requirements."
            )
        elif preset_name == "simple":
            reasoning_parts.append(
                "Your configuration uses balanced settings suitable for general use cases."
            )

        if retry_attempts:
            reasoning_parts.append(
                f"Retry attempts ({retry_attempts}) align well with {preset_name} preset expectations."
            )

        if circuit_threshold:
            reasoning_parts.append(
                f"Circuit breaker threshold ({circuit_threshold}) is compatible with {preset_name} reliability requirements."
            )

        if strategy:
            reasoning_parts.append(
                f"Default strategy '{strategy}' matches {preset_name} preset philosophy."
            )
...
class ConfigurationMigrator:
    ...
    def generate_migration_script(
        self, recommendation: MigrationRecommendation, output_format: str = "bash"
    ) -> str:
        """
    ...
    def _generate_docker_env(self, recommendation: MigrationRecommendation) -> str:
        ...
        if recommendation.custom_overrides:
            config_json = json.dumps(
                recommendation.custom_overrides, separators=(",", ":")
            )
