    ...
    def to_resilience_config(self) -> ResilienceConfig:
        ...
        return ResilienceConfig(
            ...
            retry_config=RetryConfig(
                ...
                if self.default_strategy == ResilienceStrategy.BALANCED
                else 0.5
                if self.default_strategy == ResilienceStrategy.AGGRESSIVE
                else 1.5,
                exponential_min=1.0
                if self.default_strategy == ResilienceStrategy.AGGRESSIVE
                else 2.0,
...
class PresetManager:
    ...
    def get_preset_details(self, name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "configuration": {
                ...
                "operation_overrides": {
                    k: v.value for k, v in preset.operation_overrides.items()
                },
            },
    ...
    def _basic_validate_preset(self, preset: ResiliencePreset) -> bool:
        ...
        if preset.retry_attempts < 1 or preset.retry_attempts > 10:
            logger.error(
                f"Invalid retry_attempts: {preset.retry_attempts} (must be 1-10)"
            )
            return False
        ...
        if (
            ...
            or preset.circuit_breaker_threshold > 20
        ):
            logger.error(
                f"Invalid circuit_breaker_threshold: {preset.circuit_breaker_threshold} (must be 1-20)"
            )
            return False
        ...
        if preset.recovery_timeout < 10 or preset.recovery_timeout > 300:
            logger.error(
                f"Invalid recovery_timeout: {preset.recovery_timeout} (must be 10-300)"
            )
            return False
...
def get_default_presets() -> Dict[ResilienceStrategy, ResilienceConfig]:
    ...
    return {
        ...
        ResilienceStrategy.CONSERVATIVE: ResilienceConfig(
            ...
            circuit_breaker_config=CircuitBreakerConfig(
                failure_threshold=8, recovery_timeout=120
            ),
        ),
