...
class ConfigurationMetricsCollector:
    ...
    def track_config_operation(
        ...
        try:
            ...
            error_occurred = True
            self.record_config_error(
                preset_name, operation, str(e), session_id, user_context
            )
            raise
        finally:
            ...
            if not error_occurred:
                self.record_config_load(
                    preset_name, operation, duration_ms, session_id, user_context
                )
    ...
    def record_config_error(
        ...
        """
        error_metadata = {"error_message": error_message, **(metadata or {})}
        ...
        logger.warning(
            f"Recorded config error: {preset_name} - {operation}: {error_message}"
        )
    ...
    def record_validation_event(
        ...
        self._record_metric(metric)
        logger.debug(
            f"Recorded validation event: {preset_name} - {'valid' if is_valid else 'invalid'}"
        )
    ...
    def get_usage_statistics(
        ...
        if (
            ...
            and (datetime.now(timezone.utc) - self._stats_cache_time).seconds
        ...
        with self._lock:
            ...
            relevant_metrics = [
                m for m in self.metrics if not cutoff_time or m.timestamp >= cutoff_time
            ]
            ...
            most_used = max(
                preset_counts.items(), key=lambda x: x[1], default=("none", 0)
            )[0]
            least_used = min(
                preset_counts.items(), key=lambda x: x[1], default=("none", 0)
            )[0]
    ...
    def get_preset_usage_trend(
        self, preset_name: str, hours: int = 24
    ) -> List[Dict[str, Any]]:
        """
        ...
        with self._lock:
            for metric in self.metrics:
                if (
                    ...
                    and metric.preset_name == preset_name
                    and metric.metric_type == MetricType.PRESET_USAGE
                ):
                    hour_bucket = metric.timestamp.replace(
        ...
        for hour in range(hours):
            ...
            trend_data.append(
                {"timestamp": bucket_time.isoformat(), "usage_count": usage_count}
            )
    ...
    def export_metrics(
        self, format: str = "json", time_window_hours: Optional[int] = None
    ) -> str:
        """
        ...
        if format.lower() == "json":
            return json.dumps(
                [metric.to_dict() for metric in relevant_metrics], indent=2
            )
            ...
            if relevant_metrics:
                writer = csv.DictWriter(
                    output, fieldnames=relevant_metrics[0].to_dict().keys()
                )
                writer.writeheader()
                for metric in relevant_metrics:
    ...
    def _create_performance_alert(self, preset_name: str, load_time_ms: float):
        ...
        self.alerts.append(alert)
        logger.warning(
            f"Created performance alert for {preset_name}: {load_time_ms:.2f}ms"
        )
