...
CONFIGURATION_TEMPLATES = {
    "fast_development": {
        ...
        "config": {
            ...
            "operation_overrides": {"sentiment": "aggressive"},
        },
    },
    "robust_production": {
        ...
        "config": {
            ...
            "operation_overrides": {"qa": "critical", "summarize": "conservative"},
        },
    },
    "low_latency": {
        "name": "Low Latency",
...
SECURITY_CONFIG = {
    ...
    "content_filtering": {
        ...
        "forbidden_unicode_categories": [
            ...
            "Co",
            "Cs",
        ],  # Control chars, format chars, etc.
    ...
    def check_rate_limit(self, identifier: str) -> tuple[bool, str]:
        ...
        with self._lock:
            ...
            if current_time - last_validation < cooldown:
                return (
                    False,
                    f"Rate limit: wait {cooldown - (current_time - last_validation):.1f}s before next validation",
                )
            ...
            if minute_requests >= rate_config["max_validations_per_minute"]:
                return (
                    False,
                    f"Rate limit exceeded: {minute_requests}/{rate_config['max_validations_per_minute']} validations per minute",
                )
            ...
            if len(request_times) >= rate_config["max_validations_per_hour"]:
                return (
                    False,
                    f"Rate limit exceeded: {len(request_times)}/{rate_config['max_validations_per_hour']} validations per hour",
                )
    ...
    def get_rate_limit_status(self, identifier: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "max_per_minute": SECURITY_CONFIG["rate_limiting"][
                    "max_validations_per_minute"
                ],
                "max_per_hour": SECURITY_CONFIG["rate_limiting"][
                    "max_validations_per_hour"
                ],
                "cooldown_remaining": max(
                    0,
                    SECURITY_CONFIG["rate_limiting"]["validation_cooldown_seconds"]
...
class ResilienceConfigValidator:
    ...
    def __init__(self):
        ...
        if JSONSCHEMA_AVAILABLE:
            ...
            logger.warning(
                "jsonschema package not available - validation will be basic"
            )
    ...
    def check_rate_limit(self, identifier: str) -> ValidationResult:
        ...
        if not allowed:
            return ValidationResult(
                ...
                suggestions=[
                    "Wait before making another validation request",
    ...
    def validate_with_security_checks(
        self, config_data: Any, client_identifier: Optional[str] = None
    ) -> ValidationResult:
        """
        ...
        if config_data is None:
            return ValidationResult(
                ...
                suggestions=[
                    "Ensure your configuration is a valid JSON object with key-value pairs"
                ],
            )
    ...
    def _validate_security(self, config_data: Any) -> ValidationResult:
        ...
        try:
            ...
            if len(json_str) > SECURITY_CONFIG["max_config_size"]:
                errors.append(
                    f"Configuration too large: {len(json_str)} bytes (max: {SECURITY_CONFIG['max_config_size']} bytes)"
                )
                suggestions.append(
                    "Reduce configuration size by removing unnecessary fields or simplifying values"
                )
            ...
            for pattern in SECURITY_CONFIG["forbidden_patterns"]:
                if re.search(pattern, json_str, re.IGNORECASE):
                    errors.append(
                        f"Configuration contains forbidden pattern matching: {pattern}"
                    )
                    suggestions.append(
                        "Remove any HTML, JavaScript, template syntax, or potentially dangerous content from configuration"
                    )

            # Content filtering validation
            content_errors, content_warnings = self._validate_content_filtering(
                json_str
            )
            ...
            whitelist_errors, whitelist_suggestions = self._validate_field_whitelist(
                config_data
            )
            ...
            self._validate_security_recursive(
                config_data, "", errors, warnings, suggestions, depth=0
            )

        except Exception as e:
            errors.append(f"Security validation error: {str(e)}")
            suggestions.append(
                "Ensure configuration contains only valid JSON with allowed field types"
            )
    ...
    def _validate_security_recursive(
        self,
        obj: Any,
        path: str,
        errors: List[str],
        ...
        if depth > SECURITY_CONFIG["max_nesting_depth"]:
            errors.append(
                f"Nesting too deep at '{path}': depth {depth} (max: {SECURITY_CONFIG['max_nesting_depth']})"
            )
            suggestions.append(
                f"Reduce nesting depth by flattening the structure at '{path}'"
            )
        ...
        if isinstance(obj, dict):
            if len(obj) > SECURITY_CONFIG["max_object_properties"]:
                errors.append(
                    f"Too many properties in object at '{path}': {len(obj)} (max: {SECURITY_CONFIG['max_object_properties']})"
                )
                suggestions.append(
                    f"Reduce the number of properties in the object at '{path}'"
                )
            ...
            for key, value in obj.items():
                ...
                if len(key) > SECURITY_CONFIG["max_string_length"]:
                    errors.append(
                        f"Key name too long at '{path}': '{key}' ({len(key)} characters)"
                    )
                    suggestions.append(
                        f"Key names must be {SECURITY_CONFIG['max_string_length']} characters or less"
                    )

                new_path = f"{path}.{key}" if path else key
                self._validate_security_recursive(
                    value, new_path, errors, warnings, suggestions, depth + 1
                )

        elif isinstance(obj, list):
            if len(obj) > SECURITY_CONFIG["max_array_items"]:
                errors.append(
                    f"Too many items in array at '{path}': {len(obj)} (max: {SECURITY_CONFIG['max_array_items']})"
                )
                suggestions.append(
                    f"Reduce the number of items in the array at '{path}'"
                )

            for i, item in enumerate(obj):
                new_path = f"{path}[{i}]"
                self._validate_security_recursive(
                    item, new_path, errors, warnings, suggestions, depth + 1
                )

        elif isinstance(obj, str):
            if len(obj) > SECURITY_CONFIG["max_string_length"]:
                errors.append(
                    f"String too long at '{path}': {len(obj)} characters (max: {SECURITY_CONFIG['max_string_length']})"
                )
                suggestions.append(
                    f"Shorten the text at '{path}' to {SECURITY_CONFIG['max_string_length']} characters or less"
                )
            ...
            if isinstance(obj, int) and abs(obj) > 2**53:
                ...
                suggestions.append(
                    "Consider using smaller integer values to avoid precision issues"
                )
            elif isinstance(obj, float) and (
                abs(obj) > 1e100 or (obj != 0 and abs(obj) < 1e-100)
            ):
                warnings.append(f"Extreme float value at '{path}': {obj}")
                suggestions.append(
                    "Consider using more reasonable floating-point values"
                )
    ...
    def _validate_content_filtering(self, content: str) -> tuple[List[str], List[str]]:
        ...
        for char in content:
            ...
            if (
                codepoint
                > SECURITY_CONFIG["content_filtering"]["max_unicode_codepoint"]
            ):
                errors.append(
                    f"High Unicode codepoint detected: U+{codepoint:04X} ('{char}')"
                )
                continue
            ...
            if (
                category
                in SECURITY_CONFIG["content_filtering"]["forbidden_unicode_categories"]
            ):
                errors.append(
                    f"Forbidden Unicode character category '{category}': U+{codepoint:04X}"
                )
    ...
    def _validate_field_whitelist(
        ...
        for field_name, field_value in config_data.items():
            if field_name not in whitelist:
                ...
                suggestions.append(
                    f"Allowed fields: {', '.join(sorted(whitelist.keys()))}"
                )
                continue
            ...
            if expected_type in ["int", "float"] and isinstance(
                ...
                if "min" in field_spec and field_value < field_spec["min"]:
                    errors.append(
                        f"Field '{field_name}' value {field_value} below minimum {field_spec['min']}"
                    )
                if "max" in field_spec and field_value > field_spec["max"]:
                    errors.append(
                        f"Field '{field_name}' value {field_value} above maximum {field_spec['max']}"
                    )

            # Enum validation
            if "enum" in field_spec and field_value not in field_spec["enum"]:
                errors.append(
                    f"Field '{field_name}' value '{field_value}' not in allowed values: {field_spec['enum']}"
                )
            ...
            if expected_type == "object" and isinstance(field_value, dict):
                ...
                if len(field_value) > max_props:
                    errors.append(
                        f"Field '{field_name}' has too many properties: {len(field_value)} (max: {max_props})"
                    )

        return errors, suggestions

    def _check_repeated_chars(
        self, text: str, path: str, warnings: List[str], suggestions: List[str]
    ):
        ...
        while i < len(text):
            ...
            if count > max_repeated:
                warnings.append(
                    f"Excessive repeated characters at '{path}': '{char}' repeated {count} times"
                )
                suggestions.append(
                    f"Limit repeated characters to {max_repeated} or fewer"
                )
                break

            i += count

    def validate_template_based_config(
        ...
        if not template:
            return ValidationResult(
                ...
                suggestions=[
                    f"Available templates: {', '.join(CONFIGURATION_TEMPLATES.keys())}"
                ],
            )
    ...
    def validate_preset(self, preset_data: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            (
                ...
                logical_warnings,
                logical_suggestions,
            ) = self._validate_preset_logic(preset_data)
            errors.extend(logical_errors)
            warnings.extend(logical_warnings)
            suggestions.extend(logical_suggestions)
    ...
    def _validate_config_logic(
        ...
        if exp_min is not None and exp_max is not None and exp_min >= exp_max:
            ...
            suggestions.append(
                f"Try setting exponential_min to {exp_max / 2:.1f} and exponential_max to {exp_max * 2:.1f}"
            )
        ...
        if retry_attempts is not None and max_delay is not None:
            ...
            if max_delay < estimated_min_delay:
                ...
                suggestions.append(
                    f"Consider increasing max_delay_seconds to at least {estimated_min_delay}s"
                )
        ...
        if circuit_threshold is not None and circuit_threshold < 3:
            warnings.append(
                "Very low circuit breaker threshold may cause frequent circuit opening"
            )
        ...
        if retry_attempts is not None and retry_attempts > 7:
            ...
            suggestions.append(
                "Consider using a preset like 'production' for balanced retry behavior"
            )

        return errors, warnings, suggestions

    def _validate_preset_logic(
        ...
        if retry_attempts > cb_threshold:
            ...
            suggestions.append(
                f"Consider setting circuit_breaker_threshold to at least {retry_attempts + 2}"
            )

        # Check environment contexts make sense
        env_contexts = preset_data.get("environment_contexts", [])
        if (
            "production" in env_contexts
            and preset_data.get("default_strategy") == "aggressive"
        ):
            warnings.append(
                "Using 'aggressive' strategy for production environment may impact reliability"
            )
            suggestions.append(
                "Consider using 'balanced' or 'conservative' strategy for production"
            )

        # Check for development environments with conservative strategy
        if (
            "development" in env_contexts
            and preset_data.get("default_strategy") == "conservative"
            and retry_attempts > 3
        ):
            warnings.append(
                "Conservative strategy with high retry attempts may slow development feedback"
            )
            suggestions.append(
                "Consider using 'development' preset or 'aggressive' strategy for faster feedback"
            )
        ...
        try:
            ...
            if "retry_attempts" in error_path:
                if "minimum" in error_message or "maximum" in error_message:
                    suggestions.append(
                        "💡 Try: retry_attempts: 3 (recommended for most use cases)"
                    )
                    suggestions.append(
                        "🔧 Valid range: 1 (fast failure) to 10 (maximum persistence)"
                    )
                elif "type" in error_message:
                    suggestions.append(
                        "💡 retry_attempts must be a whole number, like: retry_attempts: 5"
                    )

            elif "circuit_breaker_threshold" in error_path:
                if "minimum" in error_message or "maximum" in error_message:
                    suggestions.append(
                        "💡 Try: circuit_breaker_threshold: 5 (good balance)"
                    )
                    suggestions.append(
                        "🔧 Valid range: 1 (very sensitive) to 20 (very tolerant)"
                    )
                elif "type" in error_message:
                    suggestions.append(
                        "💡 circuit_breaker_threshold must be a whole number, like: circuit_breaker_threshold: 8"
                    )

            elif "recovery_timeout" in error_path:
                if "minimum" in error_message or "maximum" in error_message:
                    suggestions.append(
                        "💡 Try: recovery_timeout: 60 (1 minute is often good)"
                    )
                    suggestions.append(
                        "🔧 Valid range: 10 seconds (quick) to 300 seconds (conservative)"
                    )
                elif "type" in error_message:
                    suggestions.append(
                        "💡 recovery_timeout must be a whole number in seconds, like: recovery_timeout: 120"
                    )

            elif "default_strategy" in error_path or "strategy" in error_path:
                if "enum" in error_message:
                    suggestions.append(
                        "💡 Valid strategies: 'aggressive' (fast), 'balanced' (recommended), 'conservative' (reliable), 'critical' (maximum)"
                    )
                    suggestions.append(
                        "🔧 Try: default_strategy: 'balanced' for most use cases"
                    )

            elif "operation_overrides" in error_path:
                if "additionalproperties" in error_message.replace(" ", ""):
                    suggestions.append(
                        "💡 Valid operations: summarize, sentiment, key_points, questions, qa"
                    )
                    suggestions.append(
                        '🔧 Example: operation_overrides: {"qa": "critical", "sentiment": "aggressive"}'
                    )
                elif "enum" in error_message:
                    suggestions.append(
                        "💡 Each operation strategy must be: 'aggressive', 'balanced', 'conservative', or 'critical'"
                    )

            elif "exponential" in error_path:
                suggestions.append(
                    "💡 Exponential backoff settings control retry timing"
                )
                suggestions.append(
                    "🔧 Default values usually work well - consider using a preset instead"
                )
                ...
                suggestions.append(
                    "🔧 Boolean values: true or false, numeric values in reasonable ranges"
                )
            ...
            if "type" in error_message:
                if "string" in error_message:
                    suggestions.append(
                        '📝 This field expects text in quotes, like: "balanced"'
                    )
                elif "integer" in error_message or "number" in error_message:
                    suggestions.append(
                        "📝 This field expects a number without quotes, like: 5"
                    )
                elif "boolean" in error_message:
                    suggestions.append(
                        "📝 This field expects true or false (without quotes)"
                    )
                ...
                suggestions.append(
                    "💡 Check that all necessary fields are included in your configuration"
                )
            ...
            if not suggestions:
                suggestions.append(
                    "💡 Consider using a preset configuration instead of custom settings"
                )
                suggestions.append(
                    "📖 Check the documentation for examples and valid values"
                )
    ...
    def _basic_custom_config_validation(
        self, config_data: Dict[str, Any]
    ) -> ValidationResult:
        ...
        if "retry_attempts" in config_data:
            ...
            if not isinstance(val, int) or val < 1 or val > 10:
                ...
                suggestions.append(
                    "Try values like 2 (fast), 3 (balanced), or 5 (robust)"
                )
        ...
        if "circuit_breaker_threshold" in config_data:
            ...
            if not isinstance(val, int) or val < 1 or val > 20:
                errors.append(
                    "circuit_breaker_threshold must be an integer between 1 and 20"
                )
                suggestions.append(
                    "Try values like 3 (sensitive), 5 (balanced), or 10 (tolerant)"
                )
        ...
        if "recovery_timeout" in config_data:
            ...
            if not isinstance(val, int) or val < 10 or val > 300:
                ...
                suggestions.append(
                    "Try 30s (fast recovery), 60s (balanced), or 120s (stable)"
                )

        return ValidationResult(
            is_valid=len(errors) == 0, errors=errors, suggestions=suggestions
        )
    ...
    def _basic_preset_validation(self, preset_data: Dict[str, Any]) -> ValidationResult:
        ...
        if errors:
            suggestions.append(
                "Preset requires all fields: name, description, retry_attempts, circuit_breaker_threshold, recovery_timeout, default_strategy, operation_overrides, environment_contexts"
            )
