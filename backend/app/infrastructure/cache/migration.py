...
class DetailedValidationResult:
    ...
    def total_mismatches(self) -> int:
        ...
        return (
            self.keys_mismatched + 
            len(self.keys_missing_source) + 
            len(self.keys_missing_target)
        )
...
class CacheMigrationManager:
    ...
    async def create_backup(
        ...
        try:
            ...
            if hasattr(source_cache, 'redis') and source_cache.redis:
                ...
                keys_to_backup = await self._scan_redis_keys(source_cache.redis, pattern)
            else:
                # For memory caches, get keys directly
                if hasattr(source_cache, 'memory_cache'):
                    all_keys = list(source_cache.memory_cache.keys())
                    keys_to_backup = [k for k in all_keys if self._match_pattern(k, pattern)]
                else:
                    logger.warning("Unable to determine cache keys, attempting direct access")
                    keys_to_backup = []
            ...
            for i in range(0, len(keys_to_backup), self.chunk_size):
                ...
                logger.debug(f"Processing backup chunk {i//self.chunk_size + 1}: {len(chunk)} keys")
    ...
    async def migrate_ai_cache_data(
        self,
        source_cache: 'AIResponseCache',
        target_cache: 'GenericRedisCache'
    ) -> MigrationResult:
        ...
        try:
            ...
            if hasattr(source_cache, 'redis') and source_cache.redis:
                ...
                logger.warning("Source cache has no Redis connection, checking memory cache")
                if hasattr(source_cache, 'memory_cache'):
            ...
            success_rate = (keys_migrated / keys_processed * 100) if keys_processed > 0 else 0
    ...
    async def validate_data_consistency(
        ...
        try:
            ...
            if sample_size and len(common_keys) > sample_size:
                ...
                warnings.append(f"Validating sample of {sample_size} keys instead of all {len(source_key_set & target_key_set)}")
            ...
            total_keys_checked = len(common_keys) + len(keys_missing_source) + len(keys_missing_target)
            success = (
                keys_mismatched == 0 and 
                len(keys_missing_source) == 0 and 
                len(keys_missing_target) == 0 and
                len(errors) == 0
            )
    ...
    async def restore_backup(
        ...
        try:
            ...
            for i in range(0, len(keys_list), self.chunk_size):
                ...
                logger.debug(f"Processing restore chunk {i//self.chunk_size + 1}: {len(chunk)} keys")
                    ...
                    try:
                        ...
                        if not overwrite:
                            ...
                            if existing_value is not None:
                                logger.debug(f"Skipping existing key '{key}' (overwrite=False)")
                                continue
            ...
            success_rate = (keys_restored / (keys_restored + keys_failed) * 100) if (keys_restored + keys_failed) > 0 else 0
    ...
    async def _get_all_cache_keys(self, cache: CacheInterface) -> List[str]:
        ...
        if hasattr(cache, 'redis') and cache.redis:
            ...
            if hasattr(cache, 'keys'):
                keys = await cache.keys() if asyncio.iscoroutinefunction(cache.keys) else cache.keys()
                return list(keys)
            else:
                logger.warning(f"Unable to get keys from cache type: {type(cache).__name__}")
                return []
