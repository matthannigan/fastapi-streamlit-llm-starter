...
class BenchmarkReporter(ABC):
    ...
    def _analyze_performance_insights(self, suite: BenchmarkSuite) -> List[str]:
        ...
        if avg_memory > self.thresholds.memory_usage_warning_mb:
            insights.append(f"High memory usage detected: {avg_memory:.2f}MB average "
                            f"(threshold: {self.thresholds.memory_usage_warning_mb}MB)")
        else:
        ...
        if durations:
            ...
            if variability > 2.0:  # 200% variability
                insights.append(f"High performance variability detected: {variability*100:.0f}% range "
                                f"between fastest and slowest operations")
            else:
    ...
    def _generate_recommendations(self, suite: BenchmarkSuite) -> List[str]:
        ...
        slow_operations = [r for r in suite.results
                           if r.avg_duration_ms > self.thresholds.basic_operations_avg_ms]
        if slow_operations:
            recommendations.append(f"Optimize {len(slow_operations)} slow operations: "
                                    f"{', '.join(r.operation_type for r in slow_operations)}")
...
class CIReporter(BenchmarkReporter):
    ...
    def create_performance_badges(self, results: Union[BenchmarkResult, BenchmarkSuite]) -> Dict[str, str]:
        ...
        if isinstance(results, BenchmarkSuite):
            ...
            color = ('brightgreen' if results.performance_grade == 'Good'
                    else 'yellow' if results.performance_grade == 'Acceptable' else 'red')
            badges["Performance"] = f"![Performance](https://img.shields.io/badge/Performance-{results.performance_grade}-{color})"
    ...
    def _generate_ci_insights(self, suite: BenchmarkSuite) -> List[str]:
        ...
        if avg_performance <= self.thresholds.basic_operations_avg_ms:
            ...
            insights.append(f"⚠️ Average performance exceeds thresholds "
                           f"({avg_performance:.1f}ms > {self.thresholds.basic_operations_avg_ms}ms)")
        ...
        if self.include_metadata:
            ...
            if suite.results:
                report_data["computed_metrics"] = {
                    ...
                    "operations_meeting_thresholds": len([r for r in suite.results
                                                          if r.meets_threshold(self.thresholds.basic_operations_avg_ms)]),
                    "total_operations": len(suite.results)
                }
