...
class AuthConfig:
    ...
    def __init__(self):
        ...
        self.enable_user_tracking: bool = (
            os.getenv("ENABLE_USER_TRACKING", "false").lower() == "true"
        )
        self.enable_request_logging: bool = (
...
class APIKeyAuth:
    ...
    def _load_api_keys(self) -> set:
        ...
        if not api_keys:
            ...
            logger.info(
                f"Loaded {len(api_keys)} API key(s) in {self.config.get_auth_info()['mode']} mode"
            )
    ...
    def add_request_metadata(
        self, api_key: str, request_info: Dict[str, Any]
    ) -> Dict[str, Any]:
...
async def verify_api_key_with_metadata(
    ...
    if auth_config.enable_user_tracking or auth_config.enable_request_logging:
        metadata = api_key_auth.add_request_metadata(
            api_key,
            {
