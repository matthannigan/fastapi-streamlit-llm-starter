...
class TextProcessorService:
    ...
    def _register_operations(self):
        ...
        operations = {
            "summarize_text": getattr(
                self.settings, "summarize_resilience_strategy", "balanced"
            ),
            "analyze_sentiment": getattr(
                self.settings, "sentiment_resilience_strategy", "balanced"
            ),
            "extract_key_points": getattr(
                self.settings, "key_points_resilience_strategy", "balanced"
            ),
            "generate_questions": getattr(
                self.settings, "questions_resilience_strategy", "balanced"
            ),
            "answer_question": getattr(
                self.settings, "qa_resilience_strategy", "balanced"
            ),
        }
        ...
        for operation_name, strategy_name in operations.items():
            try:
                ...
                logger.info(
                    f"Registered operation '{operation_name}' with strategy '{strategy_name}'"
                )
            except ValueError:
                logger.warning(
                    f"Unknown strategy '{strategy_name}' for operation '{operation_name}', using balanced"
                )
                ai_resilience.register_operation(
                    operation_name, ResilienceStrategy.BALANCED
                )
    ...
    async def _get_fallback_response(
            ...
            if isinstance(cached_response, dict):
                if operation == TextProcessingOperation.SUMMARIZE:
                    return cached_response.get(
                        "result",
                        "Service temporarily unavailable. Please try again later for text summarization.",
                    )
                    ...
                    return cached_response.get(
                        "result",
                        "I'm sorry, I cannot answer your question right now. The service is temporarily unavailable. Please try again later.",
                    )
        ...
        return fallback_responses.get(
            operation, "Service temporarily unavailable. Please try again later."
        )
    ...
    async def process_text(
        ...
        logger.info(
            f"PROCESSING_START - ID: {processing_id}, Operation: {request.operation}, Text Length: {len(request.text)}"
        )
        ...
        cached_response = await self.cache_service.get_cached_response(
            request.text, operation_value, request.options or {}, request.question
        )

        if cached_response:
            logger.info(f"Cache hit for operation: {request.operation}")
            logger.info(
                f"PROCESSING_END - ID: {processing_id}, Operation: {request.operation}, Status: CACHE_HIT"
            )
            ...
            if isinstance(cached_response, dict):
                ...
                cached_response, TextProcessingResponse
            ):  # Or if it's already the correct type
                cached_response.cache_hit = True
                return cached_response
            else:
                ...
                logger.warning(
                    f"Unexpected cache response type: {type(cached_response)}"
                )
        ...
        sanitized_question = (
            self.sanitizer.sanitize_input(request.question)
            if request.question
            else None
        )
        ...
        try:
            ...
            response = TextProcessingResponse(
                ...
                },  # Use sanitized_text for word_count
            )

            try:
                if request.operation == TextProcessingOperation.SUMMARIZE:
                    response.result = await self._summarize_text_with_resilience(
                        sanitized_text, sanitized_options
                    )
                elif request.operation == TextProcessingOperation.SENTIMENT:
                    response.sentiment = await self._analyze_sentiment_with_resilience(
                        sanitized_text
                    )
                elif request.operation == TextProcessingOperation.KEY_POINTS:
                    response.key_points = (
                        await self._extract_key_points_with_resilience(
                            sanitized_text, sanitized_options
                        )
                    )
                elif request.operation == TextProcessingOperation.QUESTIONS:
                    response.questions = await self._generate_questions_with_resilience(
                        sanitized_text, sanitized_options
                    )
                elif request.operation == TextProcessingOperation.QA:
                    response.result = await self._answer_question_with_resilience(
                        sanitized_text, sanitized_question
                    )
                else:
                    raise ValueError(f"Unsupported operation: {request.operation}")
                ...
                if request.operation == TextProcessingOperation.SENTIMENT:
                    ...
                    response.key_points = await self._get_fallback_response(
                        request.operation, sanitized_text
                    )
                elif request.operation == TextProcessingOperation.QUESTIONS:
                    response.questions = await self._get_fallback_response(
                        request.operation, sanitized_text
                    )
                else:
                    response.result = await self._get_fallback_response(
                        request.operation, sanitized_text, sanitized_question
                    )
                ...
                logger.info(
                    f"PROCESSING_END - ID: {processing_id}, Operation: {request.operation}, Status: FALLBACK_USED"
                )
            ...
            logger.info(
                f"PROCESSING_END - ID: {processing_id}, Operation: {request.operation}, Status: SUCCESS, Duration: {processing_time:.2f}s"
            )
            return response
            ...
            logger.info(
                f"PROCESSING_END - ID: {processing_id}, Operation: {request.operation}, Status: ERROR, Duration: {processing_time:.2f}s"
            )
            raise

    @with_balanced_resilience("summarize_text")
    async def _summarize_text_with_resilience(
    ...
    async def _extract_key_points_with_resilience(
        self, text: str, options: Dict[str, Any]
    ) -> List[str]:
    ...
    async def _generate_questions_with_resilience(
        self, text: str, options: Dict[str, Any]
    ) -> List[str]:
    ...
    async def _summarize_text(self, text: str, options: Dict[str, Any]) -> str:
        ...
        additional_instructions = (
            f"Keep the summary to approximately {max_length} words."
        )
        ...
        try:
            try:
                ...
                logger.error(
                    f"AI response validation failed in summarization: {ve}. Problematic response: {result.output.strip()[:200]}..."
                )
    ...
    async def _analyze_sentiment(self, text: str) -> SentimentResult:
        ...
        prompt = create_safe_prompt(template_name="sentiment", user_input=text)
        ...
        try:
            ...
            try:
                validated_raw_output = self.response_validator.validate(
                    raw_output, "sentiment", text, "sentiment analysis"
                )
            except ValueError as ve:
                logger.error(
                    f"AI response validation failed in sentiment analysis: {ve}. Problematic response: {raw_output[:200]}..."
                )
                return SentimentResult(
                    sentiment="neutral",
                    confidence=0.0,
            ...
            try:
                ...
                ValueError,
            ) as e:  # PydanticError could also be caught if using strict parsing
                logger.warning(
                    f"Failed to parse sentiment JSON or invalid structure: {e}. Output: {validated_raw_output}"
                )
                return SentimentResult(
                    sentiment="neutral",
                    confidence=0.5,
    ...
    async def _extract_key_points(
        ...
        try:
            result = await self.agent.run(prompt)
            try:
                validated_output_str = self.response_validator.validate(
                    result.output.strip(), "key_points", text, "key points extraction"
                )
            except ValueError as ve:
                logger.error(
                    f"AI response validation failed in key points extraction: {ve}. Problematic response: {result.output.strip()[:200]}..."
                )
                return [
                    "An error occurred while processing your request. The AI response could not be validated."
                ]
            ...
            for line in validated_output_str.split("\n"):
                ...
                if line.startswith("-"):
                    ...
                    "Key Points:"
                ):  # Avoid including the "Key Points:" header if AI repeats it
                    points.append(line)
    ...
    async def _generate_questions(
        ...
        try:
            ...
            try:
                validated_output_str = self.response_validator.validate(
                    result.output.strip(), "questions", text, "question generation"
                )
            except ValueError as ve:
                logger.error(
                    f"AI response validation failed in question generation: {ve}. Problematic response: {result.output.strip()[:200]}..."
                )
                return [
                    "An error occurred while processing your request. The AI response could not be validated."
                ]
            ...
            for line in validated_output_str.split("\n"):
                ...
                if line and not line.startswith(
                    "Questions:"
                ):  # Avoid including the "Questions:" header
                    if (
                        line[0].isdigit() and "." in line[:5]
                    ):  # Basic check for numbered lists
    ...
    async def _answer_question(self, text: str, question: str) -> str:
        ...
        prompt = create_safe_prompt(
            template_name="question_answer", user_input=text, user_question=question
        )
        ...
        try:
            ...
            try:
                validated_output = self.response_validator.validate(
                    result.output.strip(), "qa", text, "question answering"
                )
                return validated_output
            except ValueError as ve:
                logger.error(
                    f"AI response validation failed in question answering: {ve}. Problematic response: {result.output.strip()[:200]}..."
                )
                return "An error occurred while processing your request. The AI response could not be validated."
        except Exception as e:
    ...
    async def process_batch(
        self, batch_request: BatchTextProcessingRequest
    ) -> BatchTextProcessingResponse:
        ...
        logger.info(
            f"BATCH_PROCESSING_START - ID: {batch_processing_id}, Batch ID: {batch_id}, Total Requests: {total_requests}"
        )
        logger.info(
            f"Processing batch of {total_requests} requests for batch_id: {batch_id}"
        )

        semaphore = asyncio.Semaphore(self.settings.BATCH_AI_CONCURRENCY_LIMIT)
        tasks = []

        async def _process_single_request_in_batch(
            index: int, item_request: TextProcessingRequest
        ) -> BatchTextProcessingItem:
            ...
            async with semaphore:
                try:
                    ...
                    if not hasattr(
                        TextProcessingOperation, item_request.operation.upper()
                    ):
                        raise ValueError(
                            f"Unsupported operation: {item_request.operation}"
                        )
                    ...
                    return BatchTextProcessingItem(
                        request_index=index,
                    ...
                    logger.warning(
                        f"Batch item {index} (batch_id: {batch_id}) degraded service: {str(e)}"
                    )
                    return BatchTextProcessingItem(
                        request_index=index,
                        status=BatchTextProcessingStatus.FAILED,
                        error=str(e),
                    )

                except Exception as e:
                    logger.error(
                        f"Batch item {index} (batch_id: {batch_id}) failed: {str(e)}"
                    )
                    return BatchTextProcessingItem(
                        request_index=index,
                        status=BatchTextProcessingStatus.FAILED,
        ...
        for i, result in enumerate(raw_results):
            if isinstance(result, Exception):
                logger.error(
                    f"Batch item {i} (batch_id: {batch_id}) failed with exception: {str(result)}"
                )
                results.append(
                    BatchTextProcessingItem(
                        request_index=i,
                ...
                logger.error(
                    f"Batch item {i} (batch_id: {batch_id}) returned unexpected type: {type(result)}"
                )
                results.append(
                    BatchTextProcessingItem(
                        request_index=i,
        ...
        completed_count = sum(
            1 for r in results if r.status == BatchTextProcessingStatus.COMPLETED
        )
        failed_count = total_requests - completed_count
        total_time = time.time() - start_time

        logger.info(
            f"BATCH_PROCESSING_END - ID: {batch_processing_id}, Batch ID: {batch_id}, Completed: {completed_count}/{total_requests}, Duration: {total_time:.2f}s"
        )
        logger.info(
            f"Batch (batch_id: {batch_id}) completed. Successful: {completed_count}/{total_requests}. Time: {total_time:.2f}s"
        )  # noqa: E231
