...
class ResponseValidator:
    ...
    def validate(
        ...
        if not validated_response:
            logger.warning(
                f"Empty response received for expected type: {expected_type}"
            )
        ...
        if (
            request_text
            and len(request_text) > 250
            and request_text.lower() in lower_output
        ):
            logger.warning(
                "Potential verbatim user input leakage in AI output for long input."
            )
        ...
        if expected_type:
            if expected_type == "summary":
                ...
                if len(validated_response) < 10:
                    logger.warning(
                        f"Summary response too short: {len(validated_response)} characters"
                    )
                ...
                if len(validated_response) < 5:
                    logger.warning(
                        f"Sentiment response too short: {len(validated_response)} characters"
                    )
                ...
                if len(validated_response) < 5:
                    logger.warning(
                        f"Key points response too short: {len(validated_response)} characters"
                    )
                ...
                if "?" not in validated_response and len(validated_response) < 10:
                    logger.warning(
                        "Questions response lacks question marks and is very short"
                    )
                    raise ValueError(
                        "Questions response should contain actual questions"
                    )
                ...
                if len(validated_response) < 5:
                    logger.warning(
                        f"Q&A response too short: {len(validated_response)} characters"
                    )
