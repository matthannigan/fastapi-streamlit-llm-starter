    ...
    def is_allowed(self, key: str, weight: int = 1) -> bool:
        ...
        try:
            if (
                ...
                and isinstance(self.last_cleanup, (int, float))
                and current_time - self.last_cleanup > self.cleanup_interval
            ):
        ...
        for key, data in self.clients.items():
            ...
            try:
                if not data["requests"]:
                    ...
                    numeric_requests = [
                        r for r in data["requests"] if isinstance(r, (int, float))
                    ]
                    if not numeric_requests or max(numeric_requests) < (
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            if not allowed:
                raise RateLimitError(
                    "Rate limit exceeded", retry_after=rate_config["window"]
                )
            ...
            if hasattr(request.state, "api_version_detection_method"):
                error_headers[
                    "X-API-Version-Detection"
                ] = request.state.api_version_detection_method
            else:
                # If API versioning middleware hasn't run yet, try to detect version from headers
                api_version_header = request.headers.get(
                    "x-api-version"
                ...
                if api_version_header:
                    ...
                    default_version = getattr(
                        self.settings, "api_default_version", "1.0"
                    )
                    supported_versions = getattr(
                        self.settings, "api_supported_versions", ["1.0"]
                    )
                    error_headers["X-API-Version"] = default_version
                    error_headers["X-API-Supported-Versions"] = ", ".join(
                        supported_versions
                    )
            ...
            return JSONResponse(
                ...
                content={
                    ...
                    "retry_after_seconds": getattr(
                        e, "retry_after", error_headers.get("Retry-After", 60)
                    ),
                },
        ...
        response.headers["X-RateLimit-Reset"] = str(
            int(time.time()) + rate_config["window"]
        )
