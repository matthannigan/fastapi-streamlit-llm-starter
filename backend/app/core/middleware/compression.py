...
class CompressionMiddleware(BaseHTTPMiddleware):
    ...
    def _should_compress_response(self, response: Response) -> bool:
        ...
        compressible = any(
            content_type.startswith(comp_type) for comp_type in self.compressible_types
        )
        ...
        if content_length:
            try:
                ...
                if size < self.min_response_size:
                    logger.debug(
                        f"Response size {size} below threshold {self.min_response_size}"
                    )
                    return False
    ...
    async def _compress_response_body(
        self, response: Response, algorithm: str
    ) -> Response:
        """Compress response body using specified algorithm."""
        logger.debug(
            f"Attempting to compress response: {type(response)}, algorithm: {algorithm}"
        )
        ...
        try:
            ...
            logger.debug(
                f"Body size: {original_size}, threshold: {self.min_response_size}"
            )
            if original_size < self.min_response_size:
                logger.debug(
                    f"Body size {original_size} below threshold {self.min_response_size}, skipping compression"
                )
                return response
            ...
            if compressed_size < original_size:
                ...
                response.headers[
                    "x-compression-ratio"
                ] = f"{compressed_size / original_size:.2f}"  # noqa: E231
    ...
    async def dispatch(
        ...
        logger.debug(
            f"CompressionMiddleware dispatch called for {request.method} {request.url.path}"
        )

        # Handle response compression
        response = await call_next(request)
        logger.debug(
            f"Got response from call_next: {type(response)}, status: {getattr(response, 'status_code', 'unknown')}"
        )
    ...
    async def __call__(self, scope, receive, send):
        ...
        if content_encoding and content_encoding in self.decompression_algorithms:
            ...
            try:
                decompressed_receive = await self._create_decompressing_receive(
                    receive, content_encoding, scope
                )
                    ...
                    if not message.get("more_body", False):
                        ...
                        if compressed_body:
                            try:
                                ...
                                logger.error(
                                    f"Failed to decompress request body ({content_encoding}): {e}"
                                )
                                # Raise ValidationError to let global handler map to 400 consistently
                                raise ValidationError(
                                    f"Invalid {content_encoding} compressed data"
                                )
                        else:
                            return message
                    else:
    ...
    async def __call__(self, scope, receive, send):
        ...
        async def compressing_send(message):
            ...
            if message["type"] == "http.response.start":
                ...
                if (
                    content_type.startswith(("application/json", "text/"))
                    and b"content-encoding" not in headers_dict
                ):
...
def configure_compression_settings(settings: Settings) -> Dict[str, Any]:
    ...
    config = {
        ...
        "algorithms": getattr(
            settings, "compression_algorithms", ["br", "gzip", "deflate"]
        ),
    }
    ...
    config["algorithms"] = [
        alg for alg in config["algorithms"] if alg in valid_algorithms
    ]
