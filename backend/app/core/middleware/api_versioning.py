...
class APIVersioningMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: ASGIApp, settings: Settings):
        ...
        self.skip_internal_paths = getattr(
            settings, "api_versioning_skip_internal", True
        )
        ...
        supported_versions_list = getattr(
            settings, "api_supported_versions", ["1.0", "2.0"]
        )
    ...
    def _get_version_from_query(self, request: Request) -> Optional[str]:
        ...
        version_str = request.query_params.get("version") or request.query_params.get(
            "api_version"
        )
        if version_str:
            return self._normalize_version(version_str)
    ...
    def _create_version_headers(
        self, request_version: str, detection_method: str
    ) -> Dict[str, str]:
    ...
    def _should_redirect_version(
        self, request: Request, detected_version: str
    ) -> Optional[str]:
    ...
    def _rewrite_request_path(self, request: Request, target_version: str) -> str:
        ...
        target_prefix = version_info.get(
            "path_prefix", f'/v{target_version.split(".")[0]}'
        )
    ...
    async def dispatch(
        ...
        if not self._is_version_supported(detected_version):
            ...
            compatible_version = self._should_redirect_version(
                request, detected_version
            )
        ...
        expected_prefix = version_info.get(
            "path_prefix", f'/v{detected_version.split(".")[0]}'
        )
        ...
        version_headers = self._create_version_headers(
            detected_version, detection_method
        )
...
class VersionCompatibilityMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: ASGIApp, settings: Settings):
        ...
        self.compatibility_rules = {
            ...
            ("1.0", "2.0"): {
                "request_transformers": [self._transform_v1_to_v2_request],
                "response_transformers": [self._transform_v2_to_v1_response],
            }
        }
    ...
    async def dispatch(
        ...
        if rules.get("request_transformers") and request.method in [
            "POST",
            "PUT",
            "PATCH",
        ]:
