...
async def get_cache_statistics(
    ...
    """
    try:
        stats = await security_service.get_cache_statistics()
        return {
            "status": "success",
            "data": stats,
        }
        ...
        raise HTTPException(
            ...
            detail=f"Failed to retrieve cache statistics: {e!s}",
        ) from e
...
async def clear_security_cache(
    ...
    """
    try:
        await security_service.clear_cache()
        logger.info("Security cache cleared via API")
        return {
            "status": "success",
...
async def get_cache_health(
    ...
    """
    try:
        # Get health from the security service
        health_info = await security_service.health_check()

        # Extract cache-specific health information
        cache_health = health_info.get("result_cache_health", {})
        cache_stats = health_info.get("result_cache_stats", {})
        ...
        return {
            ...
            "data": {
                ...
                "overall_health": health_info.get("status"),
                "service_uptime_seconds": health_info.get("uptime_seconds"),
            },
        }
    except Exception as e:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Cache health check failed: {e!s}",
        ) from e
...
async def get_cache_configuration(
    ...
    """
    try:
        config = await security_service.get_configuration()

        # Extract cache-related configuration
        cache_config = {
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to retrieve cache configuration: {e!s}",
        ) from e


@router.post("/warmup", summary="Warm Up Security Cache")
async def warmup_security_cache(
    security_service: SecurityService = Depends(get_security_service),
) -> Dict[str, str]:
    """
    ...
    """
    try:
        # TODO: Implement actual cache warmup with common patterns
        # This could include:
        # - Common prompt patterns
        # - Typical response patterns
        # - Known safe/benign content
        # - Edge cases that are frequently scanned

        logger.info("Security cache warmup requested (placeholder implementation)")

        return {
            "status": "success",
            "message": "Cache warmup completed (placeholder implementation)",
            "note": "Actual warmup patterns should be implemented based on use case",
        }
    except Exception as e:
        logger.error(f"Cache warmup failed: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Cache warmup failed: {e!s}",
        ) from e
...
async def get_cache_performance(
    ...
    """
    try:
        # Get comprehensive metrics from security service
        metrics = await security_service.get_metrics()

        # Get cache-specific statistics
        cache_stats = await security_service.get_cache_statistics()

        # Combine metrics for comprehensive view
        performance_data = {
            "security_service_metrics": {
                "input_metrics": {
                    "scan_count": metrics.input_metrics.scan_count,
                    "successful_scans": metrics.input_metrics.successful_scans,
                    "failed_scans": metrics.input_metrics.failed_scans,
                    "average_scan_time_ms": metrics.input_metrics.average_scan_time_ms,
                },
                "output_metrics": {
                    "scan_count": metrics.output_metrics.scan_count,
                    "successful_scans": metrics.output_metrics.successful_scans,
                    "failed_scans": metrics.output_metrics.failed_scans,
                    "average_scan_time_ms": metrics.output_metrics.average_scan_time_ms,
                },
