...
async def get_current_config(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get current configuration: {e!s}")
...
async def get_operation_metrics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get metrics for operation {operation_name}: {e!s}"
        )


@main_router.post("/metrics/reset")
async def reset_resilience_metrics(
    operation_name: str | None = Query(None, description="Operation name to reset (if None, resets all)"),
    api_key: str = Depends(verify_api_key)
) -> Dict[str, Any]:
...
async def get_resilience_dashboard(api_key: str = Depends(optional_verify_api_key)) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get resilience dashboard: {e!s}"
        )
