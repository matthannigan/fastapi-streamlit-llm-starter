...
async def batch_process_text(
    ...
    try:
        ...
        # Convert unexpected exceptions to InfrastructureError
        logger.error(f"Batch processing error for user (API key prefix) {api_key[:8]}..., batch_id {request.batch_id or 'N/A'}: {e!s}", exc_info=True)
...
async def get_service_health(
    ...
    try:
        ...
        raise InfrastructureError(
            f"Failed to get service health: {e!s}",
