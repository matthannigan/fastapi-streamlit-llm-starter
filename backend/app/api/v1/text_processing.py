...
async def process_text(
    ...
    logger.info(
        f"REQUEST_START - ID: {request_id}, Operation: {request.operation}, API Key: {api_key[:8]}..."
    )
    ...
    try:
        ...
        logger.info(
            f"REQUEST_END - ID: {request_id}, Status: SUCCESS, Operation: {request.operation}"
        )
        ...
        logger.info(
            f"REQUEST_END - ID: {request_id}, Status: ERROR, Operation: {request.operation}"
        )
        raise
        ...
        logger.info(
            f"REQUEST_END - ID: {request_id}, Status: VALIDATION_ERROR, Operation: {request.operation}"
        )
        raise ValidationError(
            str(e),
        ...
        logger.info(
            f"REQUEST_END - ID: {request_id}, Status: INTERNAL_ERROR, Operation: {request.operation}"
        )
...
async def batch_process_text(
    ...
    logger.info(
        f"BATCH_REQUEST_START - ID: {request_id}, Batch ID: {batch_id}, Count: {len(request.requests)}, API Key: {api_key[:8]}..."
    )
    ...
    try:
        ...
        logger.info(
            f"Batch processing {len(request.requests)} requests for user (API key prefix): {api_key[:8]}..., batch_id: {request.batch_id or 'N/A'}"
        )

        result = await text_processor.process_batch(request)

        logger.info(
            f"BATCH_REQUEST_END - ID: {request_id}, Batch ID: {result.batch_id}, Status: SUCCESS, Completed: {result.completed}/{result.total_requests}"
        )
        logger.info(
            f"Batch (batch_id: {result.batch_id}) completed: {result.completed}/{result.total_requests} successful for user (API key prefix): {api_key[:8]}..."
        )
        return result
        ...
        # Re-raise custom exceptions - they'll be handled by the global exception handler
        logger.info(
            f"BATCH_REQUEST_END - ID: {request_id}, Batch ID: {batch_id}, Status: ERROR"
        )
        raise
    except Exception as e:
        # Convert unexpected exceptions to InfrastructureError
        logger.error(
            f"Batch processing error for user (API key prefix) {api_key[:8]}..., batch_id {request.batch_id or 'N/A'}: {str(e)}",
            exc_info=True,
        )
        logger.info(
            f"BATCH_REQUEST_END - ID: {request_id}, Batch ID: {batch_id}, Status: INTERNAL_ERROR"
        )
        raise InfrastructureError(
...
async def get_batch_status(batch_id: str, api_key: str = Depends(verify_api_key)):
    ...
    """
    logger.info(
        f"Batch status requested for batch_id: {batch_id} by user (API key prefix): {api_key[:8]}..."
    )
    return {
        "batch_id": batch_id,
        "status": "COMPLETED_SYNC",
...
async def get_service_health(
    ...
    try:
        ...
        overall_healthy = resilience_health["healthy"] and service_health.get(
            "healthy", True
        )
        ...
        return {
            ...
            "service_type": "domain",
            "infrastructure": {"resilience": resilience_health},
            "domain_services": {"text_processing": service_health},
        }
    except Exception as e:
