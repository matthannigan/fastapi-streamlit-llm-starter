...
async def health_check():
    ...
    try:
        ...
        cache_healthy = (
            cache_stats.get("redis", {}).get("status") != "error"
            and cache_stats.get("memory", {}).get("status") != "unavailable"
            and cache_stats.get("performance", {}).get("status") != "unavailable"
            and "error" not in cache_stats
        )
