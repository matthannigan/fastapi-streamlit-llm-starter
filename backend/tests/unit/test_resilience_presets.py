...
from app.resilience_presets import (
    ResiliencePreset, 
    PresetManager, 
    StrategyType, 
    PRESETS,
...
from app.services.resilience import ResilienceStrategy, ResilienceConfig
...
class TestResiliencePreset:
    ...
    def test_preset_creation(self):
        ...
        preset = ResiliencePreset(
            ...
            operation_overrides={"qa": StrategyType.CRITICAL},
            environment_contexts=["test"]
        )
        
        assert preset.name == "Test"
        assert preset.retry_attempts == 3
        assert preset.default_strategy == StrategyType.BALANCED
        assert preset.operation_overrides["qa"] == StrategyType.CRITICAL
    ...
    def test_preset_to_dict(self):
        ...
        assert preset_dict["default_strategy"] == StrategyType.BALANCED
    ...
    def test_preset_to_resilience_config(self):
        ...
        assert config.strategy == StrategyType.BALANCED
...
class TestPresetDefinitions:
    ...
    def test_development_preset_specification(self):
        ...
        assert preset.default_strategy == StrategyType.AGGRESSIVE
        assert preset.operation_overrides["sentiment"] == StrategyType.AGGRESSIVE
        assert preset.operation_overrides["qa"] == StrategyType.BALANCED
        assert "development" in preset.environment_contexts
    ...
    def test_production_preset_specification(self):
        ...
        assert preset.default_strategy == StrategyType.CONSERVATIVE
        assert preset.operation_overrides["qa"] == StrategyType.CRITICAL
        assert preset.operation_overrides["sentiment"] == StrategyType.AGGRESSIVE
        assert preset.operation_overrides["summarize"] == StrategyType.CONSERVATIVE
        assert "production" in preset.environment_contexts
...
class TestSettingsIntegration:
    ...
    def test_get_resilience_config_preset(self):
        ...
        assert config.strategy == StrategyType.AGGRESSIVE
    ...
    def test_get_resilience_config_fallback_on_error(self):
        ...
        # Mock the preset_manager import inside the method
        with patch('app.resilience_presets.preset_manager') as mock_manager:
