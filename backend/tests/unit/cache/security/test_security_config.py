...
class TestSecurityConfigInitialization:
    ...
    def test_init_with_full_tls_creates_comprehensive_secure_configuration(
        ...
        )  # TLS + AUTH + certificate verification = HIGH
    ...
    def test_init_with_invalid_parameters_raises_configuration_error(
        ...
        assert (
            "cert_path" in exc_info.value.context
            or "tls_cert" in str(exc_info.value).lower()
    ...
    def test_post_init_raises_error_for_missing_ca_file(self, mock_path_exists):
        ...
        with pytest.raises(ConfigurationError) as exc_info:
            SecurityConfig(
                use_tls=True, verify_certificates=True, tls_ca_path="/etc/ssl/ca.pem"
            )
    ...
    def test_has_authentication_property_detects_auth_methods(self):
        ...
        config_both = SecurityConfig(
            redis_auth="auth_password", acl_username="user", acl_password="acl_password"
        )
    ...
    @patch("pathlib.Path.exists")
    def test_security_level_property_reflects_configuration_strength(
        self, mock_path_exists
    ):
        """
        ...
        config_standard = SecurityConfig(
            redis_auth="password123", use_tls=True, verify_certificates=True
        )
        ...
        config_tls_no_verify = SecurityConfig(
            redis_auth="password123", use_tls=True, verify_certificates=False
        )
        assert config_tls_no_verify.security_level == "MEDIUM"

    @patch("pathlib.Path.exists")
    def test_certificate_path_validation_prevents_invalid_configurations(
        self, mock_path_exists
    ):
        """
        ...
        assert (
            ...
            or "tls" in str(exc_info.value).lower()
        )
        ...
        assert isinstance(exc_info.value, ConfigurationError)
        assert (
            "key" in str(exc_info.value).lower() or "tls" in str(exc_info.value).lower()
        )
...
class TestSecurityConfigEnvironmentCreation:
    ...
    @patch("pathlib.Path.exists")
    def test_create_from_env_builds_secure_config_from_environment(
        self, mock_path_exists, mock_create_for_env
    ):
        """
    ...
    def test_create_from_env_returns_secure_config_when_no_security_variables(
        self, monkeypatch
    ):
        """
        ...
        with patch.dict(
            "os.environ", {"OTHER_VAR": "value", "PATH": "/usr/bin"}, clear=True
        ):
            config = create_security_config_from_env()
            assert config is not None
    ...
    def test_create_from_env_handles_invalid_environment_values(self, mock_path_exists):
        ...
        with patch(
            "app.infrastructure.cache.security.SecurityConfig.create_for_environment"
        ) as mock_create:
