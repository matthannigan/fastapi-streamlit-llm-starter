...
class TestWebApplicationCacheCreation:
    ...
    async def test_for_web_app_applies_custom_parameters_correctly(self):
        ...
        cache = await factory.for_web_app(
            redis_url=custom_redis_url, default_ttl=custom_ttl
        )
    ...
    @pytest.mark.asyncio
    async def test_for_web_app_handles_redis_connection_failure_with_memory_fallback(
        self,
    ):
        """
        ...
        cache = await factory.for_web_app(
            redis_url=impossible_redis_url, fail_on_connection_error=False
        )
    ...
    async def test_for_web_app_raises_error_when_redis_required(self):
        ...
        with patch(
            "app.infrastructure.cache.redis_generic.GenericRedisCache"
        ) as mock_redis:
            ...
            with pytest.raises(
                InfrastructureError,
                match="Redis connection failed for web application cache",
            ):
    ...
    async def test_for_web_app_strict_redis_requirement_with_invalid_url(self):
        ...
        with pytest.raises(InfrastructureError):
            await factory.for_web_app(
                redis_url=invalid_redis_url, fail_on_connection_error=True
            )
...
class TestAIApplicationCacheCreation:
    ...
    @pytest.mark.asyncio
    async def test_for_ai_app_integrates_with_performance_monitoring(
        ...
        if (
            hasattr(cache, "_performance_monitor")
            and cache._performance_monitor is not None
        ):
    ...
    async def test_for_ai_app_validates_operation_ttl_parameters(self):
        ...
        with pytest.raises(
            ValidationError, match="operation_ttls must be a dictionary"
        ):
    ...
    async def test_for_ai_app_handles_redis_connection_failure_with_fallback(self):
        ...
        cache = await factory.for_ai_app(
            redis_url="redis://nonexistent-host:9999", fail_on_connection_error=False
        )
    ...
    async def test_for_ai_app_raises_error_when_redis_required_for_ai(self):
        ...
        with patch(
            "app.infrastructure.cache.redis_ai.AIResponseCache"
        ) as mock_ai_class:
            ...
            with pytest.raises(
                InfrastructureError,
                match="Redis connection failed for AI application cache",
            ):
...
class TestConfigurationBasedCacheCreation:
    ...
    @pytest.mark.asyncio
    async def test_create_cache_from_config_creates_generic_cache_for_basic_config(
        self,
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_create_cache_from_config_creates_ai_cache_when_ai_params_present(
        self,
    ):
        """
    ...
    async def test_create_cache_from_config_validates_required_parameters(self):
        ...
        incomplete_config = {"default_ttl": 3600, "enable_l1_cache": True}
    ...
    async def test_create_cache_from_config_handles_redis_connection_failure(self):
        ...
        cache = await factory.create_cache_from_config(
            invalid_config, fail_on_connection_error=False
        )
        ...
        with patch(
            ...
            with pytest.raises(InfrastructureError, match="Redis connection failed"):
                await factory.create_cache_from_config(
                    valid_config, fail_on_connection_error=True
                )
