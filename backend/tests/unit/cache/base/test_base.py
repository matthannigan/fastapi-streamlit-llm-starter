...
class TestCacheInterfaceContract:
    ...
    def test_cache_interface_cannot_be_instantiated_directly(self):
        ...
        assert (
            "abstract class" in error_message.lower()
            or "can't instantiate" in error_message.lower()
        )
    ...
    def test_abstract_method_parameter_types_are_correctly_annotated(self):
        ...
        assert (
            get_params["key"].annotation == str
        ), "get() key parameter should be annotated as str"
        ...
        assert (
            set_params["key"].annotation == str
        ), "set() key parameter should be annotated as str"
        assert (
            set_params["value"].annotation == Any
        ), "set() value parameter should be annotated as Any"
        assert (
            set_params["ttl"].annotation == Optional[int]
        ), "set() ttl parameter should be annotated as Optional[int]"
        assert (
            set_params["ttl"].default is None
        ), "set() ttl parameter should have default None"
        ...
        assert (
            delete_params["key"].annotation == str
        ), "delete() key parameter should be annotated as str"
        ...
        assert (
            exists_params["key"].annotation == str
        ), "exists() key parameter should be annotated as str"

        # Verify all methods have proper parameter count (excluding self)
        assert (
            len([p for p in get_params if p != "self"]) == 1
        ), "get() should have exactly 1 parameter (plus self)"
        assert (
            len([p for p in set_params if p != "self"]) == 3
        ), "set() should have exactly 3 parameters (plus self)"
        assert (
            len([p for p in delete_params if p != "self"]) == 1
        ), "delete() should have exactly 1 parameter (plus self)"
        assert (
            len([p for p in exists_params if p != "self"]) == 1
        ), "exists() should have exactly 1 parameter (plus self)"
    ...
    def test_abstract_method_return_types_are_correctly_annotated(self):
        ...
        assert (
            exists_signature.return_annotation == bool
        ), "exists() method should return bool"
        ...
        for method_name in async_methods:
            ...
            assert inspect.iscoroutinefunction(
                method
            ), f"{method_name} should be awaitable"
...
class TestCacheInterfacePolymorphism:
    ...
    async def test_service_integration_supports_cache_switching(self):
        ...
        for i, cache_impl in enumerate(available_caches):
            ...
            assert (
                exists_result is True
            ), f"Item should exist with {type(cache_impl).__name__}"

            retrieved_data = await service.retrieve_item(test_key)
            assert (
                retrieved_data == test_data
            ), f"Data consistency failed with {type(cache_impl).__name__}"
            ...
            assert (
                exists_after_removal is False
            ), f"Item should be removed with {type(cache_impl).__name__}"
    ...
    async def test_liskov_substitution_principle_compliance(self):
        ...
        for i, cache_impl in enumerate(available_caches):
            ...
            try:
                ...
                ), f"Workflow should complete with {type(cache_impl).__name__}"
            except Exception as e:
                pytest.fail(
                    f"LSP violation: {type(cache_impl).__name__} failed workflow: {e}"
                )
        ...
        for cache_impl in available_caches:
            ...
            ), f"{type(cache_impl).__name__} should return None for missing keys"

            # All implementations should handle exists() for missing keys
            missing_exists = await cache_impl.exists("nonexistent:lsp:key")
            assert (
                missing_exists is False
            ), f"{type(cache_impl).__name__} should return False for missing keys"
            ...
            try:
                ...
                pytest.fail(
                    f"LSP violation: {type(cache_impl).__name__} should handle delete of missing keys gracefully: {e}"
                )
    ...
    async def test_interface_method_calls_work_through_polymorphic_references(self):
        ...
        if len(available_caches) == 0:
            pytest.skip(
                "No cache implementations available for polymorphic method dispatch test"
            )
        ...
        for i, cache_impl in enumerate(available_caches):
            ...
            for j, test_value in enumerate(test_values):
                ...
                assert (
                    exists_result is True
                ), f"exists() dispatch failed for {type(cache_impl).__name__} with value {test_value}"

                # Test get() method dispatch
                retrieved_value = await cache_interface.get(test_key)
                assert (
                    retrieved_value == test_value
                ), f"get() dispatch failed for {type(cache_impl).__name__} with value {test_value}"
            ...
            ), f"Async get() pattern failed for {type(cache_impl).__name__}"
            assert (
                exists is True
            ), f"Async exists() pattern failed for {type(cache_impl).__name__}"
        ...
        for cache_impl in available_caches:
            ...
            assert callable(delete_method) and inspect.iscoroutinefunction(
                delete_method
            )
            assert callable(exists_method) and inspect.iscoroutinefunction(
                exists_method
            )
