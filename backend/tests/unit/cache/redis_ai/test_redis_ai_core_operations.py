...
class TestAIResponseCacheCoreOperations:
    ...
    def test_build_key_generates_ai_optimized_cache_keys(
        ...
        result_key = cache.build_key(
            text=sample_text, operation=operation, options=sample_options
        )
        ...
        result_key_2 = cache.build_key(
            text=sample_text, operation=operation, options=sample_options
        )
        assert result_key == result_key_2

    def test_build_key_handles_questions_embedded_in_options(
        ...
        key_with_q1 = cache.build_key(
            text=sample_text, operation=operation, options=options_with_question1
        )
        key_with_q2 = cache.build_key(
            text=sample_text, operation=operation, options=options_with_question2
        )
        key_without_q = cache.build_key(
            text=sample_text, operation=operation, options=options_without_question
        )
    ...
    def test_build_key_handles_large_text_with_hashing(
        ...
        assert (
            len(sample_long_text) > 1000
        ), "sample_long_text should exceed hash threshold"

        # When: build_key is called with large text
        result_key = cache.build_key(
            text=sample_long_text, operation=operation, options=sample_options
        )
        ...
        assert len(result_key) < len(
            sample_long_text
        ), "Key should be much shorter than input text"

    def test_build_key_handles_small_text_without_hashing(
        ...
        assert (
            len(sample_short_text) < 1000
        ), "sample_short_text should be below hash threshold"

        # When: build_key is called with small text
        result_key = cache.build_key(
            text=sample_short_text, operation=operation, options=sample_options
        )
    ...
    async def test_standard_cache_interface_integration(
        self, sample_text, sample_ai_response, sample_options
    ):
        """
    ...
    def test_build_key_raises_validation_error_for_invalid_input(self):
        ...
        with pytest.raises(AttributeError):
            cache.build_key(
                text="Sample text", operation="summarize", options="invalid"
            )
    ...
    async def test_standard_cache_interface_cache_hit_scenario(
        self, sample_text, sample_options, sample_ai_response
    ):
        """
    ...
    async def test_standard_cache_interface_cache_miss_scenario(
        self, sample_text, sample_options
    ):
        """
        ...
        try:
            ...
            different_key = cache.build_key(
                text=sample_text, operation=different_operation, options=sample_options
            )
    ...
    async def test_standard_cache_interface_with_qa_operations(
        ...
        try:
            ...
            same_key_q1 = cache.build_key(
                text=text, operation=operation, options=options_q1
            )
    ...
    async def test_standard_cache_interface_validation_integration(
        ...
        try:
            ...
            with pytest.raises(AttributeError):
                cache.build_key(
                    text="Valid text", operation="summarize", options="invalid"
                )

            # And: Cache state remains clean after validation failures
            # Verify with a valid operation
            valid_key = cache.build_key(
                text="Valid text", operation="summarize", options={}
            )
