...
class TestAIResponseCacheInvalidation:
    ...
    async def test_invalidate_pattern_removes_matching_cache_entries(
        ...
        await cache.invalidate_pattern(
            pattern="summarize", operation_context="test_pattern_removal"
        )
        ...
        assert (
            await cache.get(sentiment_key) == sentiment_data["response"]
        )  # Should remain
        assert (
            await cache.get(questions_key) == questions_data["response"]
        )  # Should remain

    async def test_invalidate_pattern_records_performance_metrics(
        self, sample_text, sample_options, sample_ai_response, real_performance_monitor
    ):
        """
        ...
        test_key = cache.build_key(
            text=sample_text, operation="summarize", options=sample_options
        )
        ...
        await cache.invalidate_pattern(
            pattern=pattern, operation_context=operation_context
        )
    ...
    async def test_invalidate_pattern_handles_no_matches_gracefully(
        ...
        test_key = cache.build_key(
            text=sample_text, operation="sentiment", options=sample_options
        )
        await cache.set(test_key, sample_ai_response)
        ...
        try:
            await cache.invalidate_pattern(
                pattern=pattern_with_no_matches, operation_context=operation_context
            )
        ...
        assert (
            no_exception_raised
        ), "invalidate_pattern should handle zero matches gracefully"

        # And: Original cache entry remains unaffected
        assert await cache.get(test_key) == sample_ai_response

    async def test_invalidate_by_operation_removes_operation_specific_entries(
    ...
    async def test_invalidate_by_operation_records_comprehensive_metrics(
        self, sample_text, sample_options, sample_ai_response, real_performance_monitor
    ):
        """
        ...
        invalidated_count = await cache.invalidate_by_operation(
            operation=operation, operation_context=operation_context
        )
    ...
    async def test_invalidate_by_operation_raises_validation_error_for_invalid_operation(
        self, sample_text, sample_options, sample_ai_response
    ):
        """
        ...
        with pytest.raises(ValidationError) as exc_info:
            await cache.invalidate_by_operation(
                operation="", operation_context="test_empty_operation"
            )
    ...
    async def test_invalidate_by_operation_handles_no_matches_gracefully(
        self, sample_text, sample_options, sample_ai_response
    ):
        """
        ...
        invalidated_count = await cache.invalidate_by_operation(
            operation=nonexistent_operation, operation_context=operation_context
        )
    ...
    async def test_clear_removes_all_ai_cache_entries(self, ai_cache_test_data):
        ...
        for tier_name, tier_text in text_tiers.items():
            key = cache.build_key(
                text=tier_text, operation="test_tier", options={"tier": tier_name}
            )
    ...
    async def test_clear_records_maintenance_metrics(
        self, sample_text, sample_options, sample_ai_response, real_performance_monitor
    ):
        """
