...
class TestCallbackRegistration:
    ...
    async def test_register_single_callback(
        self, default_generic_redis_config, sample_callback_functions, fake_redis_client
    ):
        """
    ...
    async def test_register_multiple_callbacks_same_event(
        self, default_generic_redis_config, sample_callback_functions, fake_redis_client
    ):
        """
    ...
    async def test_register_callbacks_different_events(
        self, default_generic_redis_config, sample_callback_functions, fake_redis_client
    ):
        """
    ...
    async def test_supported_event_types(
        self, default_generic_redis_config, sample_callback_functions, fake_redis_client
    ):
        """
        ...
        supported_events = ["get_success", "get_miss", "set_success", "delete_success"]
...
class TestMultipleCallbackHandling:
    ...
    async def test_callback_registry_state_management(
        ...
        callback_state["registry_snapshots"].append(
            len(cache._callbacks["set_success"])
        )
        ...
        callback_state["registry_snapshots"].append(
            len(cache._callbacks["get_success"])
        )
        ...
        # Then: Registry state should be properly maintained
        assert (
            len(cache._callbacks["set_success"]) == 2
        )  # State unchanged after invocation
        assert (
            len(cache._callbacks["get_success"]) == 1
        )  # State unchanged after invocation

        # And: Callback history should be accurately tracked
        assert (
            len(callback_state["invocations"]) == 3
        )  # 2 set callbacks + 1 get callback

        set_invocations = [
            inv
            for inv in callback_state["invocations"]
            if inv["callback"] in ["callback1", "callback2"]
        ]
        get_invocations = [
            inv
            for inv in callback_state["invocations"]
            if inv["callback"] == "callback3"
        ]
        ...
        max_concurrent = max(
            inv["concurrent_level"] for inv in callback_state["invocations"]
        )
...
class TestCallbackErrorHandling:
    ...
    async def test_callback_memory_leak_prevention(
        self, default_generic_redis_config, fake_redis_client
    ):
        """
        ...
        assert (
            len(cache._callbacks["get_miss"]) == 10
        )  # get_miss callbacks were registered
        assert (
            len(cache._callbacks["delete_success"]) == 10
        )  # delete_success callbacks were registered
    ...
    async def test_callback_error_logging_and_monitoring(
        self, default_generic_redis_config, fake_redis_client, caplog
    ):
        """
        ...
        callback_state = {"successful_calls": 0, "error_calls": 0, "error_messages": []}
        ...
        cache.register_callback(
            "set_success", exception_callback
        )  # This will throw different error
        ...
        get_error_logged = any(
            "get_success" in msg and "failed" in msg for msg in error_log_messages
        )
        set_error_logged = any(
            "set_success" in msg and "failed" in msg for msg in error_log_messages
        )

        assert (
            get_error_logged
        ), f"Expected get_success callback error in logs: {error_log_messages}"
        assert (
            set_error_logged
        ), f"Expected set_success callback error in logs: {error_log_messages}"

        # And: Cache operations should continue to work despite callback errors
        assert (
            result == "error_value"
        )  # Cache operation succeeded despite callback error
        ...
        assert (
            callback_state["successful_calls"] >= 2
        )  # Successful callback called for both get operations
        assert (
            callback_state["error_calls"] >= 3
        )  # Error callbacks called (2 gets + 2 sets but may vary due to timing)
        ...
        assert (
            len(cache._callbacks["get_success"]) == 2
        )  # Both callbacks still registered
        assert (
            len(cache._callbacks["set_success"]) == 1
        )  # Error callback still registered
