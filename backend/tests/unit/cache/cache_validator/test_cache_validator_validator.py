...
class TestCacheValidatorInitialization:
    ...
    def test_cache_validator_initializes_template_registry(self):
        ...
        for template_name, template in validator.templates.items():
            assert all(
                field in template for field in expected_fields
            ), f"Template {template_name} missing required fields"
...
class TestCacheValidatorPresetValidation:
    ...
    def test_cache_validator_validate_preset_confirms_valid_preset_configurations(self):
        ...
        error_fields = [
            msg.field_path for msg in result.messages if msg.severity.value == "error"
        ]
    ...
    def test_cache_validator_validate_preset_identifies_invalid_preset_structure(self):
        ...
        missing_field_errors = [
            error for error in result.errors if "Missing required field" in error
        ]
        ...
        range_errors = [
            error
            for error in complete_result.errors
            if "must be integer between" in error
        ]
    ...
    def test_cache_validator_validate_preset_validates_ai_optimization_parameters(self):
        ...
        ai_preset = {
            ...
            "ai_optimizations": {
                ...
                "text_size_tiers": {"small": 1000, "medium": 5000, "large": 15000},
            },
        }
        ...
        invalid_ai_preset["ai_optimizations"] = {
            ...
            "operation_ttls": {"text_generation": 30},  # Too low (below 60)
        }
        ...
        info_messages = [
            msg.message
            for msg in no_opts_result.messages
            if msg.severity.value == "info" and "AI cache enabled" in msg.message
        ]
        assert len(info_messages) > 0

    def test_cache_validator_validate_preset_validates_environment_context_appropriateness(
        self,
    ):
        """
        ...
        invalid_contexts_preset["environment_contexts"] = [
            "unknown_context",
            "invalid_env",
            "production",
        ]
        ...
        unknown_warnings = [
            msg.message
            for msg in invalid_result.messages
            if msg.severity.value == "warning"
            and "Unknown environment context" in msg.message
        ]
        ...
        empty_warnings = [
            msg.message
            for msg in empty_result.messages
            if msg.severity.value == "warning"
            and "No environment contexts specified" in msg.message
        ]
        ...
        for context in valid_contexts:
            ...
            assert (
                len(context_warnings) == 0
            ), f"Valid context '{context}' generated warnings"
...
class TestCacheValidatorConfigurationValidation:
    ...
    def test_cache_validator_validate_configuration_confirms_complete_configurations(
        self,
    ):
        """
        ...
        critical_errors = [
            msg.message for msg in result.messages if msg.severity.value == "error"
        ]
    ...
    def test_cache_validator_validate_configuration_validates_redis_connection_parameters(
        self,
    ):
        """
        ...
        url_errors = [
            error for error in result.errors if "Invalid Redis URL format" in error
        ]
        ...
        url_errors = [
            error
            for error in invalid_url_result.errors
            if "Invalid Redis URL format" in error
        ]
        assert len(url_errors) > 0
        ...
        tls_warnings = [
            msg.message
            for msg in tls_result.messages
        ...
        tls_valid_warnings = [
            msg.message
            for msg in tls_valid_result.messages
            if msg.severity.value == "warning" and "rediss://" in msg.message
        ]
        assert len(tls_valid_warnings) == 0

    def test_cache_validator_validate_configuration_validates_performance_parameters(
        self,
    ):
        """
        ...
        perf_warnings = [
            ...
            for msg in result.messages
            if msg.severity.value == "warning"
            and "High connection count" in msg.message
        ]
        ...
        balanced_perf_warnings = [
            msg.message
            for msg in balanced_result.messages
    ...
    def test_cache_validator_validate_configuration_validates_strategy_alignment(self):
        ...
        assert (
            fast_config["default_ttl"] < robust_config["default_ttl"]
        )  # Fast < Robust TTL
        assert (
            fast_config["max_connections"] < robust_config["max_connections"]
        )  # Fast < Robust connections
        assert (
            robust_config["compression_level"] > fast_config["compression_level"]
        )  # Robust > Fast compression
...
class TestCacheValidatorCustomOverrideValidation:
    ...
    def test_cache_validator_validate_custom_overrides_confirms_valid_overrides(self):
        ...
        type_errors = [
            error for error in result.errors if "expects" in error and "got" in error
        ]
    ...
    def test_cache_validator_validate_custom_overrides_identifies_invalid_parameters(
        ...
        ttl_error = any(
            "default_ttl" in error and ("int" in error or "integer" in error)
            for error in type_errors
        )
        assert ttl_error, "default_ttl type error should be detected"

        ai_error = any(
            "enable_ai_cache" in error and "bool" in error for error in type_errors
        )
        ...
        assert (
            has_critical_issues
        ), "Invalid overrides should generate validation errors"

    def test_cache_validator_validate_custom_overrides_validates_json_yaml_structure(
        self,
    ):
        """
        ...
        structure_errors = [
            error for error in result.errors if "structure" in error.lower()
        ]
...
class TestCacheValidatorTemplateManagement:
    ...
    def test_cache_validator_get_template_retrieves_valid_configuration_templates(self):
        ...
        for field in essential_fields:
            assert (
                field in dev_template
            ), f"Development template missing required field: {field}"
        ...
        assert (
            dev_template["enable_ai_cache"] is False
        )  # AI disabled for fast development
        ...
        assert (
            prod_template["compression_level"] == 9
    ...
    def test_cache_validator_list_templates_returns_available_template_names(self):
        ...
        for expected in expected_templates:
            assert (
                expected in actual_templates
            ), f"Expected template '{expected}' not found in list"

        # And: Template names are descriptive of their deployment scenarios
        # Names should indicate their purpose
        assert any(
            "development" in name for name in template_names
        ), "Should include development template"
        assert any(
            "production" in name for name in template_names
        ), "Should include production template"
        assert any(
            "ai" in name for name in template_names
        ), "Should include AI-optimized template"
        ...
        for name in template_names:
            assert isinstance(
                name, str
            ), f"Template name should be string, got {type(name)}"
            assert len(name) > 0, "Template name should not be empty"
            assert (
                "_" in name or "-" in name
            ), f"Template name '{name}' should be descriptive with separators"
    ...
    def test_cache_validator_templates_pass_comprehensive_validation(self):
        ...
        for template_name, result in validation_results.items():
            ...
            ), f"Validation result for '{template_name}' should be ValidationResult"

            # Templates should not have critical validation errors
            critical_errors = [error for error in result.errors if error]
            assert (
                len(critical_errors) == 0
            ), f"Template '{template_name}' has validation errors: {critical_errors}"
        ...
        assert (
            len(dev_result.errors) == 0
        ), f"Development template validation errors: {dev_result.errors}"

        # robust_production template passes validation
        prod_template = validator.get_template("robust_production")
        prod_result = validator.validate_configuration(prod_template)
        assert (
            len(prod_result.errors) == 0
        ), f"Production template validation errors: {prod_result.errors}"

        # ai_optimized template passes validation
        ai_template = validator.get_template("ai_optimized")
        ai_result = validator.validate_configuration(ai_template)
        assert (
            len(ai_result.errors) == 0
        ), f"AI template validation errors: {ai_result.errors}"
        ...
        for template_name in template_names:
            ...
            if "default_ttl" in template:
                assert (
                    template["default_ttl"] >= 60
                ), f"Template '{template_name}' TTL too low"
                assert (
                    template["default_ttl"] <= 86400
                ), f"Template '{template_name}' TTL too high"

            if "max_connections" in template:
                assert (
                    template["max_connections"] >= 1
                ), f"Template '{template_name}' connections too low"
                assert (
                    template["max_connections"] <= 100
                ), f"Template '{template_name}' connections too high"

            if "compression_level" in template:
                assert (
                    template["compression_level"] >= 1
                ), f"Template '{template_name}' compression level too low"
                assert (
                    template["compression_level"] <= 9
                ), f"Template '{template_name}' compression level too high"
        ...
        for template_name in template_names:
            ...
            for field in required_deployment_fields:
                assert (
                    field in template
                ), f"Template '{template_name}' missing deployment field '{field}'"

            # Strategy should be valid
            assert template["strategy"] in [
                "fast",
                "balanced",
                "robust",
                "ai_optimized",
...
class TestCacheValidatorConfigurationComparison:
    ...
    def test_cache_validator_compare_configurations_identifies_parameter_differences(
        self,
    ):
        """
        ...
        strategy_change = next(
            (change for change in changed_values if change["key"] == "strategy"), None
        )
        ...
        ttl_change = next(
            (change for change in changed_values if change["key"] == "default_ttl"),
            None,
        )
        ...
        compression_change = next(
            (
                change
                for change in changed_values
                if change["key"] == "compression_level"
            ),
            None,
        )
        ...
        identical_differences = validator.compare_configurations(
            config1, identical_config
        )
    ...
    def test_cache_validator_compare_configurations_analyzes_performance_impact(self):
        ...
        assert ttl_change is not None
        assert (
            ttl_change["old_value"] < ttl_change["new_value"]
        )  # TTL increased (better cache hit rate)

        # Connection pool changes analyzed for throughput impact
        conn_change = next(
            (change for change in changed_values if change["key"] == "max_connections"),
            None,
        )
        assert conn_change is not None
        assert (
            conn_change["old_value"] < conn_change["new_value"]
        ...
        assert (
            comp_change["old_value"] < comp_change["new_value"]
        )  # Higher compression (lower bandwidth)
        ...
        ai_change = next(
            (change for change in changed_values if change["key"] == "enable_ai_cache"),
            None,
        )
        ...
        degradation_differences = validator.compare_configurations(
            optimized_config, degraded_config
        )
        degradation_changes = degradation_differences["changed_values"]

        # Performance degradation should be visible in the comparison
        ttl_degradation = next(
            (
                change
                for change in degradation_changes
                if change["key"] == "default_ttl"
            ),
            None,
        )
        assert (
            ttl_degradation["old_value"] > ttl_degradation["new_value"]
        )  # TTL decreased (worse performance)

        conn_degradation = next(
            (
                change
                for change in degradation_changes
                if change["key"] == "max_connections"
            ),
            None,
        )
        assert (
            conn_degradation["old_value"] > conn_degradation["new_value"]
        )  # Fewer connections (lower throughput)
