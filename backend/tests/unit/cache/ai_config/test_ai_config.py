...
class TestAIResponseCacheConfigValidation:
    ...
    def test_validate_returns_valid_result_for_default_configuration(self):
        ...
        assert (
            result.is_valid
        ), f"Default configuration should be valid. Errors: {result.errors}"
        assert (
            len(result.errors) == 0
        ), f"Should have no validation errors, got: {result.errors}"
    ...
    def test_validate_identifies_invalid_redis_url_format(self):
        ...
        for invalid_url in invalid_urls:
            ...
            if not result.is_valid:
                assert (
                    len(result.errors) > 0
                ), f"Should have validation errors for URL '{invalid_url}'"
                error_text = " ".join(result.errors).lower()
                assert (
                    "redis" in error_text or "url" in error_text
    ...
    def test_validate_identifies_ttl_out_of_range(self):
        ...
        for ttl_value, description in invalid_ttl_configs:
            ...
            assert (
                not result.is_valid
            ), f"{description} should be invalid (value: {ttl_value})"
            assert (
                len(result.errors) > 0
            ), f"Should have validation errors for {description}"

            error_text = " ".join(result.errors).lower()
            assert (
                "ttl" in error_text
            ), f"Error should mention TTL for {description}. Errors: {result.errors}"
        ...
        assert (
            not result.is_valid
        ), "Configuration with invalid operation TTLs should be invalid"
        assert (
            len(result.errors) > 0
        ), "Should have validation errors for invalid operation TTLs"
    ...
    def test_validate_identifies_memory_cache_size_violations(self):
        ...
        for size_value, description in invalid_sizes:
            ...
            if not result.is_valid:
                ...
                error_text = " ".join(result.errors).lower()
                assert any(
                    term in error_text for term in ["memory", "cache", "size"]
        ...
        if result_zero.is_valid:
            assert (
                len(result_zero.recommendations) > 0 or len(result_zero.warnings) > 0
    ...
    def test_validate_identifies_compression_parameter_issues(self):
        ...
        for threshold_value, description in invalid_threshold_configs:
            ...
            assert (
                not result.is_valid
            ), f"{description} should be invalid (value: {threshold_value})"
            assert (
                len(result.errors) > 0
            ), f"Should have validation errors for {description}"
            ...
            error_text = " ".join(result.errors).lower()
            assert (
                "compression" in error_text or "threshold" in error_text
        ...
        for level_value, description in invalid_level_configs:
            ...
            assert (
                not result.is_valid
            ), f"{description} should be invalid (value: {level_value})"
            assert (
                len(result.errors) > 0
            ), f"Should have validation errors for {description}"
            ...
            error_text = " ".join(result.errors).lower()
            assert (
                "compression" in error_text or "level" in error_text
    ...
    def test_validate_identifies_text_processing_parameter_issues(self):
        ...
        for threshold_value, description in invalid_hash_thresholds:
            ...
            error_text = " ".join(result.errors).lower()
            assert any(
                term in error_text for term in ["text", "hash", "threshold"]
        ...
        if not result.is_valid:
            error_text = " ".join(result.errors).lower()
            assert (
                "hash" in error_text or "algorithm" in error_text
        ...
        if not result.is_valid:
            error_text = " ".join(result.errors).lower()
            assert any(
                term in error_text for term in ["tier", "size", "text"]
    ...
    def test_validate_generates_performance_recommendations(self):
        ...
        for config, expected_topic in configs_with_expected_recommendations:
            ...
            assert (
                result.is_valid
            ), f"Configuration should be valid. Errors: {result.errors}"
            assert isinstance(
                result.recommendations, list
            ), "Recommendations should be a list"
        ...
        assert (
            result.is_valid
        ), f"Suboptimal but valid configuration should pass validation. Errors: {result.errors}"
        assert isinstance(
            result.recommendations, list
    ...
    def test_validate_handles_complex_configuration_scenarios(self):
        ...
        assert (
            not result.is_valid
        ), "Configuration with multiple issues should be invalid"
        assert (
            len(result.errors) >= 3
        ), f"Should have multiple validation errors. Got: {result.errors}"
        ...
        assert (
            topics_found >= 2
        ), f"Should capture errors from multiple parameter categories. Errors: {result.errors}"
...
class TestAIResponseCacheConfigFactory:
    ...
    def test_create_default_returns_development_friendly_configuration(self):
        ...
        assert isinstance(
            config, AIResponseCacheConfig
        ), "Should return AIResponseCacheConfig instance"
        ...
        assert (
            config.text_hash_threshold > 0
        ), "Should have positive text hash threshold"
        assert (
            config.compression_threshold >= 0
        ), "Should have valid compression threshold"
        assert 1 <= config.compression_level <= 9, "Should have valid compression level"

        # Verify AI-specific features are configured
        assert (
            config.operation_ttls is not None
        ), "Should have operation TTLs configured"
        assert (
            config.text_size_tiers is not None
        ), "Should have text size tiers configured"

        # Verify reasonable defaults for development
        assert (
            config.memory_cache_size <= 1000
        ), "Development config should have reasonable memory cache size"
        assert (
            config.default_ttl <= 86400
        ), "Development config should have reasonable TTL (<=24h)"
    ...
    def test_create_production_returns_production_optimized_configuration(self):
        ...
        assert (
            result.is_valid
        ), f"Production configuration should be valid. Errors: {result.errors}"
        ...
        ), "Production should have larger memory cache"

        # Verify compression is optimized for production
        assert (
            config.compression_threshold <= 2000
        ), "Production should use aggressive compression"
        assert (
            config.compression_level >= 6
        ), "Production should use good compression level"
        ...
        assert (
            config.memory_cache_size >= default_config.memory_cache_size
    ...
    def test_create_development_returns_development_optimized_configuration(self):
        ...
        assert (
            result.is_valid
        ), f"Development configuration should be valid. Errors: {result.errors}"
        ...
        ), "Development should have moderate memory cache size"
        assert (
            config.default_ttl <= 7200
        ), "Development should have shorter TTL (<=2h) for faster iteration"

        # Verify minimal but effective compression for development speed
        assert (
            config.compression_level <= 7
        ), "Development should use moderate compression for speed"
    ...
    def test_create_testing_returns_testing_optimized_configuration(self):
        ...
        assert (
            result.is_valid
        ), f"Testing configuration should be valid. Errors: {result.errors}"
        ...
        assert (
            config.memory_cache_size <= 50
        ), "Testing should have small memory cache for efficiency"
        assert (
            config.default_ttl <= 600
        ), "Testing should have very short TTL (<=10m) for fast tests"

        # Verify minimal compression for test speed
        assert (
            config.compression_level <= 6
        ), "Testing should use minimal compression for speed"
        assert (
            config.compression_threshold >= 1000
        ), "Testing should avoid compression for small test data"
        ...
        assert (
            config.default_ttl <= dev_config.default_ttl
        ), "Testing TTL should be <= development TTL"
    ...
    def test_from_dict_creates_configuration_from_dictionary_data(self):
        ...
        complete_config_dict = {
            ...
            "operation_ttls": {"summarize": 7200, "sentiment": 14400, "qa": 1800},
        }
        ...
        assert (
            ...
            == complete_config_dict["compression_threshold"]
        )
        ...
        assert (
            result.is_valid
        ), f"Configuration from dictionary should be valid. Errors: {result.errors}"
        ...
        assert (
            partial_config.memory_cache_size == partial_config_dict["memory_cache_size"]
        )
        ...
        ), f"Empty dictionary config should be valid. Errors: {empty_result.errors}"
    ...
    def test_from_dict_raises_configuration_error_for_invalid_parameters(self):
        ...
        try:
            ...
            assert isinstance(
                config, AIResponseCacheConfig
            ), "Should create config despite unknown params"
            assert (
    ...
    def test_from_env_loads_configuration_from_preset_system(self, monkeypatch):
        ...
        with patch.dict(
            ...
            assert (
                result.is_valid
            ), f"Environment configuration should be valid. Errors: {result.errors}"

            # Should have some configuration values set (either from env or defaults)
            assert config.default_ttl > 0, "Should have positive TTL"
            assert (
                config.memory_cache_size > 0
            ), "Should have positive memory cache size"
        ...
        cache_env_vars.extend(
            [k for k in os.environ.keys() if k.startswith("AI_CACHE_")]
        )
        ...
        assert (
            result.is_valid
        ), f"Default environment configuration should be valid. Errors: {result.errors}"

        # Test environment variables with custom prefix
        with patch.dict(
            os.environ,
            {"CUSTOM_DEFAULT_TTL": "1800", "CUSTOM_MEMORY_CACHE_SIZE": "50"},
            clear=False,
        ):
            config = AIResponseCacheConfig.from_env(prefix="CUSTOM_")
            assert isinstance(
                config, AIResponseCacheConfig
            ), "Should return configuration with custom prefix"

            result = config.validate()
            assert (
                result.is_valid
            ), f"Custom prefix configuration should be valid. Errors: {result.errors}"
  ...
  qa: 2700
        ...
        with patch(
            ...
            mock_yaml_load.return_value = {
                ...
                "text_hash_threshold": 2500,
                "hash_algorithm": "sha256",
                "text_size_tiers": {"small": 400, "medium": 4000, "large": 40000},
                "operation_ttls": {"summarize": 10800, "sentiment": 21600, "qa": 2700},
            }
            ...
            assert (
                result.is_valid
            ), f"YAML configuration should be valid. Errors: {result.errors}"
        ...
        with patch("app.infrastructure.cache.ai_config.yaml", None):
            ...
            assert (
                "yaml" in error_msg
            ), f"Error should mention YAML library requirement. Got: {exc_info.value}"
  ...
  "operation_ttls": {
    ...
    "key_points": 7200
        ...
        with patch("builtins.open", mock_open(read_data=json_content)) as mock_file:
            ...
            assert (
                result.is_valid
            ), f"JSON configuration should be valid. Errors: {result.errors}"
        ...
        with patch("builtins.open", mock_open(read_data=invalid_json)):
            ...
            error_msg = str(exc_info.value).lower()
            assert any(
                term in error_msg for term in ["json", "parse", "syntax", "invalid"]
        ...
        with patch("builtins.open", side_effect=FileNotFoundError("File not found")):
            ...
            error_msg = str(exc_info.value).lower()
            assert any(
                term in error_msg for term in ["file", "found", "missing"]
...
class TestAIResponseCacheConfigConversion:
    ...
    def test_to_ai_cache_kwargs_converts_for_legacy_ai_cache_constructor(self):
        ...
        config = AIResponseCacheConfig(
            ...
            operation_ttls={"summarize": 7200, "sentiment": 10800, "qa": 3600},
            text_size_tiers={"small": 400, "medium": 4000, "large": 40000},
        )
        ...
        assert (
            kwargs.get("redis_url") == "redis://legacy-test:6379/2"
        ), "Should include redis_url"
        ...
        assert (
            "memory_cache_size" in kwargs
        ), "Should use legacy parameter name memory_cache_size"
        assert (
            kwargs["memory_cache_size"] == 150
        ), "Should map memory_cache_size correctly"
        assert (
            "l1_cache_size" not in kwargs
        ), "Should not use new parameter name l1_cache_size"
        assert (
            "enable_l1_cache" not in kwargs
        ), "Should not use new parameter name enable_l1_cache"

        # Verify compression parameters are included
        assert (
            kwargs.get("compression_threshold") == 1500
        ), "Should include compression_threshold"
        assert kwargs.get("compression_level") == 7, "Should include compression_level"

        # Verify AI-specific parameters are included
        assert (
            kwargs.get("text_hash_threshold") == 2000
        ), "Should include text_hash_threshold"
        assert "hash_algorithm" in kwargs, "Should include hash_algorithm"
        assert callable(
            kwargs.get("hash_algorithm")
        ), "hash_algorithm should be a callable function"
        ...
        assert isinstance(
            kwargs["memory_cache_size"], int
        ), "memory_cache_size should be integer"
        assert isinstance(
            kwargs["compression_threshold"], int
        ), "compression_threshold should be integer"
        assert isinstance(
            kwargs["text_hash_threshold"], int
        ), "text_hash_threshold should be integer"
        assert isinstance(
            kwargs["operation_ttls"], dict
        ), "operation_ttls should be dictionary"
        assert isinstance(
            kwargs["text_size_tiers"], dict
        ), "text_size_tiers should be dictionary"
    ...
    def test_to_generic_cache_kwargs_converts_for_new_modular_architecture(self):
        ...
        # Verify core Redis parameters are included
        assert (
            kwargs.get("redis_url") == "redis://new-arch:6379/3"
        ), "Should include redis_url"
        ...
        assert (
            "enable_l1_cache" in kwargs
        ), "Should use new parameter name enable_l1_cache"
        assert (
            kwargs["enable_l1_cache"] is True
        ), "Should enable L1 cache when memory_cache_size > 0"
        assert "l1_cache_size" in kwargs, "Should use new parameter name l1_cache_size"
        assert (
            kwargs["l1_cache_size"] == 200
        ), "Should map memory_cache_size to l1_cache_size"
        assert (
            "memory_cache_size" not in kwargs
        ), "Should not use legacy parameter name memory_cache_size"

        # Verify compression parameters are included (generic parameters)
        assert (
            kwargs.get("compression_threshold") == 800
        ), "Should include compression_threshold"
        assert kwargs.get("compression_level") == 6, "Should include compression_level"

        # Verify AI-specific parameters are excluded
        assert (
            "text_hash_threshold" not in kwargs
        ), "Should exclude AI-specific text_hash_threshold"
        assert (
            "hash_algorithm" not in kwargs
        ), "Should exclude AI-specific hash_algorithm"
        assert (
            "operation_ttls" not in kwargs
        ), "Should exclude AI-specific operation_ttls"
        assert (
            "text_size_tiers" not in kwargs
        ), "Should exclude AI-specific text_size_tiers"

        # Verify parameter types are correct for GenericRedisCache
        assert isinstance(kwargs["redis_url"], str), "redis_url should be string"
        assert isinstance(kwargs["default_ttl"], int), "default_ttl should be integer"
        assert isinstance(
            kwargs["enable_l1_cache"], bool
        ), "enable_l1_cache should be boolean"
        assert isinstance(
            kwargs["l1_cache_size"], int
        ), "l1_cache_size should be integer"
        assert isinstance(
            kwargs["compression_threshold"], int
        ), "compression_threshold should be integer"
    ...
    def test_parameter_conversion_maintains_data_integrity(self):
        ...
        config = AIResponseCacheConfig(
            ...
            text_hash_threshold=2500,
            # hash_algorithm uses default function value
            operation_ttls={"summarize": 10800, "sentiment": 14400, "qa": 5400},
        )
        ...
        assert (
            ai_kwargs["redis_url"] == generic_kwargs["redis_url"]
        ), "redis_url should be identical across conversions"
        assert (
            ai_kwargs["redis_url"] == "redis://integrity-test:6379/4"
        ), "redis_url should preserve original value"

        # Verify default_ttl is preserved accurately
        assert (
            ai_kwargs["default_ttl"] == generic_kwargs["default_ttl"]
        ), "default_ttl should be identical across conversions"
        assert (
            ai_kwargs["default_ttl"] == 7200
        ), "default_ttl should preserve original value"

        # Verify compression parameters maintain exact values
        assert (
            ai_kwargs["compression_threshold"]
            == generic_kwargs["compression_threshold"]
        ), "compression_threshold should be identical"
        assert (
            ai_kwargs["compression_threshold"] == 1200
        ), "compression_threshold should preserve original value"
        assert (
            ai_kwargs["compression_level"] == generic_kwargs["compression_level"]
        ), "compression_level should be identical"
        assert (
            ai_kwargs["compression_level"] == 8
        ), "compression_level should preserve original value"

        # Verify parameter types are preserved correctly
        assert (
            type(ai_kwargs["redis_url"]) == type(generic_kwargs["redis_url"]) == str
        ), "redis_url type should be consistent"
        assert (
            type(ai_kwargs["default_ttl"]) == type(generic_kwargs["default_ttl"]) == int
        ), "default_ttl type should be consistent"
        assert (
            type(ai_kwargs["compression_threshold"])
            == type(generic_kwargs["compression_threshold"])
            == int
        ), "compression_threshold type should be consistent"

        # Verify no data loss occurs - original values can be reconstructed
        assert (
            config.redis_url == ai_kwargs["redis_url"]
        ), "Original redis_url should match AI conversion"
        assert (
            config.default_ttl == generic_kwargs["default_ttl"]
        ), "Original default_ttl should match generic conversion"
        assert (
            config.compression_threshold == ai_kwargs["compression_threshold"]
        ), "Original compression_threshold should be preserved"
        ...
        assert (
            ai_kwargs["memory_cache_size"] == generic_kwargs["l1_cache_size"]
        ), "Memory cache size should map consistently"
        assert (
            ai_kwargs["memory_cache_size"] == 180
        ), "Memory cache size should preserve original value"
    ...
    def test_parameter_conversion_handles_optional_parameters_correctly(self):
        ...
        assert (
            "default_ttl" in ai_kwargs_minimal
        ), "Should include default_ttl with default value"
        assert (
            "memory_cache_size" in ai_kwargs_minimal
        ), "Should include memory_cache_size with default value"

        # Verify default values are reasonable
        assert ai_kwargs_minimal["default_ttl"] > 0, "Default TTL should be positive"
        assert (
            ai_kwargs_minimal["memory_cache_size"] > 0
        ), "Default memory cache size should be positive"
        ...
        assert (
            ai_kwargs_mixed["default_ttl"] == 5400
        ), "Specified default_ttl should be preserved"
        assert (
            ai_kwargs_mixed["compression_threshold"] == 2000
        ), "Specified compression_threshold should be preserved"
        assert (
            ai_kwargs_mixed["text_hash_threshold"] == 1500
        ), "Specified text_hash_threshold should be preserved"
        assert (
            ai_kwargs_mixed["operation_ttls"]["summarize"] == 7200
        ), "Specified operation_ttls should be preserved"

        # Verify defaults are applied for unspecified parameters
        assert (
            "compression_level" in ai_kwargs_mixed
        ), "Should have default compression_level"
        assert (
            ai_kwargs_mixed["compression_level"] > 0
        ), "Default compression_level should be reasonable"
        assert "hash_algorithm" in ai_kwargs_mixed, "Should have default hash_algorithm"
        assert callable(
            ai_kwargs_mixed["hash_algorithm"]
        ), "Default hash_algorithm should be callable function"

        # Verify complex parameters handle partial specification
        assert isinstance(
            ai_kwargs_mixed["operation_ttls"], dict
        ), "operation_ttls should be dictionary"
        assert (
            len(ai_kwargs_mixed["operation_ttls"]) >= 1
        ), "operation_ttls should contain specified values"
        assert isinstance(
            ai_kwargs_mixed["text_size_tiers"], dict
        ), "text_size_tiers should have defaults"

        # Verify generic conversion excludes AI parameters but preserves common ones
        assert (
            generic_kwargs_mixed["default_ttl"] == 5400
        ), "Generic conversion should preserve specified default_ttl"
        assert (
            generic_kwargs_mixed["compression_threshold"] == 2000
        ), "Generic conversion should preserve compression settings"
        assert (
            "text_hash_threshold" not in generic_kwargs_mixed
        ), "Generic conversion should exclude AI parameters"
        assert (
            "operation_ttls" not in generic_kwargs_mixed
        ), "Generic conversion should exclude AI parameters"
    ...
    def test_parameter_mapping_excludes_ai_specific_parameters_for_generic(self):
        ...
        config = AIResponseCacheConfig(
            ...
            text_size_tiers={"small": 350, "medium": 3500, "large": 35000},
        )
        ...
        assert (
            "redis_url" in generic_kwargs
        ), "Should include generic redis_url parameter"
        assert (
            generic_kwargs["redis_url"] == "redis://separation-test:6379/5"
        ), "Should preserve redis_url value"

        assert (
            "default_ttl" in generic_kwargs
        ), "Should include generic default_ttl parameter"
        assert (
            generic_kwargs["default_ttl"] == 8100
        ), "Should preserve default_ttl value"

        assert (
            "enable_l1_cache" in generic_kwargs
        ), "Should include generic enable_l1_cache parameter"
        assert (
            "l1_cache_size" in generic_kwargs
        ), "Should include generic l1_cache_size parameter"
        assert (
            generic_kwargs["l1_cache_size"] == 220
        ), "Should map memory_cache_size to l1_cache_size"

        assert (
            "compression_threshold" in generic_kwargs
        ), "Should include generic compression_threshold parameter"
        assert (
            generic_kwargs["compression_threshold"] == 1800
        ), "Should preserve compression_threshold value"
        assert (
            "compression_level" in generic_kwargs
        ), "Should include generic compression_level parameter"
        assert (
            generic_kwargs["compression_level"] == 9
        ), "Should preserve compression_level value"

        # Verify AI-specific parameters are excluded
        assert (
            "text_hash_threshold" not in generic_kwargs
        ), "Should exclude AI-specific text_hash_threshold"
        assert (
            "hash_algorithm" not in generic_kwargs
        ), "Should exclude AI-specific hash_algorithm"
        assert (
            "text_size_tiers" not in generic_kwargs
        ), "Should exclude AI-specific text_size_tiers"
        assert (
            "operation_ttls" not in generic_kwargs
        ), "Should exclude AI-specific operation_ttls"
        ...
        for ai_param in ai_specific_params:
            assert (
                ai_param not in generic_kwargs
            ), f"Should exclude AI-specific parameter {ai_param}"
        ...
        unexpected_params = (
            actual_params
            - expected_generic_params
            - {"performance_monitor", "security_config"}
        )
        ...
        assert (
            not ai_params_found
        ), f"Found unexpected AI-specific parameters: {ai_params_found}"

        # Verify parameter types are correct for GenericRedisCache
        assert isinstance(
            generic_kwargs["redis_url"], str
        ), "redis_url should be string"
        assert isinstance(
            generic_kwargs["default_ttl"], int
        ), "default_ttl should be integer"
        assert isinstance(
            generic_kwargs["enable_l1_cache"], bool
        ), "enable_l1_cache should be boolean"
        assert isinstance(
            generic_kwargs["l1_cache_size"], int
        ), "l1_cache_size should be integer"
...
class TestAIResponseCacheConfigMerging:
    ...
    def test_merge_combines_configurations_with_precedence(self):
        ...
        base_config = AIResponseCacheConfig(
            ...
            text_size_tiers={"small": 200, "medium": 2000, "large": 20000},
        )
        ...
        assert isinstance(
            merged_config, AIResponseCacheConfig
        ), "Should return AIResponseCacheConfig instance"
        assert (
            merged_config is not base_config
        ), "Should create new configuration instance"
        assert (
            merged_config is not override_config
        ), "Should create new configuration instance"

        # Verify override values take precedence (explicit overrides detected by smart merge)
        assert (
            merged_config.redis_url == "redis://override:6379/1"
        ), "Override redis_url should take precedence"
        assert (
            merged_config.default_ttl == 7200
        ), "Override default_ttl should take precedence"
        assert (
            merged_config.compression_threshold == 1500
        ), "Override compression_threshold should take precedence"
        assert (
            merged_config.text_hash_threshold == 2000
        ), "Override text_hash_threshold should take precedence"

        # Verify base values are preserved when override doesn't specify
        assert (
            merged_config.memory_cache_size == 50
        ), "Base memory_cache_size should be preserved"
        assert (
            merged_config.compression_level == 4
        ), "Base compression_level should be preserved"
        ...
        assert (
            merged_config.operation_ttls["summarize"] == 5400
        ), "Override operation_ttls should replace base"
        assert (
            merged_config.operation_ttls["qa"] == 1800
        ), "Override operation_ttls should be used"
        assert (
            "sentiment" not in merged_config.operation_ttls
        ), "Base operation_ttls should be replaced, not merged"

        assert (
            merged_config.text_size_tiers["small"] == 300
        ), "Override text_size_tiers should replace base"
        assert (
            merged_config.text_size_tiers["medium"] == 3000
        ), "Override text_size_tiers should be used"
        assert (
            merged_config.text_size_tiers["large"] == 30000
        ), "Override text_size_tiers should be used"
        ...
        assert (
            result.is_valid
        ), f"Merged configuration should be valid. Errors: {result.errors}"

        # Verify original configurations are unchanged
        assert (
            base_config.redis_url == "redis://base:6379/0"
        ), "Base config should remain unchanged"
        assert (
            override_config.redis_url == "redis://override:6379/1"
        ), "Override config should remain unchanged"
    ...
    def test_merge_with_combines_explicit_override_values(self):
        ...
        base_config = AIResponseCacheConfig(
            ...
            text_size_tiers={"small": 150, "medium": 1500, "large": 15000},
        )
        ...
        assert (
            merged_config.redis_url == "redis://explicit:6379/2"
        ), "Explicit redis_url override should be applied"
        assert (
            merged_config.default_ttl == 7200
        ), "Explicit default_ttl override should be applied"
        assert (
            merged_config.compression_level == 8
        ), "Explicit compression_level override should be applied"

        # Verify non-specified parameters retain base values
        assert (
            merged_config.memory_cache_size == 75
        ), "Non-specified memory_cache_size should retain base value"
        assert (
            merged_config.compression_threshold == 1500
        ), "Non-specified compression_threshold should retain base value"
        assert (
            merged_config.text_hash_threshold == 800
        ), "Non-specified text_hash_threshold should retain base value"

        # Verify complex parameter override behavior (complete replacement)
        assert (
            merged_config.operation_ttls["summarize"] == 10800
        ), "Explicit operation TTL override should be applied"
        assert (
            merged_config.operation_ttls["qa"] == 3600
        ), "New operation TTL should be added"
        assert (
            "sentiment" not in merged_config.operation_ttls
        ), "operation_ttls is completely replaced, not merged"
        assert (
            "questions" not in merged_config.operation_ttls
        ), "operation_ttls is completely replaced, not merged"

        # Verify text_size_tiers remain unchanged (not explicitly overridden)
        assert (
            merged_config.text_size_tiers["small"] == 150
        ), "Non-overridden text tier should be preserved"
        assert (
            merged_config.text_size_tiers["medium"] == 1500
        ), "Non-overridden text tier should be preserved"
        assert (
            merged_config.text_size_tiers["large"] == 15000
        ), "Non-overridden text tier should be preserved"
        ...
        assert (
            minimal_override.memory_cache_size == 200
        ), "Only specified parameter should change"
        assert (
            minimal_override.redis_url == base_config.redis_url
        ), "Non-specified parameters should remain unchanged"
        assert (
            minimal_override.default_ttl == base_config.default_ttl
        ), "Non-specified parameters should remain unchanged"
        assert (
            minimal_override.compression_threshold == base_config.compression_threshold
        ), "Non-specified parameters should remain unchanged"
        assert (
            minimal_override.operation_ttls == base_config.operation_ttls
        ), "Non-specified complex parameters should remain unchanged"
        ...
        assert (
            partial_ttl_override.operation_ttls["summarize"] == 14400
        ), "Overridden operation TTL should change"
        assert (
            "sentiment" not in partial_ttl_override.operation_ttls
        ), "operation_ttls is completely replaced"
        assert (
            "questions" not in partial_ttl_override.operation_ttls
        ), "operation_ttls is completely replaced"
        assert (
            partial_ttl_override.operation_ttls["analyze"] == 1800
        ), "New operation TTL should be added"
    ...
    def test_merge_preserves_base_configuration_defaults(self):
        ...
        assert (
            merged.redis_url == "redis://failover:6379/1"
        ), "Redis URL should be overridden"
        ...
        assert (
            merged.default_ttl == 7200
        ), "Tuned default_ttl should be preserved from base configuration"
        assert (
            merged.memory_cache_size == 500
        ), "Tuned memory_cache_size should be preserved"
        assert (
            merged.compression_threshold == 800
        ), "Tuned compression_threshold should be preserved"
        assert (
            merged.compression_level == 7
        ), "Tuned compression_level should be preserved"
        assert (
            merged.text_hash_threshold == 1500
        ), "Tuned text_hash_threshold should be preserved"

        # Verify complex parameters are preserved entirely
        assert (
            merged.operation_ttls == production_base.operation_ttls
        ), "Operation TTLs should be preserved"
        assert (
            merged.text_size_tiers == production_base.text_size_tiers
        ), "Text size tiers should be preserved"
        ...
        # Verify only the specified parameter changed
        assert (
            ttl_only_override.default_ttl == 9000
        ), "Only specified parameter should change"
        assert (
            ttl_only_override.redis_url == production_base.redis_url
        ), "Non-specified Redis URL should be preserved"
        assert (
            ttl_only_override.memory_cache_size == production_base.memory_cache_size
        ), "Non-specified memory cache should be preserved"
        assert (
            ttl_only_override.compression_threshold
            == production_base.compression_threshold
        ), "Non-specified compression should be preserved"
        assert (
            ttl_only_override.operation_ttls == production_base.operation_ttls
        ), "Non-specified operation TTLs should be preserved"
        ...
        assert (
            merged_with_defaults.default_ttl == 7200
        ), "Production TTL should be preserved when merging with defaults"
        assert (
            merged_with_defaults.memory_cache_size == production_base.memory_cache_size
        ), "Production cache size should be preserved"
        assert (
            merged_with_defaults.compression_threshold
            == production_base.compression_threshold
        ), "Production compression should be preserved"
        assert (
            merged_with_defaults.operation_ttls == production_base.operation_ttls
        ), "Production operation TTLs should be preserved"
        ...
        assert (
            merged_partial.operation_ttls == expected_partial_operations
        ...
        assert (
            merged_partial.redis_url == production_base.redis_url
        ), "Non-overridden parameters should be preserved"
        # With correct merge logic, only operation_ttls should be overridden, all other values preserved
        assert (
            merged_partial.default_ttl == 7200
        ), "Non-overridden default_ttl should be preserved from production base"
        assert (
            merged_partial.text_size_tiers == production_base.text_size_tiers
        ), "Non-overridden complex parameters should be preserved"

        # Verify merged configurations are still valid
        assert merged.validate().is_valid, "Merged configuration should remain valid"
        assert (
            ttl_only_override.validate().is_valid
        ), "Override configuration should be valid"
        assert (
            merged_partial.validate().is_valid
        ), "Partially merged configuration should be valid"
    ...
    def test_merge_handles_complex_parameter_inheritance(self):
        ...
        assert (
            merged.operation_ttls == expected_operation_ttls
        ...
        assert (
            merged.text_size_tiers == expected_text_size_tiers
        ...
        assert (
            merge_with_result.operation_ttls == expected_merge_with_operations
        ...
        assert (
            merge_with_result.text_size_tiers == expected_merge_with_tiers
        ...
        assert (
            valid_merged.operation_ttls == expected_operations
        ...
        assert (
            valid_merged.text_size_tiers == expected_tiers
    ...
    def test_merged_configuration_passes_validation(self):
        ...
        assert (
            base_validation.is_valid
        ), f"Base configuration should be valid. Errors: {base_validation.errors}"

        override_validation = valid_override.validate()
        assert (
            override_validation.is_valid
        ), f"Override configuration should be valid. Errors: {override_validation.errors}"
        ...
        assert (
            merged_config.redis_url == "redis://valid-override:6379/1"
        ), "Override should take precedence"
        assert merged_config.default_ttl == 7200, "Override TTL should take precedence"
        assert (
            merged_config.memory_cache_size == 100
        ), "Base memory cache should be preserved"
        assert (
            merged_config.compression_threshold == 512
        ), "Override compression should take precedence"
        ...
        assert (
            merge_with_validation.is_valid
        ...
        for operation, ttl in complex_merged.operation_ttls.items():
            assert (
                1 <= ttl <= 31536000
            ), f"Operation {operation} TTL {ttl} should be in valid range"
        ...
        assert (
            comprehensive_validation.is_valid
        ...
        assert (
            comprehensive_merged.compression_threshold == 800
        ), "Should have Redis-focused compression"
        assert (
            comprehensive_merged.text_hash_threshold == 1500
        ), "Should have AI-focused hash threshold"
        assert (
            len(comprehensive_merged.operation_ttls) == 3
        ), "Should have all AI operations"
        assert (
            len(comprehensive_merged.text_size_tiers) == 3
        ), "Should have all text tiers"

        # Verify validation results don't contain merge-introduced errors
        assert isinstance(
            comprehensive_validation.errors, list
        ), "Errors should be a list"
        assert isinstance(
            comprehensive_validation.warnings, list
        ), "Warnings should be a list"
        assert isinstance(
            comprehensive_validation.recommendations, list
        ), "Recommendations should be a list"
