...
class TestCacheStrategyEnumBehavior:
    ...
    def test_cache_strategy_defines_all_required_strategy_values(self):
        ...
        assert hasattr(
            CacheStrategy, "AI_OPTIMIZED"
        ), "AI_OPTIMIZED strategy should be defined"
        ...
        assert isinstance(
            fast_strategy, CacheStrategy
        ), "FAST should be CacheStrategy instance"
        assert isinstance(
            balanced_strategy, CacheStrategy
        ), "BALANCED should be CacheStrategy instance"
        assert isinstance(
            robust_strategy, CacheStrategy
        ), "ROBUST should be CacheStrategy instance"
        assert isinstance(
            ai_optimized_strategy, CacheStrategy
        ), "AI_OPTIMIZED should be CacheStrategy instance"
    ...
    def test_cache_strategy_values_have_correct_string_representations(self):
        ...
        assert (
            str(CacheStrategy.FAST) == "CacheStrategy.FAST"
        ), "str() should return enum name"
        assert (
            str(CacheStrategy.BALANCED) == "CacheStrategy.BALANCED"
        ), "str() should return enum name"
        assert (
            str(CacheStrategy.ROBUST) == "CacheStrategy.ROBUST"
        ), "str() should return enum name"
        assert (
            str(CacheStrategy.AI_OPTIMIZED) == "CacheStrategy.AI_OPTIMIZED"
        ), "str() should return enum name"
        ...
        ), "BALANCED should equal its string value"
        assert CacheStrategy.ROBUST == "robust", "ROBUST should equal its string value"
        assert (
            CacheStrategy.AI_OPTIMIZED == "ai_optimized"
        ), "AI_OPTIMIZED should equal its string value"

        # Test string representations are consistent
        assert CacheStrategy.FAST.value == "fast", "FAST value should be 'fast'"
        assert (
            CacheStrategy.BALANCED.value == "balanced"
        ), "BALANCED value should be 'balanced'"
        assert CacheStrategy.ROBUST.value == "robust", "ROBUST value should be 'robust'"
        assert (
            CacheStrategy.AI_OPTIMIZED.value == "ai_optimized"
        ), "AI_OPTIMIZED value should be 'ai_optimized'"
    ...
    def test_cache_strategy_supports_equality_comparisons(self):
        ...
        assert (
            CacheStrategy.FAST == CacheStrategy.FAST
        ), "Same strategy values should be equal"
        assert (
            CacheStrategy.BALANCED == CacheStrategy.BALANCED
        ), "Same strategy values should be equal"
        assert (
            CacheStrategy.ROBUST == CacheStrategy.ROBUST
        ), "Same strategy values should be equal"
        assert (
            CacheStrategy.AI_OPTIMIZED == CacheStrategy.AI_OPTIMIZED
        ), "Same strategy values should be equal"

        # Test inequality comparisons between different strategy values
        assert (
            CacheStrategy.FAST != CacheStrategy.BALANCED
        ), "Different strategy values should be unequal"
        assert (
            CacheStrategy.FAST != CacheStrategy.ROBUST
        ), "Different strategy values should be unequal"
        assert (
            CacheStrategy.FAST != CacheStrategy.AI_OPTIMIZED
        ), "Different strategy values should be unequal"
        assert (
            CacheStrategy.BALANCED != CacheStrategy.ROBUST
        ), "Different strategy values should be unequal"
        assert (
            CacheStrategy.BALANCED != CacheStrategy.AI_OPTIMIZED
        ), "Different strategy values should be unequal"
        assert (
            CacheStrategy.ROBUST != CacheStrategy.AI_OPTIMIZED
        ), "Different strategy values should be unequal"

        # Test identity comparisons (same enum instance)
        assert (
            CacheStrategy.FAST is CacheStrategy.FAST
        ), "Same enum values should be identical"
        assert (
            CacheStrategy.BALANCED is CacheStrategy.BALANCED
        ), "Same enum values should be identical"
        ...
        assert (
            result == "fast_selected"
        ), "Strategy should work correctly in conditional logic"
    ...
    def test_cache_strategy_supports_iteration_and_membership_testing(self):
        ...
        assert (
            strategy_values == expected_values
        ), "Iteration should include all expected strategy values"

        # Test that all defined strategies are in the iteration
        assert (
            CacheStrategy.FAST in all_strategies
        ), "FAST should be in iteration results"
        assert (
            CacheStrategy.BALANCED in all_strategies
        ), "BALANCED should be in iteration results"
        assert (
            CacheStrategy.ROBUST in all_strategies
        ), "ROBUST should be in iteration results"
        assert (
            CacheStrategy.AI_OPTIMIZED in all_strategies
        ), "AI_OPTIMIZED should be in iteration results"

        # Test membership testing with enum values
        assert (
            CacheStrategy.FAST in CacheStrategy
        ), "FAST should be member of CacheStrategy"
        assert (
            CacheStrategy.BALANCED in CacheStrategy
        ), "BALANCED should be member of CacheStrategy"
        assert (
            CacheStrategy.ROBUST in CacheStrategy
        ), "ROBUST should be member of CacheStrategy"
        assert (
            CacheStrategy.AI_OPTIMIZED in CacheStrategy
        ), "AI_OPTIMIZED should be member of CacheStrategy"
        ...
        assert (
            "balanced" in valid_strategy_names
        ), "balanced should be valid strategy name"
        assert (
            "invalid_strategy" not in valid_strategy_names
        ), "invalid_strategy should not be valid"
    ...
    def test_cache_strategy_string_enum_inheritance_works_correctly(self):
        ...
        assert isinstance(
            CacheStrategy.BALANCED, str
        ), "BALANCED should be instance of str"
        assert isinstance(CacheStrategy.ROBUST, str), "ROBUST should be instance of str"
        assert isinstance(
            CacheStrategy.AI_OPTIMIZED, str
        ), "AI_OPTIMIZED should be instance of str"
        ...
        assert isinstance(
            CacheStrategy.BALANCED, Enum
        ), "BALANCED should be instance of Enum"
        ...
        assert (
            fast_value_formatted == "Using fast strategy"
        ), "Using .value should give string value"

        # Test string methods work on strategy values
        assert (
            CacheStrategy.FAST.upper() == "FAST"
        ), "upper() should work on strategy values"
        assert (
            CacheStrategy.BALANCED.lower() == "balanced"
        ), "lower() should work on strategy values"
        assert (
            CacheStrategy.ROBUST.capitalize() == "Robust"
        ), "capitalize() should work on strategy values"

        # Test string concatenation
        concatenated = "cache_" + CacheStrategy.AI_OPTIMIZED
        assert (
            concatenated == "cache_ai_optimized"
        ), "String concatenation should work with strategies"

        concatenated_reverse = CacheStrategy.FAST + "_mode"
        assert (
            concatenated_reverse == "fast_mode"
        ), "Strategy concatenation with strings should work"
        ...
        assert (
            result == "ai-optimized"
        ), "Strategy should work in string-expecting functions"
...
class TestCacheStrategyConfigurationIntegration:
    ...
    def test_cache_strategy_integrates_with_default_presets_system(self):
        ...
        ), "DEFAULT_PRESETS should be a dictionary"

        # Verify all strategy values can be used as keys in DEFAULT_PRESETS
        assert (
            CacheStrategy.FAST in DEFAULT_PRESETS
        ), "FAST strategy should have default preset"
        assert (
            CacheStrategy.BALANCED in DEFAULT_PRESETS
        ), "BALANCED strategy should have default preset"
        assert (
            CacheStrategy.ROBUST in DEFAULT_PRESETS
        ), "ROBUST strategy should have default preset"
        assert (
            CacheStrategy.AI_OPTIMIZED in DEFAULT_PRESETS
        ), "AI_OPTIMIZED strategy should have default preset"
        ...
        assert (
            len(set(preset_ids)) == 4
        ), "All strategies should map to different preset instances"

        # Test that strategy-based access works both ways (enum and string)
        assert (
            DEFAULT_PRESETS[CacheStrategy.FAST] == DEFAULT_PRESETS["fast"]
        ), "Strategy and string access should be equivalent"
        assert (
            DEFAULT_PRESETS[CacheStrategy.BALANCED] == DEFAULT_PRESETS["balanced"]
        ), "Strategy and string access should be equivalent"
    ...
    def test_cache_strategy_enables_environment_based_configuration_selection(self):
        ...
        assert (
            dev_strategy == CacheStrategy.FAST
        ), "Development should use FAST strategy"
        ...
        assert (
            staging_strategy == CacheStrategy.BALANCED
        ), "Staging should use BALANCED strategy"

        # Test production environment strategy selection
        prod_strategy = select_strategy_for_environment("production")
        assert (
            prod_strategy == CacheStrategy.ROBUST
        ), "Production should use ROBUST strategy"

        # Test AI environment strategy selection
        ai_prod_strategy = select_strategy_for_environment("ai-production")
        assert (
            ai_prod_strategy == CacheStrategy.AI_OPTIMIZED
        ), "AI production should use AI_OPTIMIZED strategy"

        ai_dev_strategy = select_strategy_for_environment("ai-development")
        assert (
            ai_dev_strategy == CacheStrategy.AI_OPTIMIZED
        ), "AI development should use AI_OPTIMIZED strategy"

        # Test unknown environment defaults to safe choice
        unknown_strategy = select_strategy_for_environment("unknown")
        assert (
            unknown_strategy == CacheStrategy.BALANCED
        ), "Unknown environment should default to BALANCED strategy"
        ...
        assert (
            get_optimal_ttl_for_strategy(CacheStrategy.FAST) == 300
        ), "FAST strategy should have short TTL"
        assert (
            get_optimal_ttl_for_strategy(CacheStrategy.ROBUST) == 7200
        ), "ROBUST strategy should have long TTL"
        assert (
            get_optimal_ttl_for_strategy(CacheStrategy.AI_OPTIMIZED) == 1800
        ), "AI_OPTIMIZED strategy should have medium TTL"
    ...
    def test_cache_strategy_supports_configuration_validation(self):
        ...
        def validate_strategy_configuration(
            strategy_value: str, config: dict
        ) -> tuple[bool, list[str]]:
            ...
            if strategy_value not in valid_strategies:
                errors.append(
                    f"Invalid strategy '{strategy_value}'. Valid strategies: {sorted(valid_strategies)}"
                )
                return False, errors
            ...
            if strategy_value == "ai_optimized":
                ...
                if config.get("text_hash_threshold", 0) < 100:
                    errors.append(
                        "AI_OPTIMIZED strategy requires text_hash_threshold >= 100"
                    )

            if strategy_value == "robust":
                if config.get("default_ttl", 0) < 3600:
                    errors.append(
                        "ROBUST strategy should have default_ttl >= 3600 for reliability"
                    )
        ...
        is_valid, errors = validate_strategy_configuration(
            "ai_optimized", valid_ai_config
        )
        assert (
            is_valid == True
        ), "Valid AI_OPTIMIZED configuration should pass validation"
        assert len(errors) == 0, "Valid AI configuration should have no errors"

        valid_robust_config = {"default_ttl": 7200}
        is_valid, errors = validate_strategy_configuration(
            "robust", valid_robust_config
        )
        ...
        assert (
            "invalid_strategy" in errors[0]
        ), "Error message should mention invalid strategy"
        assert (
            "fast" in errors[0] and "balanced" in errors[0]
        ), "Error should list valid strategies"
        ...
        is_valid, errors = validate_strategy_configuration(
            "ai_optimized", invalid_ai_config
        )
        assert (
            is_valid == False
        ), "AI_OPTIMIZED without AI features should fail validation"
        assert any(
            "enable_ai_features=True" in error for error in errors
        ), "Should require AI features"

        invalid_robust_config = {"default_ttl": 60}  # Too short TTL for robust strategy
        is_valid, errors = validate_strategy_configuration(
            "robust", invalid_robust_config
        )
        assert is_valid == False, "ROBUST with short TTL should fail validation"
        assert any(
            "default_ttl >= 3600" in error for error in errors
        ), "Should require longer TTL"
    ...
    def test_cache_strategy_serialization_supports_configuration_persistence(self):
        ...
        assert (
            json_serialized is not None
        ), "Strategy configuration should serialize to JSON"
        assert (
            '"strategy": "balanced"' in json_serialized
        ), "Strategy should serialize as string value"
        assert (
            '"ttl": 3600' in json_serialized
        ), "Other values should serialize normally"

        # Deserialize from JSON
        deserialized_config = json.loads(json_serialized)
        assert (
            deserialized_config["strategy"] == "balanced"
        ), "Deserialized strategy should be string value"
        assert (
            deserialized_config["ttl"] == 3600
        ), "Other values should deserialize correctly"
        ...
        assert (
            reconstructed_strategy == original_strategy
        ), "Round-trip should preserve strategy identity"
        assert (
            reconstructed_strategy is original_strategy
        ), "Reconstructed strategy should be same enum instance"
        ...
        assert (
            deserialized_all["balanced"] == "balanced"
        ), "BALANCED should serialize to 'balanced'"
        assert (
            deserialized_all["robust"] == "robust"
        ), "ROBUST should serialize to 'robust'"
        assert (
            deserialized_all["ai_optimized"] == "ai_optimized"
        ), "AI_OPTIMIZED should serialize to 'ai_optimized'"
        ...
        assert (
            '"cache_strategy": "robust"' in human_readable_json
        ), "Serialized strategy should be human-readable"
        ...
        assert (
            deserialize_strategy("fast") == CacheStrategy.FAST
        ), "Should deserialize 'fast' to FAST"
        assert (
            deserialize_strategy("balanced") == CacheStrategy.BALANCED
        ), "Should deserialize 'balanced' to BALANCED"
        assert (
            deserialize_strategy("robust") == CacheStrategy.ROBUST
        ), "Should deserialize 'robust' to ROBUST"
        assert (
            deserialize_strategy("ai_optimized") == CacheStrategy.AI_OPTIMIZED
        ), "Should deserialize 'ai_optimized' to AI_OPTIMIZED"
    ...
    def test_cache_strategy_type_safety_in_configuration_systems(self):
        ...
        def configure_cache_with_strategy(
            strategy: CacheStrategy, ttl: int = 3600
        ) -> dict:
            """Example function using CacheStrategy type annotation."""
            return {"strategy": strategy, "ttl": ttl, "strategy_name": strategy.value}
        ...
        def get_strategy_characteristics(strategy: CacheStrategy) -> dict:
            ...
            characteristics = {
                ...
                "supports_production": strategy
                in [CacheStrategy.BALANCED, CacheStrategy.ROBUST],
            }
        ...
        assert (
            fast_config["strategy"] == CacheStrategy.FAST
        ), "FAST strategy should be configured correctly"
        assert (
            fast_config["strategy_name"] == "fast"
        ), "Strategy name should be extracted correctly"

        balanced_config = configure_cache_with_strategy(CacheStrategy.BALANCED)
        assert (
            balanced_config["strategy"] == CacheStrategy.BALANCED
        ), "BALANCED strategy should be configured correctly"

        # Test strategy characteristic analysis
        fast_characteristics = get_strategy_characteristics(CacheStrategy.FAST)
        assert (
            fast_characteristics["is_fast"] == True
        ), "FAST strategy should be identified as fast"
        assert (
            fast_characteristics["is_ai_optimized"] == False
        ), "FAST strategy should not be AI optimized"
        assert (
            fast_characteristics["supports_production"] == False
        ), "FAST strategy should not support production"

        ai_characteristics = get_strategy_characteristics(CacheStrategy.AI_OPTIMIZED)
        assert (
            ai_characteristics["is_ai_optimized"] == True
        ), "AI_OPTIMIZED should be identified as AI optimized"
        ...
        assert (
            robust_characteristics["supports_production"] == True
        ), "ROBUST should support production"
        ...
        assert (
            validate_strategy_parameter(CacheStrategy.FAST) == True
        ), "FAST should pass runtime validation"
        assert (
            validate_strategy_parameter(CacheStrategy.BALANCED) == True
        ), "BALANCED should pass runtime validation"
        ...
        strategy_list: list[CacheStrategy] = [
            CacheStrategy.FAST,
            CacheStrategy.BALANCED,
            CacheStrategy.ROBUST,
        ]
        assert len(strategy_list) == 3, "Strategy list should contain 3 strategies"
        assert all(
            isinstance(s, CacheStrategy) for s in strategy_list
        ), "All items should be CacheStrategy instances"
        ...
        assert (
            strategy_dict["dev"] == CacheStrategy.FAST
        ), "Dictionary should store strategies correctly"
        assert (
            strategy_dict["ai"] == CacheStrategy.AI_OPTIMIZED
        ), "Dictionary should retrieve strategies correctly"
