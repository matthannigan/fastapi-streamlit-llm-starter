...
class TestCachePresetManagerInitialization:
    ...
    def test_cache_preset_manager_initializes_with_default_presets(self):
        ...
        for preset_name in expected_presets:
            assert (
                preset_name in available_presets
            ), f"Expected preset '{preset_name}' not found in available presets"
        ...
        for preset_name in expected_presets:
            ...
            assert isinstance(
                preset, CachePreset
            ), f"Retrieved object for '{preset_name}' should be a CachePreset instance"
    ...
    def test_cache_preset_manager_get_preset_retrieves_valid_presets(self):
        ...
        dev_preset = manager.get_preset("development")
        assert isinstance(
            dev_preset, CachePreset
        ), "Development preset should be a CachePreset instance"
        assert (
            dev_preset.name == "Development"
        ), "Development preset should have correct name"
        assert (
            dev_preset.strategy == CacheStrategy.FAST
        ), "Development preset should use FAST strategy"
        assert (
            dev_preset.default_ttl == 600
        ), "Development preset should have 10-minute TTL for quick feedback"
        assert (
            dev_preset.max_connections == 3
        ), "Development preset should have minimal connections"
        assert (
            dev_preset.log_level == "DEBUG"
        ), "Development preset should use DEBUG logging"
        assert (
            "development" in dev_preset.environment_contexts
        ), "Development preset should support development context"

        # Test production preset retrieval
        prod_preset = manager.get_preset("production")
        assert isinstance(
            prod_preset, CachePreset
        ), "Production preset should be a CachePreset instance"
        assert (
            prod_preset.name == "Production"
        ), "Production preset should have correct name"
        assert (
            prod_preset.strategy == CacheStrategy.ROBUST
        ), "Production preset should use ROBUST strategy"
        assert (
            prod_preset.default_ttl == 7200
        ), "Production preset should have 2-hour TTL"
        assert (
            prod_preset.max_connections == 20
        ), "Production preset should have high connection pool"
        assert (
            prod_preset.compression_level == 9
        ), "Production preset should use maximum compression"
        assert (
            "production" in prod_preset.environment_contexts
        ), "Production preset should support production context"

        # Test AI development preset retrieval
        ai_dev_preset = manager.get_preset("ai-development")
        assert isinstance(
            ai_dev_preset, CachePreset
        ), "AI development preset should be a CachePreset instance"
        assert (
            ai_dev_preset.name == "AI Development"
        ), "AI development preset should have correct name"
        assert (
            ai_dev_preset.strategy == CacheStrategy.AI_OPTIMIZED
        ), "AI development preset should use AI_OPTIMIZED strategy"
        assert (
            ai_dev_preset.enable_ai_cache is True
        ), "AI development preset should enable AI cache features"
        assert (
            ai_dev_preset.ai_optimizations is not None
        ), "AI development preset should have AI optimizations"
        assert (
            "operation_ttls" in ai_dev_preset.ai_optimizations
        ), "AI preset should have operation-specific TTLs"

        # Test AI production preset retrieval
        ai_prod_preset = manager.get_preset("ai-production")
        assert isinstance(
            ai_prod_preset, CachePreset
        ), "AI production preset should be a CachePreset instance"
        assert (
            ai_prod_preset.enable_ai_cache is True
        ), "AI production preset should enable AI cache features"
        assert (
            ai_prod_preset.max_connections == 25
        ), "AI production preset should have high connection pool for AI workloads"
        assert (
            ai_prod_preset.memory_cache_size == 1000
        ), "AI production preset should have large memory cache"
        ...
        for preset_name in [
            ...
            preset = manager.get_preset(preset_name)
            assert (
                preset.default_ttl > 0
            ), f"{preset_name} preset should have positive TTL"
            assert (
                preset.max_connections > 0
            ), f"{preset_name} preset should have positive max connections"
            assert (
                preset.memory_cache_size > 0
            ), f"{preset_name} preset should have positive memory cache size"
            assert (
                preset.compression_level >= 1
            ), f"{preset_name} preset should have valid compression level"
    ...
    def test_cache_preset_manager_get_preset_raises_error_for_invalid_names(self):
        ...
        with pytest.raises(
            Exception
        ) as exc_info:  # Using generic Exception since implementation uses ConfigurationError
            manager.get_preset("non_existent_preset")

        error_message = str(exc_info.value)
        assert (
            "non_existent_preset" in error_message
        ), "Error message should mention the invalid preset name"
        assert (
            "Available presets:" in error_message
        ...
        for preset_name in expected_presets:
            assert (
                preset_name in error_message
            ), f"Error message should list '{preset_name}' as available preset"
        ...
        error_message = str(exc_info.value)
        assert (
            "Development" in error_message
        ), "Error message should mention the incorrectly cased preset name"
        ...
        assert (
            "Available presets:" in error_message
        ), "Error message should provide helpful guidance for empty string"
        ...
        for typo_name in typo_names:
            ...
            error_message = str(exc_info.value)
            assert (
                typo_name in error_message
            ), f"Error message should mention the typo '{typo_name}'"
            assert (
                "Available presets:" in error_message
            ), "Error message should list available presets for guidance"
    ...
    def test_cache_preset_manager_list_presets_returns_all_available_presets(self):
        ...
        assert len(available_presets) == len(
            expected_presets
        ), f"Should have {len(expected_presets)} presets, got {len(available_presets)}"

        for expected_preset in expected_presets:
            assert (
                expected_preset in available_presets
            ), f"Expected preset '{expected_preset}' should be in list"

        # Verify no duplicate names appear
        unique_presets = set(available_presets)
        assert len(unique_presets) == len(
            available_presets
        ), "Preset list should not contain duplicates"
        ...
        for category, category_presets in deployment_categories.items():
            for preset in category_presets:
                assert (
                    preset in available_presets
                ), f"Deployment category '{category}' should include preset '{preset}'"
        ...
        for preset_name in available_presets:
            ...
            assert (
                preset is not None
            ), f"Listed preset '{preset_name}' should be retrievable"
            assert isinstance(
                preset, CachePreset
            ), f"Listed preset '{preset_name}' should be a CachePreset instance"

        # Verify list order is consistent across multiple calls
        second_call = manager.list_presets()
        assert (
            available_presets == second_call
        ), "list_presets() should return consistent order across calls"
...
class TestCachePresetManagerRecommendation:
    ...
    def test_cache_preset_manager_recommend_preset_detects_development_environments(
        self, monkeypatch
    ):
        """
        ...
        for env_name in development_environments:
            ...
            ), f"Environment '{env_name}' should recommend development preset, got '{preset_name}'"
        ...
        assert isinstance(
            detailed_recommendation, EnvironmentRecommendation
        ), "Should return EnvironmentRecommendation instance"
        assert (
            detailed_recommendation.preset_name == "development"
        ), "Should recommend development preset"
        assert (
            detailed_recommendation.confidence >= 0.85
        ), "Should have high confidence for explicit development environment"
        assert (
            "development" in detailed_recommendation.reasoning.lower()
        ), "Reasoning should mention development"
        assert (
            detailed_recommendation.environment_detected == "development"
        ), "Should correctly identify environment"

        # Test 'dev' abbreviation
        dev_recommendation = manager.recommend_preset_with_details("dev")
        assert (
            dev_recommendation.preset_name == "development"
        ), "'dev' should map to development preset"
        assert (
            dev_recommendation.confidence >= 0.80
        ), "Should have high confidence for 'dev' abbreviation"
        ...
        assert (
            auto_recommendation.preset_name == "development"
        ), "Auto-detection should find development from NODE_ENV"
        assert (
            auto_recommendation.confidence >= 0.70
        ), "Auto-detection should have reasonable confidence"

        with patch.dict(os.environ, {"ENVIRONMENT": "dev"}, clear=False):
            auto_recommendation = manager.recommend_preset_with_details(None)
            assert (
                auto_recommendation.preset_name == "development"
            ), "Auto-detection should find development from ENVIRONMENT"

        with patch.dict(os.environ, {"FLASK_ENV": "development"}, clear=False):
            auto_recommendation = manager.recommend_preset_with_details(None)
            assert (
                auto_recommendation.preset_name == "development"
            ), "Auto-detection should find development from FLASK_ENV"

        # Verify development preset characteristics for optimization
        dev_preset = manager.get_preset("development")
        assert (
            dev_preset.strategy == CacheStrategy.FAST
        ), "Development preset should use FAST strategy for quick feedback"
        assert (
            dev_preset.default_ttl == 600
        ), "Development preset should have short TTL for fast iteration"
        assert (
            dev_preset.connection_timeout == 2
        ), "Development preset should have fast timeout"
        assert (
            dev_preset.log_level == "DEBUG"
        ), "Development preset should enable debug logging"

    @pytest.mark.xdist_group(name="environment_detection")
    def test_cache_preset_manager_recommend_preset_detects_production_environments(
        self, monkeypatch
    ):
        """
        ...
        for env_name in production_environments:
            ...
            ), f"Environment '{env_name}' should recommend production preset, got '{preset_name}'"
        ...
        assert (
            detailed_recommendation.preset_name == "production"
        ), "Should recommend production preset"
        assert (
            detailed_recommendation.confidence >= 0.85
        ), "Should have high confidence for explicit production environment"
        assert (
            "production" in detailed_recommendation.reasoning.lower()
        ), "Reasoning should mention production"
        assert (
            detailed_recommendation.environment_detected == "production"
        ), "Should correctly identify environment"

        # Test 'prod' abbreviation
        prod_recommendation = manager.recommend_preset_with_details("prod")
        assert (
            prod_recommendation.preset_name == "production"
        ), "'prod' should map to production preset"
        assert (
            prod_recommendation.confidence >= 0.80
        ), "Should have high confidence for 'prod' abbreviation"

        # Test 'staging' should map to production (common pattern)
        staging_recommendation = manager.recommend_preset_with_details("staging")
        assert (
            staging_recommendation.preset_name == "production"
        ), "Staging should use production-like settings"
        assert (
            staging_recommendation.confidence >= 0.80
        ), "Should have high confidence for staging environment"
        ...
        ), "Auto-detection should find production from NODE_ENV"
        assert (
            auto_recommendation.confidence >= 0.70
        ), "Auto-detection should have reasonable confidence"
        ...
        with patch.dict(os.environ, {"ENVIRONMENT": "prod"}, clear=False):
            ...
            assert (
                auto_recommendation.preset_name == "production"
            ), "Auto-detection should find production from ENVIRONMENT"
        ...
        assert (
            prod_preset.strategy == CacheStrategy.ROBUST
        ), "Production preset should use ROBUST strategy for reliability"
        assert (
            prod_preset.default_ttl == 7200
        ), "Production preset should have longer TTL for efficiency"
        assert (
            prod_preset.max_connections == 20
        ), "Production preset should have high connection pool"
        assert (
        ...
        assert (
            prod_preset.log_level == "INFO"
        ), "Production preset should use INFO logging level"
    ...
    def test_cache_preset_manager_recommend_preset_detects_ai_environments(self):
        ...
        for env_name in ai_development_environments:
            ...
            ), f"Environment '{env_name}' should recommend ai-development preset, got '{preset_name}'"

        for env_name in ai_production_environments:
            preset_name = manager.recommend_preset(env_name)
            assert (
                preset_name == "ai-production"
            ), f"Environment '{env_name}' should recommend ai-production preset, got '{preset_name}'"

        # Test AI development environment with details
        ai_dev_recommendation = manager.recommend_preset_with_details("ai-development")
        assert isinstance(
            ai_dev_recommendation, EnvironmentRecommendation
        ), "Should return EnvironmentRecommendation instance"
        assert (
            ai_dev_recommendation.preset_name == "ai-development"
        ), "Should recommend ai-development preset"
        assert (
            ai_dev_recommendation.confidence >= 0.90
        ), "Should have high confidence for explicit AI development environment"
        assert (
            "ai" in ai_dev_recommendation.reasoning.lower()
        ), "Reasoning should mention AI"

        # Test AI production environment with details
        ai_prod_recommendation = manager.recommend_preset_with_details("ai-production")
        assert (
            ai_prod_recommendation.preset_name == "ai-production"
        ), "Should recommend ai-production preset"
        assert (
            ai_prod_recommendation.confidence >= 0.90
        ), "Should have high confidence for explicit AI production environment"

        # Test environment variable mocking for AI detection
        with patch.dict(os.environ, {"ENVIRONMENT": "ai-development"}, clear=False):
            auto_recommendation = manager.recommend_preset_with_details(
                None
            )  # Auto-detect
            assert (
                auto_recommendation.preset_name == "ai-development"
            ), "Auto-detection should find AI development from ENVIRONMENT"
            assert (
                auto_recommendation.confidence >= 0.70
            ), "Auto-detection should have reasonable confidence"
        ...
        with patch.dict(os.environ, {"ENVIRONMENT": "ai-prod"}, clear=False):
            auto_recommendation = manager.recommend_preset_with_details(None)
            assert (
                auto_recommendation.preset_name == "ai-production"
            ), "Auto-detection should find AI production from ENVIRONMENT"

        with patch.dict(
            os.environ,
            {"ENABLE_AI_CACHE": "true", "NODE_ENV": "development"},
            clear=True,
        ):
            auto_recommendation = manager.recommend_preset_with_details(None)
            assert (
                auto_recommendation.preset_name == "ai-development"
            ), "Should detect AI development when AI cache enabled with dev environment"

        with patch.dict(
            os.environ,
            {"ENABLE_AI_CACHE": "true", "NODE_ENV": "production"},
            clear=True,
        ):
            auto_recommendation = manager.recommend_preset_with_details(None)
            assert (
                auto_recommendation.preset_name == "ai-production"
            ), "Should detect AI production when AI cache enabled with prod environment"
        ...
        assert (
            "text_hash_threshold" in ai_dev_preset.ai_optimizations
        ), "AI preset should have text hashing configuration"

        ai_prod_preset = manager.get_preset("ai-production")
        assert (
            ai_prod_preset.strategy == CacheStrategy.AI_OPTIMIZED
        ), "AI production preset should use AI_OPTIMIZED strategy"
        assert (
            ai_prod_preset.enable_ai_cache is True
        ), "AI production preset should enable AI cache features"
        assert (
        ...
        assert (
            ai_prod_preset.memory_cache_size == 1000
        ), "AI production preset should have large memory cache for AI data"

    @pytest.mark.xdist_group(name="environment_detection")
    def test_cache_preset_manager_recommend_preset_with_details_provides_comprehensive_reasoning(
        self,
    ):
        """
        ...
        ), "Should have preset_name attribute"
        ...
        assert hasattr(
            recommendation, "environment_detected"
        ), "Should have environment_detected attribute"

        # Verify preset name is valid
        assert (
            recommendation.preset_name in manager.list_presets()
        ), "Recommended preset should be available"

        # Verify confidence score is within valid range
        assert (
            0.0 <= recommendation.confidence <= 1.0
        ), f"Confidence should be between 0.0 and 1.0, got {recommendation.confidence}"
        ...
        assert (
            "development" in recommendation.reasoning.lower()
        ), "Reasoning should explain development context"

        # Verify environment detected
        assert (
            recommendation.environment_detected == "development"
        ), "Should correctly identify detected environment"
        ...
        for env in high_confidence_envs:
            ...
            assert (
                rec.confidence >= 0.90
            ), f"Exact match for '{env}' should have high confidence, got {rec.confidence}"
            ...
            if env == "ai-production":
                ...
                assert (
                    "ai" in reasoning_lower and "production" in reasoning_lower
                ), f"Reasoning should mention AI and production for '{env}', got: {rec.reasoning}"
            else:
                assert (
                    env in rec.reasoning.lower()
                ), f"Reasoning should mention '{env}' environment"
        ...
        for env in medium_confidence_envs:
            rec = manager.recommend_preset_with_details(env)
            assert (
                0.70 <= rec.confidence < 0.95
            ), f"Abbreviation '{env}' should have medium confidence, got {rec.confidence}"
            assert (
                len(rec.reasoning) > 20
            ), "Reasoning should be descriptive for abbreviations"

        # Test auto-detection with comprehensive details
        with patch.dict(
            os.environ, {"NODE_ENV": "production", "DEBUG": "false"}, clear=True
        ):
            auto_rec = manager.recommend_preset_with_details(None)
            assert (
                auto_rec.preset_name == "production"
            ), "Auto-detection should find production"
            assert (
                auto_rec.confidence >= 0.70
            ), "Auto-detection should have reasonable confidence"
            assert (
                "auto-detected" in auto_rec.environment_detected.lower()
            ), "Should indicate auto-detection"
            assert (
                len(auto_rec.reasoning) > 30
            ), "Auto-detection reasoning should be comprehensive"
        ...
        for env in pattern_envs:
            ...
            assert isinstance(
                rec.reasoning, str
            ), f"Pattern matching for '{env}' should provide reasoning"
            assert (
                "pattern" in rec.reasoning.lower() or "match" in rec.reasoning.lower()
            ), "Should explain pattern matching"
        ...
        for env in unknown_envs:
            ...
            assert (
                rec.confidence < 0.60
            ), f"Unknown environment '{env}' should have lower confidence"
            assert (
                "default" in rec.reasoning.lower() or "unknown" in rec.reasoning.lower()
    ...
    def test_cache_preset_manager_handles_ambiguous_environment_scenarios(self):
        ...
        truly_ambiguous_environments = [
            "custom-deployment",
            "unknown-env",
            "mystery-env",
        ]
        pattern_matching_environments = [
            "testing-prod",
            "dev-like-prod",
        ]  # These may match patterns
        ...
        for env_name in truly_ambiguous_environments:
            ...
            assert recommendation.preset_name in [
                "simple",
                "development",
            ], f"Ambiguous environment '{env_name}' should get safe default preset"

            # Should have lower confidence to reflect uncertainty
            assert (
                recommendation.confidence <= 0.50
            ), f"Truly ambiguous environment '{env_name}' should have low confidence, got {recommendation.confidence}"

            # Reasoning should explain the ambiguity
            reasoning_lower = recommendation.reasoning.lower()
            assert any(
                word in reasoning_lower
                for word in ["unknown", "default", "pattern", "simple"]
            ), f"Reasoning should explain ambiguity for '{env_name}'"
        ...
        for env_name in pattern_matching_environments:
            ...
            assert (
                recommendation.preset_name in manager.list_presets()
            ), f"Pattern-matched environment '{env_name}' should get valid preset"

            # Pattern matching gives moderate confidence (not as low as truly unknown)
            assert (
                0.40 <= recommendation.confidence <= 0.80
            ), f"Pattern-matched environment '{env_name}' should have moderate confidence, got {recommendation.confidence}"
        ...
        with patch.dict(
            os.environ, {"NODE_ENV": "development", "PROD": "true"}, clear=True
        ):
            ...
            ), "Primary environment signal should still have reasonable confidence"
        ...
        for env_name in complex_environments:
            ...
            assert (
                recommendation.preset_name in manager.list_presets()
            ), "Should recommend a valid preset"

            # But with appropriate confidence reflecting the complexity (allowing for pattern matching)
            assert (
                0.30 <= recommendation.confidence <= 0.80
            ), f"Complex environment '{env_name}' should have moderate confidence"

        # Verify conservative preset characteristics for ambiguous cases
        simple_preset = manager.get_preset("simple")
        assert (
            simple_preset.strategy == CacheStrategy.BALANCED
        ), "Simple preset should be balanced/conservative"
        assert (
            simple_preset.enable_monitoring is True
        ), "Conservative preset should enable monitoring for visibility"

        # Test that reasoning includes helpful guidance for manual review
        unclear_recommendation = manager.recommend_preset_with_details(
            "unclear-environment"
        )
        reasoning_words = unclear_recommendation.reasoning.lower().split()
        # Should provide some guidance about the selection
        assert (
            len(reasoning_words) >= 5
        ), "Reasoning should be descriptive for unclear environments"
...
class TestCachePresetManagerValidation:
    ...
    def test_cache_preset_manager_validate_preset_confirms_preset_quality(self):
        ...
        assert (
            is_boundary_valid is True
        ), "Preset with minimum valid values should pass validation"
    ...
    def test_cache_preset_manager_validates_all_predefined_presets(self):
        ...
        for preset_name in preset_names:
            ...
            assert (
                is_valid is True
            ), f"Predefined preset '{preset_name}' should pass validation"
        ...
        for expected_preset in expected_presets:
            assert (
                expected_preset in validation_results
            ), f"Expected preset '{expected_preset}' should be validated"
            assert (
                validation_results[expected_preset] is True
            ), f"Expected preset '{expected_preset}' should be valid"
        ...
        for preset_name in basic_presets:
            preset = manager.get_preset(preset_name)
            assert (
                preset.enable_ai_cache is False
            ), f"Basic preset '{preset_name}' should not have AI cache enabled"
            assert (
                manager.validate_preset(preset) is True
            ), f"Basic preset '{preset_name}' should be valid"
        ...
        for preset_name in development_presets:
            ...
            ), f"Development preset '{preset_name}' should use DEBUG logging"
            assert (
                preset.default_ttl <= 1800
            ), f"Development preset '{preset_name}' should have short TTL for quick feedback"
            assert (
                manager.validate_preset(preset) is True
            ), f"Development preset '{preset_name}' should be valid"

        production_presets = ["production", "ai-production"]
        for preset_name in production_presets:
            preset = manager.get_preset(preset_name)
            assert (
                preset.max_connections >= 20
            ), f"Production preset '{preset_name}' should have high connection pool"
            assert (
                preset.compression_level >= 6
            ), f"Production preset '{preset_name}' should use good compression"
            assert (
                manager.validate_preset(preset) is True
            ), f"Production preset '{preset_name}' should be valid"
        ...
        for preset_name in ai_presets:
            ...
            ), f"AI preset '{preset_name}' should enable AI cache"
            assert (
                preset.strategy == CacheStrategy.AI_OPTIMIZED
            ), f"AI preset '{preset_name}' should use AI_OPTIMIZED strategy"
            assert (
                len(preset.ai_optimizations) > 0
            ), f"AI preset '{preset_name}' should have AI optimizations"
            assert (
                manager.validate_preset(preset) is True
            ), f"AI preset '{preset_name}' should be valid"
        ...
        assert (
            validation_rate == 1.0
        ), f"All predefined presets should be valid, got {validation_rate:.2%} success rate"

    def test_cache_preset_manager_get_all_presets_summary_provides_comprehensive_overview(
        self,
    ):
        """
        ...
        assert isinstance(
            summary, dict
        ), "get_all_presets_summary() should return a dictionary"

        # Verify all expected presets are included in summary
        expected_presets = manager.list_presets()
        assert len(summary) == len(
            expected_presets
        ), "Summary should include all available presets"

        for preset_name in expected_presets:
            assert (
                preset_name in summary
            ), f"Summary should include '{preset_name}' preset"

        # Verify each preset summary has required information
        for preset_name, preset_info in summary.items():
            assert isinstance(
                preset_info, dict
            ), f"Preset '{preset_name}' summary should be a dictionary"
            ...
            for field in required_fields:
                assert (
                    field in preset_info
                ), f"Preset '{preset_name}' summary should include '{field}' field"
            ...
            for field in config_fields:
                assert (
                    field in config
                ), f"Preset '{preset_name}' configuration should include '{field}'"
        ...
        ), "Summary should include balanced strategy presets"
        ...
        for ai_preset_name in ai_presets:
            ...
            assert (
                "ai_optimizations" in ai_preset_info
            ), f"AI preset '{ai_preset_name}' summary should include AI optimizations"
            assert (
                ai_preset_info["ai_optimizations"] is not None
            ), f"AI preset '{ai_preset_name}' should have non-null AI optimizations"
        ...
        assert (
            len(context_coverage) >= 3
        ), f"Summary should cover major deployment contexts, got {context_coverage}"

        # Verify performance characteristics are distinguishable
        ttl_values = [
            preset_info["configuration"]["default_ttl"]
            for preset_info in summary.values()
        ]
        connection_values = [
            preset_info["configuration"]["max_connections"]
            for preset_info in summary.values()
        ]

        # Should have variety in performance characteristics
        assert (
            len(set(ttl_values)) >= 4
        ), "Summary should show variety in TTL configurations for comparison"
        assert (
            len(set(connection_values)) >= 4
        ), "Summary should show variety in connection pool sizes for comparison"
        ...
        for preset_name, preset_info in summary.items():
            ...
            assert isinstance(
                description, str
            ), f"Preset '{preset_name}' description should be a string"
            assert (
                len(description) > 20
            ), f"Preset '{preset_name}' description should be descriptive for UI display"

    def test_cache_preset_manager_get_preset_details_provides_specific_preset_information(
        self,
    ):
        """
        ...
        assert isinstance(
            dev_details, dict
        ), "get_preset_details() should return a dictionary"
        ...
        for field in required_fields:
            assert (
                field in dev_details
            ), f"Development preset details should include '{field}' field"
        ...
        ), "Development preset should have 10-minute TTL"
        assert (
        ...
        ), "Development preset should list development context"
        ...
        assert (
            ai_config["enable_ai_cache"] is True
        ), "AI production preset should enable AI cache"
        assert (
            ai_config["strategy"] == "ai_optimized"
        ), "AI production preset should use ai_optimized strategy"
        ...
        assert (
            ai_opts is not None
        ), "AI production preset should have AI optimizations details"

        # Test details for simple preset
        simple_details = manager.get_preset_details("simple")
        simple_config = simple_details["configuration"]
        assert (
            simple_config["strategy"] == "balanced"
        ), "Simple preset should use balanced strategy"
        assert (
            simple_config["enable_ai_cache"] is False
        ), "Simple preset should not enable AI cache"

        # Verify multiple environment contexts for flexible presets
        simple_contexts = simple_details["environment_contexts"]
        assert (
            len(simple_contexts) >= 3
        ), "Simple preset should support multiple environment contexts"
        ...
        for preset_name in manager.list_presets():
            ...
            assert isinstance(
                details, dict
            ), f"Preset '{preset_name}' should provide valid details dictionary"
            assert (
                "name" in details
            ), f"Preset '{preset_name}' details should include name"
            assert (
                "description" in details
            ), f"Preset '{preset_name}' details should include description"
            assert (
                len(details["description"]) > 10
            ), f"Preset '{preset_name}' should have descriptive description"
