...
class TestCachePerformanceMonitorInitialization:
    ...
    def test_monitor_validates_configuration_parameters(self):
        ...
        monitor_inverted = CachePerformanceMonitor(
            ...
            memory_critical_threshold_bytes=50 * 1024 * 1024   # 50MB (less than warning)
        )
        ...
        def record_metrics(thread_id: int) -> Dict[str, int]:
            ...
            for i in range(operations_per_thread):
                ...
                monitor.record_memory_usage(
                    ...
                    redis_stats={"memory_used_bytes": 1000000 + i * 1000, "keys": 100 + i}
                )
    ...
    def test_monitor_provides_consistent_behavior(self):
        ...
        for iteration in range(3):  # Run 3 times
            ...
            expected_avg_key_gen = sum(s["key_gen_duration"] for s in test_scenarios) / len(test_scenarios)
            assert abs(stats["key_generation"]["avg_duration"] - expected_avg_key_gen) < 0.001
            
            expected_avg_cache_op = sum(s["cache_duration"] for s in test_scenarios) / len(test_scenarios)
            assert abs(stats["cache_operations"]["avg_duration"] - expected_avg_cache_op) < 0.001
            
            expected_avg_compression = sum(s["compression_time"] for s in test_scenarios) / len(test_scenarios)
            assert abs(stats["compression"]["avg_compression_time"] - expected_avg_compression) < 0.001
        ...
        assert stats1["key_generation"]["avg_duration"] == stats2["key_generation"]["avg_duration"]
        assert stats1["cache_operations"]["avg_duration"] == stats2["cache_operations"]["avg_duration"]
...
class TestMetricRecording:
    ...
    def test_record_key_generation_time_captures_accurate_metrics(self):
        ...
        for i, (metric, expected) in enumerate(zip(monitor.key_generation_times, test_cases)):
    ...
    def test_record_key_generation_time_handles_edge_cases(self):
        ...
        for metric, case in zip(monitor.key_generation_times, edge_cases):
            assert metric.duration == case["duration"]  # Preserve exact values, even if unusual
    ...
    def test_record_key_generation_time_maintains_performance(self):
        ...
        assert recording_duration < max_acceptable_duration, \
        ...
        assert cleanup_duration < max_cleanup_duration, \
    ...
    def test_record_cache_operation_time_captures_accurate_metrics(self):
        ...
        expected_avg = sum(op["duration"] for op in test_operations) / len(test_operations)
        assert abs(stats["cache_operations"]["avg_duration"] - expected_avg) < 0.001
        
        # Verify hit rate calculation
        expected_hit_rate = (expected_hits / len(get_operations)) * 100 if get_operations else 0
        if get_operations:
    ...
    def test_record_cache_operation_time_tracks_hit_miss_ratios(self):
        ...
        monitor.reset_stats()
        monitor.record_cache_operation_time("set", 0.010, False, 1000)  # Set operations don't count as misses
        monitor.record_cache_operation_time("delete", 0.005, True, 0)   # Delete operations don't count as hits
    ...
    def test_record_cache_operation_time_handles_various_operations(self):
        ...
        operation_types = [
            # Standard operations
            {"type": f"{test_prefix}_get_hit", "duration": 0.003, "hit": True, "text_len": 1000, "count": 5},
            {"type": f"{test_prefix}_get_miss", "duration": 0.015, "hit": False, "text_len": 1200, "count": 2},
            {"type": f"{test_prefix}_set", "duration": 0.008, "hit": False, "text_len": 1500, "count": 3},
            {"type": f"{test_prefix}_delete", "duration": 0.005, "hit": True, "text_len": 0, "count": 2},
            
            # Extended operations
            {"type": f"{test_prefix}_exists", "duration": 0.002, "hit": True, "text_len": 0, "count": 4},
            {"type": f"{test_prefix}_expire", "duration": 0.004, "hit": True, "text_len": 0, "count": 1},
            {"type": f"{test_prefix}_ttl", "duration": 0.001, "hit": True, "text_len": 0, "count": 2},
            
            # Custom operation types
            {"type": f"{test_prefix}_bulk_get", "duration": 0.025, "hit": True, "text_len": 5000, "count": 2},
            {"type": f"{test_prefix}_pattern_delete", "duration": 0.020, "hit": True, "text_len": 0, "count": 1},
            {"type": f"{test_prefix}_health_check", "duration": 0.001, "hit": True, "text_len": 0, "count": 3},
        ]
        ...
        fast_operations = [f"{test_prefix}_exists", f"{test_prefix}_ttl", f"{test_prefix}_health_check"]
        ...
        assert "custom_analytics_query" in updated_stats["cache_operations"]["by_operation_type"]
        custom_stats = updated_stats["cache_operations"]["by_operation_type"]["custom_analytics_query"]
    ...
    def test_record_compression_ratio_captures_efficiency_metrics(self):
        ...
        expected_ratios = [case["compressed_size"] / case["original_size"] for case in compression_test_cases]
        expected_avg_ratio = sum(expected_ratios) / len(expected_ratios)
        assert abs(stats["compression"]["avg_compression_ratio"] - expected_avg_ratio) < 0.001
        ...
        assert abs(stats["compression"]["avg_compression_time"] - expected_avg_time) < 0.001
        
        # Verify bytes processed and savings calculations
        expected_total_original = sum(case["original_size"] for case in compression_test_cases)
        expected_total_compressed = sum(case["compressed_size"] for case in compression_test_cases)
        expected_total_saved = expected_total_original - expected_total_compressed
        expected_savings_percent = (expected_total_saved / expected_total_original) * 100
        ...
        assert abs(stats["compression"]["overall_savings_percent"] - expected_savings_percent) < 0.1
    ...
    def test_record_compression_ratio_tracks_timing_accurately(self):
        ...
        assert abs(stats["compression"]["avg_compression_time"] - expected_avg_time) < 0.001
        assert abs(stats["compression"]["max_compression_time"] - expected_max_time) < 0.001
        ...
        fast_operations = [m for m in monitor.compression_ratios if m.compression_time < 0.010]
        slow_operations = [m for m in monitor.compression_ratios if m.compression_time > 0.050]
        ...
        assert high_freq_duration < 0.1, f"High frequency recording too slow: {high_freq_duration:.3f}s"
        ...
        for i, metric in enumerate(high_freq_metrics):
            ...
            assert abs(metric.compression_time - expected_time) < 0.000001  # Very high precision
    ...
    def test_record_memory_usage_captures_comprehensive_data(self):
        ...
        memory_test_cases = [
            ...
            },
            {
                ...
                "additional_data": {
                    "threshold_check": "warning_approach"
                },
        ...
        for metric, expected in zip(monitor.memory_usage_measurements, memory_test_cases):
            ...
            if metric.memory_cache_entry_count > 0:
                expected_avg = metric.memory_cache_size_bytes / metric.memory_cache_entry_count
                assert abs(metric.avg_entry_size_bytes - expected_avg) < 1.0
            
            # Verify cache utilization percentage calculation
            expected_utilization = (metric.total_cache_size_bytes / monitor.memory_warning_threshold_bytes) * 100
            assert abs(metric.cache_utilization_percent - expected_utilization) < 0.1
            
            # Verify threshold warnings
            expected_warning = metric.total_cache_size_bytes >= monitor.memory_warning_threshold_bytes
        ...
        assert abs(stats["current"]["total_cache_size_mb"] - (latest_metric.total_cache_size_bytes / 1024 / 1024)) < 0.1
        assert stats["current"]["cache_entry_count"] == latest_metric.cache_entry_count
        assert stats["current"]["warning_threshold_reached"] == latest_metric.warning_threshold_reached
        
        # Verify threshold information
        assert stats["thresholds"]["warning_threshold_mb"] == monitor.memory_warning_threshold_bytes / 1024 / 1024
        assert stats["thresholds"]["critical_threshold_mb"] == monitor.memory_critical_threshold_bytes / 1024 / 1024
    ...
    def test_record_memory_usage_calculates_sizes_accurately(self):
        ...
        size_test_cases = [
            {
                ...
                "expected_size_range": (0, 300),  # Should be small but not necessarily 0
            ...
            },
            {
                ...
                "expected_size_range": (1000, 2000),  # Should include the 1KB string plus overhead
        ...
        for case in size_test_cases:
            ...
            assert min_size <= metric.memory_cache_size_bytes <= adjusted_max_size, \
        ...
        assert nested_metric.memory_cache_size_bytes > 500  # At least the size of the string data
        ...
        assert 0.5 <= size_ratio <= 2.0, f"Size calculation seems inaccurate: {simple_metric.memory_cache_size_bytes} vs {manual_size}"
        
        # Test performance with large cache
        large_cache = {f"key_{i}": f"value_{i}" * 10 for i in range(1000)}  # 1000 entries
        ...
        assert large_metric.memory_cache_size_bytes > 10000  # Should be substantial size
    ...
    def test_record_memory_usage_handles_missing_redis_stats(self):
        ...
        metric_none = monitor.record_memory_usage(
            ...
            redis_stats=None  # Explicitly None
        )
        ...
        expected_avg = metric_none.memory_cache_size_bytes / metric_none.memory_cache_entry_count
        ...
        metric_empty = monitor.record_memory_usage(
            memory_cache=test_memory_cache,
            redis_stats={}  # Empty dict
        )
        
        assert metric_empty is not None
        # Should behave the same as None case
        assert metric_empty.total_cache_size_bytes == metric_empty.memory_cache_size_bytes
        ...
        metric_incomplete = monitor.record_memory_usage(
            memory_cache=test_memory_cache,
            redis_stats=incomplete_redis_stats
        )
        
        assert metric_incomplete is not None
        # Should fall back to memory cache only since required keys are missing
        assert metric_incomplete.total_cache_size_bytes == metric_incomplete.memory_cache_size_bytes
        assert metric_incomplete.cache_entry_count == metric_incomplete.memory_cache_entry_count
        ...
        metric_partial = monitor.record_memory_usage(
            memory_cache=test_memory_cache,
            redis_stats=partial_redis_stats
        )
        ...
        assert metric_partial.cache_entry_count == metric_partial.memory_cache_entry_count
        ...
        try:
            metric_invalid = monitor.record_memory_usage(
                memory_cache=test_memory_cache,
                redis_stats=invalid_redis_stats
            )
            # If it succeeds, should fall back to memory cache only
            assert metric_invalid.total_cache_size_bytes == metric_invalid.memory_cache_size_bytes
        ...
        metric_all_empty = monitor.record_memory_usage(
            memory_cache=empty_cache,
            redis_stats=None
        )
        
        assert metric_all_empty is not None
        assert metric_all_empty.memory_cache_entry_count == 0
        assert metric_all_empty.memory_cache_size_bytes >= 0  # Might be small but not necessarily 0
        assert metric_all_empty.total_cache_size_bytes == metric_all_empty.memory_cache_size_bytes
        ...
        assert 7 <= len(monitor.memory_usage_measurements) <= 8  # Most test cases recorded
    ...
    def test_record_invalidation_event_captures_event_data(self):
        ...
        for event, expected in zip(monitor.invalidation_events, invalidation_test_cases):
        ...
        expected_total_keys = sum(case["keys_invalidated"] for case in invalidation_test_cases)
        ...
        expected_avg_keys_per_invalidation = expected_total_keys / expected_total_invalidations
    ...
    def test_record_invalidation_event_categorizes_types_correctly(self):
        ...
        type_categorization_cases = [
            ...
            {"type": "manual", "pattern": "user_*", "keys": 10, "duration": 0.010, "context": "admin_action", "count": 3},
            {"type": "manual", "pattern": "session_*", "keys": 5, "duration": 0.008, "context": "user_logout", "count": 2},
            
            # Automatic invalidations
            {"type": "automatic", "pattern": "cache_*", "keys": 50, "duration": 0.025, "context": "data_change", "count": 4},
            {"type": "automatic", "pattern": "summary_*", "keys": 20, "duration": 0.015, "context": "dependency_update", "count": 2},
            
            # TTL expirations
            {"type": "ttl_expired", "pattern": "temp_*", "keys": 100, "duration": 0.030, "context": "scheduled_cleanup", "count": 5},
            {"type": "ttl_expired", "pattern": "short_term_*", "keys": 75, "duration": 0.020, "context": "hourly_cleanup", "count": 3},
            
            # Custom invalidation types
            {"type": "emergency", "pattern": "*", "keys": 1000, "duration": 0.200, "context": "system_recovery", "count": 1},
            {"type": "maintenance", "pattern": "old_*", "keys": 200, "duration": 0.080, "context": "scheduled_maintenance", "count": 2},
            {"type": "batch_cleanup", "pattern": "expired_*", "keys": 300, "duration": 0.120, "context": "daily_batch", "count": 1},
        ]
        ...
        for case in type_categorization_cases:
            expected_type_counts[case["type"]] = expected_type_counts.get(case["type"], 0) + case["count"]
        ...
        manual_events = [e for e in monitor.invalidation_events if e.invalidation_type == "manual"]
        automatic_events = [e for e in monitor.invalidation_events if e.invalidation_type == "automatic"]
        ttl_events = [e for e in monitor.invalidation_events if e.invalidation_type == "ttl_expired"]
        custom_events = [e for e in monitor.invalidation_events if e.invalidation_type not in ["manual", "automatic", "ttl_expired"]]
        
        # Verify counts match expectations
        manual_expected = sum(case["count"] for case in type_categorization_cases if case["type"] == "manual")
        automatic_expected = sum(case["count"] for case in type_categorization_cases if case["type"] == "automatic")
        ttl_expected = sum(case["count"] for case in type_categorization_cases if case["type"] == "ttl_expired")
        
        assert len(manual_events) == manual_expected
        assert len(automatic_events) == automatic_expected
        assert len(ttl_events) == ttl_expected
        assert len(custom_events) == total_events - manual_expected - automatic_expected - ttl_expected
        
        # Verify type-specific characteristics
        # Manual events should have admin or user contexts
        for event in manual_events:
            assert "admin" in event.operation_context or "user" in event.operation_context or "logout" in event.operation_context
        
        # Automatic events should have system contexts
        for event in automatic_events:
            assert "data" in event.operation_context or "dependency" in event.operation_context or "change" in event.operation_context
        
        # TTL events should have cleanup contexts
        for event in ttl_events:
            assert "cleanup" in event.operation_context or "scheduled" in event.operation_context
    ...
    def test_record_invalidation_event_tracks_efficiency_metrics(self):
        ...
        efficiency_test_cases = [
            ...
            {
                "scenarios": [
                    {"pattern": "batch_*", "keys": 500, "duration": 0.100, "type": "batch_cleanup", "context": "daily_cleanup"},
                    {"pattern": "user_session_*", "keys": 200, "duration": 0.050, "type": "automatic", "context": "session_timeout"},
                    {"pattern": "cache_group_*", "keys": 300, "duration": 0.075, "type": "manual", "context": "group_invalidation"},
                ],
            ...
            {
                "scenarios": [
                    {"pattern": "specific_key_1", "keys": 1, "duration": 0.005, "type": "manual", "context": "single_key_clear"},
                    {"pattern": "specific_key_2", "keys": 1, "duration": 0.004, "type": "manual", "context": "single_key_clear"},
                    {"pattern": "rare_pattern_*", "keys": 2, "duration": 0.008, "type": "automatic", "context": "rare_event"},
                ],
            ...
            {
                "scenarios": [
                    {"pattern": "nonexistent_*", "keys": 0, "duration": 0.002, "type": "ttl_expired", "context": "cleanup_check"},
                    {"pattern": "empty_pattern_*", "keys": 0, "duration": 0.001, "type": "automatic", "context": "preventive_check"},
                    {"pattern": "test_*", "keys": 0, "duration": 0.003, "type": "manual", "context": "test_cleanup"},
                ],
            ...
            {
                "scenarios": [
                    {"pattern": "mixed_a_*", "keys": 50, "duration": 0.020, "type": "automatic", "context": "moderate_cleanup"},
                    {"pattern": "mixed_b_*", "keys": 10, "duration": 0.008, "type": "manual", "context": "small_cleanup"},
                    {"pattern": "mixed_c_*", "keys": 100, "duration": 0.040, "type": "ttl_expired", "context": "scheduled_cleanup"},
                ],
        ...
        expected_avg_keys_per_invalidation = total_keys / total_invalidations if total_invalidations > 0 else 0
        ...
        high_efficiency_events = [e for e in monitor.invalidation_events if e.additional_data.get("efficiency_category") == "high"]
        low_efficiency_events = [e for e in monitor.invalidation_events if e.additional_data.get("efficiency_category") == "low"]
        zero_efficiency_events = [e for e in monitor.invalidation_events if e.additional_data.get("efficiency_category") == "zero"]
        mixed_efficiency_events = [e for e in monitor.invalidation_events if e.additional_data.get("efficiency_category") == "mixed"]
        ...
        assert all(keys >= 200 for keys in high_efficiency_keys), "High efficiency events should have many keys"
        
        # Verify low efficiency events have low key counts
        low_efficiency_keys = [e.keys_invalidated for e in low_efficiency_events]
        assert all(keys <= 2 for keys in low_efficiency_keys), "Low efficiency events should have few keys"
        
        # Verify zero efficiency events have zero keys
        zero_efficiency_keys = [e.keys_invalidated for e in zero_efficiency_events]
        assert all(keys == 0 for keys in zero_efficiency_keys), "Zero efficiency events should have no keys"
        ...
        trend_scenarios = [
            {"keys": 100, "duration": 0.020, "time_offset": 0},    # Early: good efficiency
            {"keys": 80, "duration": 0.025, "time_offset": 60},    # 1 min later: declining
            {"keys": 60, "duration": 0.030, "time_offset": 120},   # 2 min later: worse
            {"keys": 40, "duration": 0.035, "time_offset": 180},   # 3 min later: poor
            {"keys": 20, "duration": 0.040, "time_offset": 240},   # 4 min later: very poor
        ]
        ...
        for i, scenario in enumerate(trend_scenarios):
            ...
            monitor_trending.invalidation_events[-1].timestamp = base_time + scenario["time_offset"]
        
        # Verify trend captures declining efficiency
        trend_keys = [e.keys_invalidated for e in monitor_trending.invalidation_events]
        assert trend_keys == [100, 80, 60, 40, 20], "Should capture declining efficiency trend"
        
        trend_durations = [e.duration for e in monitor_trending.invalidation_events]
        assert trend_durations == [0.020, 0.025, 0.030, 0.035, 0.040], "Should capture increasing duration trend"
        ...
        efficiency_recommendations = [
            ...
            if "efficiency" in rec.get("issue", "").lower() or 
               "keys" in rec.get("message", "").lower()
        ]
    ...
    def test_record_generic_operation_supports_integration(self):
        ...
        integration_test_cases = [
            ...
            {"name": "security_check", "duration_ms": 15.5, "success": True, "component": "security_manager"},
            {"name": "token_validation", "duration_ms": 8.2, "success": True, "component": "security_manager"},
            {"name": "permission_check", "duration_ms": 25.0, "success": False, "component": "security_manager"},
            
            # Cache-related operations from other components
            {"name": "cache_warmup", "duration_ms": 150.0, "success": True, "component": "cache_warmer"},
            {"name": "cache_health_check", "duration_ms": 5.0, "success": True, "component": "health_monitor"},
            {"name": "cache_backup", "duration_ms": 2000.0, "success": True, "component": "backup_service"},
            
            # External service integrations
            {"name": "database_query", "duration_ms": 45.0, "success": True, "component": "db_service"},
            {"name": "api_call", "duration_ms": 120.0, "success": False, "component": "external_api"},
            {"name": "file_upload", "duration_ms": 800.0, "success": True, "component": "file_service"},
            
            # Edge cases
            {"name": "very_fast_operation", "duration_ms": 0.1, "success": True, "component": "micro_service"},
            {"name": "very_slow_operation", "duration_ms": 5000.0, "success": False, "component": "slow_service"},
            {"name": "", "duration_ms": 10.0, "success": True, "component": "anonymous"},  # Empty name
        ]
        ...
        for i, (recorded, expected) in enumerate(zip(monitor.cache_operation_times, integration_test_cases)):
        ...
        stats = monitor.get_performance_stats()
        assert "cache_operations" in stats
        assert stats["cache_operations"]["total_operations"] == len(integration_test_cases)
        ...
        async def test_error_handling():
            ...
            await monitor.record_operation("test", float('inf'), True)  # Infinite duration
        ...
        assert threshold_stats["cache_operations"]["slow_operations"] == 1  # Only slow_op should be flagged
        ...
        recorded_names = [op.operation_type for op in concurrent_monitor.cache_operation_times]
