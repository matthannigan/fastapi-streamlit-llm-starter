...
class TestValidationResult:
    ...
    def test_add_conflict_parameter_tracking(self):
        ...
        assert (
            result.parameter_conflicts["memory_cache_size"]
            == "Conflicts with l1_cache_size"
        )
        assert (
            result.parameter_conflicts["compression_threshold"]
            == "Value too low for performance"
        )
...
class TestCacheParameterMapperInitialization:
    ...
    def test_parameter_classifications_setup(self):
        ...
        # Then: AI-specific parameters should be correctly identified
        ai_specific_key = (
            "ai_specific_parameters"
            if "ai_specific_parameters" in info
            else "ai_specific_params"
        )
        ...
        # And: Generic parameters should be properly categorized
        generic_key = (
            "generic_parameters" if "generic_parameters" in info else "generic_params"
        )
    ...
    def test_immutable_parameter_definitions(self):
        ...
        ai_key = (
            "ai_specific_parameters"
            if "ai_specific_parameters" in initial_info
            else "ai_specific_params"
        )
        generic_key = (
            "generic_parameters"
            if "generic_parameters" in initial_info
            else "generic_params"
        )

        initial_ai_params = (
            initial_info[ai_key].copy()
            if hasattr(initial_info[ai_key], "copy")
            else set(initial_info[ai_key])
        )
        initial_generic_params = (
            initial_info[generic_key].copy()
            if hasattr(initial_info[generic_key], "copy")
            else set(initial_info[generic_key])
        )
        ...
        second_ai_params = (
            second_info[ai_key]
            if isinstance(second_info[ai_key], set)
            else set(second_info[ai_key])
        )
        second_generic_params = (
            second_info[generic_key]
            if isinstance(second_info[generic_key], set)
            else set(second_info[generic_key])
        )

        # Convert to sets for comparison to handle list vs set differences
        initial_ai_set = (
            initial_ai_params
            if isinstance(initial_ai_params, set)
            else set(initial_ai_params)
        )
        initial_generic_set = (
            initial_generic_params
            if isinstance(initial_generic_params, set)
            else set(initial_generic_params)
        )
...
class TestParameterMapping:
    ...
    def test_basic_parameter_separation(self):
        ...
        assert isinstance(
            generic_params, dict
        ), "Generic parameters should be a dictionary"
        assert isinstance(
            ai_specific_params, dict
        ), "AI-specific parameters should be a dictionary"

        # And: Generic parameters should be suitable for GenericRedisCache
        # Check that expected generic parameters are in the result
        assert (
            "redis_url" in generic_params
        ), "redis_url should be in generic parameters"
        assert (
            "default_ttl" in generic_params
        ), "default_ttl should be in generic parameters"
        assert (
            "compression_threshold" in generic_params
        ), "compression_threshold should be in generic parameters"
        assert (
            "l1_cache_size" in generic_params
        ), "l1_cache_size should be in generic parameters"
        ...
        assert (
            "enable_l1_cache" in generic_params
        ), "L1 cache should be auto-enabled when l1_cache_size is provided"
        assert (
            generic_params["enable_l1_cache"] == True
        ), "L1 cache should be enabled automatically"

        # And: AI-specific parameters should contain only AI functionality
        # Check that AI-specific parameters are separated correctly
        assert (
            "text_hash_threshold" in ai_specific_params
        ), "text_hash_threshold should be in AI-specific parameters"
        assert (
            "hash_algorithm" in ai_specific_params
        ), "hash_algorithm should be in AI-specific parameters"
        assert (
            "text_size_tiers" in ai_specific_params
        ), "text_size_tiers should be in AI-specific parameters"
        ...
        assert ai_specific_params["text_size_tiers"] == {
            "small": 500,
            "medium": 5000,
            "large": 50000,
        }
        ...
        assert (
            len(generic_keys.intersection(ai_keys)) == 0
        ), "Parameters should not appear in both generic and AI-specific"
        ...
        assert original_keys.issubset(
            all_separated_keys
        ), "All original parameters should be accounted for after separation"
    ...
    def test_parameter_name_mapping(self):
        ...
        assert (
            "l1_cache_size" in generic_params
        ), "AI parameter 'memory_cache_size' should be mapped to generic parameter 'l1_cache_size'"

        # And: Parameter values should be preserved during mapping
        assert (
            generic_params["l1_cache_size"] == 100
        ), "Value for mapped parameter 'l1_cache_size' should be preserved from 'memory_cache_size'"

        # And: Direct generic parameters should be included in generic parameters
        assert (
            "redis_url" in generic_params
        ), "Direct generic parameter 'redis_url' should be in generic parameters"
        assert (
            "default_ttl" in generic_params
        ), "Direct generic parameter 'default_ttl' should be in generic parameters"
        assert (
            generic_params["redis_url"] == "redis://localhost:6379"
        ), "redis_url value should be preserved"
        assert (
            generic_params["default_ttl"] == 7200
        ), "default_ttl value should be preserved"
        ...
        assert (
            "text_hash_threshold" in ai_specific_params
        ), "text_hash_threshold should remain AI-specific"
        assert (
            "hash_algorithm" in ai_specific_params
        ), "hash_algorithm should remain AI-specific"
        assert (
            ai_specific_params["text_hash_threshold"] == 800
        ), "text_hash_threshold value should be preserved"
        assert (
            ai_specific_params["hash_algorithm"] == "md5"
        ), "hash_algorithm value should be preserved"

        # Verify original AI parameter names don't appear in generic parameters
        assert (
            "memory_cache_size" not in generic_params
        ), "Original AI parameter name 'memory_cache_size' should not appear in generic parameters"

        # Verify mapped parameters don't appear in AI-specific parameters
        assert (
            "l1_cache_size" not in ai_specific_params
        ), "Mapped generic parameter 'l1_cache_size' should not appear in AI-specific parameters"
    ...
    def test_empty_parameters_handling(self):
        ...
        assert (
            mapping_successful
        ), "Parameter mapping should handle empty parameters without exceptions"
        assert isinstance(
            generic_params, dict
        ), "Generic parameters should be a dictionary"
        assert isinstance(
        ...
        assert len(generic_params) == 0 or all(
            v is None or v == "" or v == 0 for v in generic_params.values()
        ), "Generic parameters should be empty or contain only default values"
        assert (
            len(ai_specific_params) == 0
        ), "AI-specific parameters should be empty for empty input"

        # Verify the results are usable
        assert generic_params is not None, "Generic parameters should not be None"
        assert (
            ai_specific_params is not None
        ), "AI-specific parameters should not be None"
    ...
    def test_only_generic_parameters(self):
        ...
        for param_name, param_value in only_generic_params.items():
            assert (
                param_name in generic_params
            ), f"Generic parameter '{param_name}' should be in generic parameters"
            assert (
                generic_params[param_name] == param_value
            ), f"Generic parameter '{param_name}' value should be preserved"

        # And: The AI-specific dictionary should be empty
        assert (
            len(ai_specific_params) == 0
        ), "AI-specific parameters should be empty when only generic parameters are provided"

        # And: Generic parameters should be unchanged
        # Verify exact preservation of parameter names and values
        assert set(generic_params.keys()).issuperset(
            set(only_generic_params.keys())
        ), "All original generic parameter names should be preserved"

        for param_name in only_generic_params:
            assert (
                generic_params[param_name] == only_generic_params[param_name]
            ), f"Value for generic parameter '{param_name}' should be unchanged"

        # Verify data types are preserved
        assert isinstance(
            generic_params["redis_url"], str
        ), "redis_url should remain string"
        assert isinstance(
            generic_params["default_ttl"], int
        ), "default_ttl should remain int"
        assert isinstance(
            generic_params["enable_l1_cache"], bool
        ), "enable_l1_cache should remain bool"
        assert isinstance(
            generic_params["l1_cache_size"], int
        ), "l1_cache_size should remain int"
    ...
    def test_only_ai_specific_parameters(self):
        ...
        for param_name, param_value in only_ai_params.items():
            assert (
                param_name in ai_specific_params
            ), f"AI parameter '{param_name}' should be in AI-specific parameters"
            assert (
                ai_specific_params[param_name] == param_value
            ), f"AI parameter '{param_name}' value should be preserved"
        ...
        for param_name in only_ai_params:
            assert (
                param_name not in generic_params
            ), f"AI-specific parameter '{param_name}' should not appear in generic parameters"
        ...
        if len(generic_params) > 0:
            ...
            for key, value in generic_params.items():
                assert (
                    key not in only_ai_params
                ), f"Generic parameter '{key}' should not be from AI-specific input"

        # And: AI-specific parameters should be preserved
        # Verify exact preservation of parameter names and values
        assert len(ai_specific_params) == len(
            only_ai_params
        ), "All AI-specific parameters should be preserved"
        ...
        assert ai_specific_params["preprocessing_pipeline"] == [
            "normalize",
            "tokenize",
            "hash",
        ...
        # Verify data types are preserved
        assert isinstance(
            ai_specific_params["text_hash_threshold"], int
        ), "text_hash_threshold should remain int"
        assert isinstance(
            ai_specific_params["enable_text_preprocessing"], bool
        ), "enable_text_preprocessing should remain bool"
        assert isinstance(
            ai_specific_params["text_size_tiers"], dict
        ), "text_size_tiers should remain dict"
        assert isinstance(
            ai_specific_params["preprocessing_pipeline"], list
        ), "preprocessing_pipeline should remain list"
    ...
    def test_mixed_parameter_scenarios(self):
        ...
        assert expected_generic_keys.issubset(
            actual_generic_keys
        ), f"Expected generic keys {expected_generic_keys} should be subset of {actual_generic_keys}"
        ...
        assert expected_ai_keys.issubset(
            actual_ai_keys
        ), f"Expected AI keys {expected_ai_keys} should be subset of {actual_ai_keys}"
        ...
        assert total_output_keys >= len(expected_generic_keys) + len(
            expected_ai_keys
        ...
        assert (
            generic_params["default_ttl"] == 7200
        ), "Original default_ttl should be preserved"
        assert (
            generic_params["compression_threshold"] == 3000
        ), "compression_threshold should be preserved"

        assert (
            ai_specific_params["text_hash_threshold"] == 1500
        ), "text_hash_threshold should be preserved"
        assert (
            ai_specific_params["hash_algorithm"] == "sha256"
        ), "hash_algorithm should be preserved"
        assert ai_specific_params["text_size_tiers"] == {
            "small": 300,
            "medium": 3000,
            "large": 30000,
        }, "Complex AI parameters should be preserved"
        ...
        if "ai_redis_url" in mixed_complex_params and "redis_url" in generic_params:
            ...
            assert generic_params["redis_url"] in [
                "redis://complex:6379",
                "redis://ai-specific:6379",
        ...
        assert (
            len(overlap) == 0
        ), f"Parameters should not appear in both categories: {overlap}"
    ...
    def test_parameter_value_preservation(self):
        ...
        if text_size_tiers_key in ai_specific_params:
            ...
            assert isinstance(
                preserved_tiers, dict
            ), "text_size_tiers should remain a dictionary"
            assert (
                preserved_tiers == original_tiers
            ), "Complex nested dictionary should be preserved exactly"

            # Check nested structure preservation
            assert (
                preserved_tiers["metadata"]["version"] == "v2"
            ), "Nested dictionary values should be preserved"
            assert (
                preserved_tiers["metadata"]["enabled"] is True
        ...
        if preprocessing_key in ai_specific_params:
            ...
            assert isinstance(
                preserved_pipeline, list
            ), "preprocessing_pipeline should remain a list"
            assert (
                preserved_pipeline == original_pipeline
            ), "List values should be preserved exactly"
            assert len(preserved_pipeline) == 3, "List length should be preserved"
            assert all(
                isinstance(item, str) for item in preserved_pipeline
            ), "List item types should be preserved"

        # Verify numeric precision preservation
        if "compression_ratio" in all_output_params:
            assert (
                abs(all_output_params["compression_ratio"] - 0.75) < 1e-10
            ), "Float precision should be preserved"

        if "connection_timeout" in all_output_params:
            assert (
                abs(all_output_params["connection_timeout"] - 5.5) < 1e-10
            ), "Float values should be preserved with precision"
    ...
    def test_configuration_error_scenarios(self):
        ...
        try:
            ...
            # And: The error should describe the specific conflict
            assert (
                "parameter" in str(e).lower() or "configuration" in str(e).lower()
            ...
            assert (
                False
            ), "Only ConfigurationError should be raised for configuration conflicts"
        ...
        try:
            ...
            assert (
                "redis_url" in generic_params
            ), "redis_url should be resolved from conflicting parameters"
            assert (
                "default_ttl" in generic_params
            ), "default_ttl should be resolved from conflicting parameters"

            # Verify that conflicting parameters don't both appear in generic params
            assert (
                "ai_redis_url" not in generic_params
            ), "Mapped parameter names should not appear in generic params"
            assert (
                "ai_default_ttl" not in generic_params
            ), "Mapped parameter names should not appear in generic params"
            ...
            assert any(
                ...
                for keyword in ["conflict", "duplicate", "parameter", "configuration"]
        ...
        try:
            ...
            if "redis_url" in generic_params:
                ...
                assert isinstance(
                    redis_url_value, str
                ), "redis_url should remain a string type"
            ...
            if "default_ttl" in generic_params:
                ...
                if ttl_value is not None:
                    assert isinstance(
                        ttl_value, (int, float)
                    ), "If TTL is not None, it should be numeric"
            ...
            assert (
                "text_hash_threshold" in ai_specific_params
            ), "Valid AI parameters should be preserved"
            assert (
                ai_specific_params["text_hash_threshold"] == 1000
            ), "Valid AI parameter values should be preserved"
            ...
            assert any(
                keyword in error_message
                for keyword in ["empty", "none", "required", "invalid", "parameter"]
            ), f"Error should describe empty/None parameter issues: {e}"

        # Ensure the test completes without unexpected exceptions
        assert (
            True
        ), "Configuration error scenarios should be handled without unexpected exceptions"
...
class TestParameterValidation:
    ...
    def test_invalid_parameter_types(self):
        ...
        assert (
            "type" in error_text
            or "int" in error_text
            or "str" in error_text
            or "bool" in error_text
        )
    ...
    def test_parameter_value_range_validation(self):
        ...
        parameter_mentioned = any(
            param in error_text for param in ["ttl", "cache_size", "threshold"]
        )
        assert (
            parameter_mentioned
        ), f"Error should mention specific parameters: {error_text}"
    ...
    def test_parameter_conflict_detection(self):
        ...
        conflicts_detected = (
            ...
            or len(validation_result.parameter_conflicts) > 0
            or any("conflict" in error.lower() for error in validation_result.errors)
        )
        ...
        if conflicts_detected:
            ...
            if validation_result.parameter_conflicts:
                ...
                for param, description in validation_result.parameter_conflicts.items():
                    assert (
                        len(description) > 0
                    ), f"Conflict description for {param} should not be empty"
    ...
    def test_configuration_consistency_checks(self):
        ...
        if has_consistency_feedback:
            ...
            if validation_result.recommendations:
                ...
                assert any(
                    keyword in recommendation_text for keyword in consistency_keywords
                )
    ...
    def test_warning_generation_for_suboptimal_config(self):
        ...
        has_feedback = (
            len(validation_result.warnings) > 0
            or len(validation_result.recommendations) > 0
            or len(validation_result.errors) > 0
        )
        ...
        if has_feedback:
            ...
            if validation_result.recommendations:
                ...
                assert any(
                    keyword in recommendation_text for keyword in improvement_keywords
                )
    ...
    def test_best_practice_recommendations(self):
        ...
        if validation_result.recommendations:
            ...
            has_actionable_content = any(
                keyword in recommendation_text for keyword in actionable_keywords
            )
            assert (
                has_actionable_content
            ), f"Recommendations should be actionable: {validation_result.recommendations}"
    ...
    def test_comprehensive_validation_result_structure(self):
        ...
        """
        from app.infrastructure.cache.parameter_mapping import (
            CacheParameterMapper, ValidationResult)
        ...
        total_classifications = len(validation_result.ai_specific_params) + len(
            validation_result.generic_params
        )
        assert (
            total_classifications >= 0
        )  # Should have some parameter classification information
    ...
    def test_empty_parameters_validation(self):
        ...
        try:
            ...
            if not validation_result.is_valid:
                ...
                ), "If empty params are invalid, should explain why"
            ...
            assert (
                False
            ), f"Empty parameters validation should not raise exceptions: {e}"
    ...
    def test_get_parameter_info_comprehensive_data(self):
        ...
        ai_key = (
            "ai_specific_parameters"
            if "ai_specific_parameters" in parameter_info
            else "ai_specific_params"
        )
        generic_key = (
            "generic_parameters"
            if "generic_parameters" in parameter_info
            else "generic_params"
        )
        ...
        assert (
            len(info_keys) >= 3
        ), f"Should provide comprehensive information: {info_keys}"

        # Information should be consistent across calls
        second_info = mapper.get_parameter_info()
        assert (
            parameter_info == second_info
        ), "get_parameter_info() should return consistent data"
