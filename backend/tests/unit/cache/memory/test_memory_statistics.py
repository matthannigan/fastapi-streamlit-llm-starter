...
class TestInMemoryCacheStatistics:
    ...
    @pytest.mark.asyncio
    async def test_get_stats_provides_comprehensive_cache_metrics(
        self,
        populated_memory_cache: InMemoryCache,
        cache_statistics_sample: Dict[str, Any],
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_statistics_update_correctly_after_cache_operations(
        self,
    ...
    @pytest.mark.asyncio
    async def test_utilization_percentage_calculated_accurately(
        ...
        assert small_memory_cache.get_stats()["utilization_percent"] == pytest.approx(
            33.33, rel=0.01
        )
    ...
    @pytest.mark.asyncio
    async def test_hit_rate_calculation_reflects_actual_operation_outcomes(
        self,
        default_memory_cache: InMemoryCache,
        sample_cache_key: str,
        sample_cache_value: Any,
        ...
        await default_memory_cache.get(sample_cache_key)  # Hit
        assert default_memory_cache.get_stats()["hit_rate"] == pytest.approx(
            66.67, rel=0.01
        )
    ...
    @pytest.mark.asyncio
    async def test_get_keys_returns_all_cache_keys_including_expired(
        self, default_memory_cache: InMemoryCache, mock_time_provider
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_get_active_keys_returns_only_non_expired_keys(
        self, default_memory_cache: InMemoryCache, mock_time_provider
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_size_method_returns_accurate_current_entry_count(
        self, default_memory_cache: InMemoryCache
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_get_ttl_returns_accurate_remaining_time_for_entries(
    ...
    @pytest.mark.asyncio
    async def test_memory_usage_statistics_reflect_actual_storage_requirements(
        self, default_memory_cache: InMemoryCache
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_statistics_remain_consistent_across_async_operations(
        self, default_memory_cache: InMemoryCache, cache_test_keys: List[str]
    ):
        """
