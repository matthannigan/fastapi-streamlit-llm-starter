...
class TestPerformanceMetric:
    ...
    def test_performance_metric_handles_optional_fields(self):
        ...
        assert metric_none_data.additional_data == {}  # __post_init__ converts None to {}
...
class TestCompressionMetric:
    ...
    def test_compression_metric_supports_efficiency_analysis(self):
        ...
        compression_speed = metric.original_size / metric.compression_time if metric.compression_time > 0 else float('inf')
        
        # Calculate efficiency score (bytes saved per millisecond)
        efficiency_score = bytes_saved / (metric.compression_time * 1000) if metric.compression_time > 0 else float('inf')
        ...
        high_eff_bytes_saved = high_efficiency_metric.original_size - high_efficiency_metric.compressed_size
        fast_bytes_saved = fast_compression_metric.original_size - fast_compression_metric.compressed_size
        
        high_eff_speed = high_efficiency_metric.original_size / high_efficiency_metric.compression_time
        fast_speed = fast_compression_metric.original_size / fast_compression_metric.compression_time
        
        # Verify trade-off analysis is supported by the data
        assert high_eff_bytes_saved > fast_bytes_saved  # High efficiency saves more bytes
        ...
        assert high_efficiency_metric.compression_time > fast_compression_metric.compression_time
        assert high_efficiency_metric.compression_ratio < fast_compression_metric.compression_ratio
    ...
    def test_compression_metric_maintains_measurement_integrity(self):
        ...
        assert abs(metric.compression_ratio - calculated_ratio) < 0.0001  # Should be mathematically consistent
        ...
        # Field relationship integrity
        assert metric.original_size >= metric.compressed_size  # In this case, compression worked
        assert 0 <= metric.compression_ratio <= 1.0  # Ratio should be between 0 and 1 for good compression
...
class TestMemoryUsageMetric:
    ...
    def test_memory_usage_metric_validates_size_values(self):
        ...
        assert metric_consistent.memory_cache_size_bytes <= metric_consistent.total_cache_size_bytes
        assert metric_consistent.memory_cache_entry_count <= metric_consistent.cache_entry_count
    ...
    def test_memory_usage_metric_validates_entry_counts(self):
        ...
        expected_total_size = metric_consistent.cache_entry_count * metric_consistent.avg_entry_size_bytes
    ...
    def test_memory_usage_metric_calculates_totals_accurately(self):
        ...
        implied_redis_size = metric_aggregated.total_cache_size_bytes - metric_aggregated.memory_cache_size_bytes
        implied_redis_entries = metric_aggregated.cache_entry_count - metric_aggregated.memory_cache_entry_count
        ...
        calculated_avg = metric_aggregated.total_cache_size_bytes / metric_aggregated.cache_entry_count
        ...
        assert metric_memory_only.total_cache_size_bytes == metric_memory_only.memory_cache_size_bytes
        assert metric_memory_only.cache_entry_count == metric_memory_only.memory_cache_entry_count
        ...
        implied_redis_size_only = metric_redis_only.total_cache_size_bytes - metric_redis_only.memory_cache_size_bytes
        implied_redis_entries_only = metric_redis_only.cache_entry_count - metric_redis_only.memory_cache_entry_count
        ...
        calculated_large_avg = metric_large.total_cache_size_bytes / metric_large.cache_entry_count
        assert abs(metric_large.avg_entry_size_bytes - calculated_large_avg) < 1.0  # Within 1 byte
    ...
    def test_memory_usage_metric_supports_capacity_analysis(self):
        ...
        redis_cache_size = metric.total_cache_size_bytes - metric.memory_cache_size_bytes
        redis_entries = metric.cache_entry_count - metric.memory_cache_entry_count
        memory_tier_percentage = (metric.memory_cache_size_bytes / metric.total_cache_size_bytes) * 100
        redis_tier_percentage = (redis_cache_size / metric.total_cache_size_bytes) * 100
        
        # Calculate entry density metrics
        memory_density = metric.memory_cache_size_bytes / metric.memory_cache_entry_count if metric.memory_cache_entry_count > 0 else 0
        ...
        assert metric.cache_utilization_percent > 0  # Utilization relative to thresholds
        ...
        assert abs(memory_density - avg_entry_size) < 1.0  # Should be close to overall average
        assert abs(redis_density - avg_entry_size) < 1.0  # Should be close to overall average
        
        # Verify growth trend analysis data provision
        current_capacity_ratio = capacity_used_ratio  # Current usage vs threshold
        projected_entries_at_threshold = warning_threshold_bytes / metric.avg_entry_size_bytes
        current_entry_efficiency = metric.total_cache_size_bytes / metric.cache_entry_count
        
        assert current_capacity_ratio > 1.0  # Over threshold
        assert projected_entries_at_threshold < metric.cache_entry_count  # Current exceeds safe capacity
    ...
    def test_memory_usage_metric_maintains_measurement_consistency(self):
        ...
        assert abs(metric.avg_entry_size_bytes - calculated_avg) < 0.1  # Should be mathematically consistent
        ...
        implied_redis_size = metric.total_cache_size_bytes - metric.memory_cache_size_bytes
        implied_redis_entries = metric.cache_entry_count - metric.memory_cache_entry_count
        ...
        memory_size_ratio = metric.memory_cache_size_bytes / metric.total_cache_size_bytes
        memory_entry_ratio = metric.memory_cache_entry_count / metric.cache_entry_count
        assert abs(memory_size_ratio - memory_entry_ratio) < 0.01  # Ratios should be similar
        ...
        assert metric.process_memory_mb > (metric.memory_cache_size_bytes / 1024 / 1024)  # Process should include cache
        ...
        assert metric_with_none_data.additional_data == {}  # Should be empty dict, not None
        ...
        assert reconstructed.timestamp == metric.timestamp
        assert reconstructed.cache_utilization_percent == metric.cache_utilization_percent
        assert reconstructed.warning_threshold_reached == metric.warning_threshold_reached
...
class TestInvalidationMetric:
    ...
    def test_invalidation_metric_creates_with_invalidation_data(self):
        ...
        assert minimal_metric.additional_data == {}  # Should be initialized by __post_init__
    ...
    def test_invalidation_metric_validates_key_counts(self):
        ...
        assert specific_pattern_metric.keys_invalidated < broad_pattern_metric.keys_invalidated
        assert specific_pattern_metric.duration < broad_pattern_metric.duration  # Fewer keys = faster
    ...
    def test_invalidation_metric_validates_timing_values(self):
        ...
        few_keys_efficiency = few_keys_metric.keys_invalidated / few_keys_metric.duration if few_keys_metric.duration > 0 else 0
        many_keys_efficiency = many_keys_metric.keys_invalidated / many_keys_metric.duration if many_keys_metric.duration > 0 else 0
    ...
    def test_invalidation_metric_calculates_efficiency_accurately(self):
        ...
        # When: Efficiency calculations are performed
        high_eff_keys_per_second = high_efficiency_metric.keys_invalidated / high_efficiency_metric.duration
        high_eff_keys_per_ms = high_efficiency_metric.keys_invalidated / (high_efficiency_metric.duration * 1000)
        high_eff_ms_per_key = (high_efficiency_metric.duration * 1000) / high_efficiency_metric.keys_invalidated
        ...
        low_eff_keys_per_second = low_efficiency_metric.keys_invalidated / low_efficiency_metric.duration
        low_eff_keys_per_ms = low_efficiency_metric.keys_invalidated / (low_efficiency_metric.duration * 1000)
        low_eff_ms_per_key = (low_efficiency_metric.duration * 1000) / low_efficiency_metric.keys_invalidated
        ...
        zero_keys_per_second = zero_keys_metric.keys_invalidated / zero_keys_metric.duration
        ...
        if zero_time_metric.duration > 0:
            zero_time_keys_per_second = zero_time_metric.keys_invalidated / zero_time_metric.duration
        else:
        ...
        precise_keys_per_second = precise_metric.keys_invalidated / precise_metric.duration
        ...
        simple_efficiency = simple_pattern_metric.keys_invalidated / simple_pattern_metric.duration
        complex_efficiency = complex_pattern_metric.keys_invalidated / complex_pattern_metric.duration
        ...
        complexity_overhead = (complex_pattern_metric.duration - simple_pattern_metric.duration) / simple_pattern_metric.duration
    ...
    def test_invalidation_metric_supports_pattern_analysis(self):
        ...
        for metric in pattern_metrics:
            pattern = metric.pattern
            efficiency = metric.keys_invalidated / metric.duration if metric.duration > 0 else 0
        ...
        l1_efficiency = pattern_efficiency["cache:l1:*"][0]['efficiency']  # 50/0.010 = 5000
        global_efficiency = pattern_efficiency["*"][0]['efficiency']  # 10000/1.200 = 8333.33
        ...
        for i in range(1, len(temporal_analysis)):
            assert temporal_analysis[i]['timestamp'] >= temporal_analysis[i-1]['timestamp']
        ...
        for metric in pattern_metrics:
            ...
            total_keys_by_pattern[pattern] = total_keys_by_pattern.get(pattern, 0) + metric.keys_invalidated
        
        # Most frequent pattern analysis
        most_frequent_pattern = max(pattern_frequency, key=pattern_frequency.get)
        highest_impact_pattern = max(total_keys_by_pattern, key=total_keys_by_pattern.get)
    ...
    def test_invalidation_metric_maintains_event_integrity(self):
        ...
        type_context_pairs = {
            metric.invalidation_type: metric.operation_context
        }
        ...
        # Timing and efficiency consistency
        calculated_efficiency = metric.keys_invalidated / metric.duration if metric.duration > 0 else 0
        keys_per_ms = metric.keys_invalidated / (metric.duration * 1000) if metric.duration > 0 else 0
        ...
        assert metric.timestamp > 0  # Should be positive Unix timestamp
        assert metric.timestamp <= time.time() + 1  # Should not be significantly in future
        ...
        assert reconstructed.additional_data["metadata"]["batch_size"] == metric.additional_data["metadata"]["batch_size"]
        assert reconstructed.additional_data["validation_checks"] == metric.additional_data["validation_checks"]
        
        # Verify field relationships remain intact after reconstruction
        reconstructed_efficiency = reconstructed.keys_invalidated / reconstructed.duration
