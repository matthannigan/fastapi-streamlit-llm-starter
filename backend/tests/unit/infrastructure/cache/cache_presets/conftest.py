...
def mock_validation_result_with_errors():
    """
    Mock ValidationResult with validation errors for error testing.
    
    Provides a ValidationResult mock configured with validation errors
    for testing error handling scenarios in individual tests.
    """
    from app.infrastructure.cache.cache_validator import ValidationResult
    
    result = MagicMock(spec=ValidationResult)
    
    # Mock failed validation per contract
    result.is_valid = False
    result.messages = []
    result.validation_type = "preset"
    result.schema_version = "1.0"
    
    # Mock property methods with errors
    result.errors = [
        "Invalid TTL value: must be between 60 and 86400",
        "Redis URL format is invalid"
    ]
    result.warnings = [
        "High compression level may impact performance"
    ]
    result.info = [
        "Using default strategy settings"
    ]
...
@pytest.fixture
def mock_cache_validator():
    """
    Mock CacheValidator for testing validation functionality.
    
    Provides 'happy path' mock of the CacheValidator contract with
    all validation methods returning successful results. This is
    stateless since validators are utility classes without state.
    """
    from app.infrastructure.cache.cache_validator import CacheValidator
    
    validator = MagicMock(spec=CacheValidator)
    
    # Mock successful validation methods per contract
    def mock_validate_preset(preset_dict):
        result = MagicMock()
    ...
    def mock_validate_configuration(config_dict):
        result = MagicMock()
        result.is_valid = True
        result.errors = []
        result.warnings = []
    ...
    def mock_validate_custom_overrides(overrides):
        result = MagicMock()
        result.is_valid = True
        result.errors = []
        result.warnings = []
    ...
    validator.validate_preset.side_effect = mock_validate_preset
    validator.validate_configuration.side_effect = mock_validate_configuration
    validator.validate_custom_overrides.side_effect = mock_validate_custom_overrides
    
    # Mock template methods per contract
    validator.get_template.return_value = {
        "strategy": "balanced",
        "default_ttl": 3600,
        "max_connections": 10,
    ...
    validator.list_templates.return_value = [
        "fast_development",
        "balanced_production", 
        "robust_production",
        "ai_optimized"
    ...
    validator.compare_configurations.return_value = {
        "differences": {},
...
def preset_manager_test_data():
    ...
    return {
        "available_presets": [
            "disabled", "minimal", "simple", "development", 
            "production", "ai-development", "ai-production"
        ],
...
def mock_validation_result():
    """
    Mock ValidationResult for testing validation behavior.
    
    Provides 'happy path' mock of the cache_validator.ValidationResult contract with
    successful validation status. This is stateless since validation
    results are immutable containers for validation outcomes.
    ...
    # Mock successful validation per contract
    mock_result.is_valid = True
    mock_result.messages = []
    mock_result.validation_type = "preset"
    mock_result.schema_version = "1.0"
    ...
    # Mock methods per contract
    def mock_add_error(message, field_path='', context=None):
        mock_result.is_valid = False
        mock_result.errors.append(message)
        
    def mock_add_warning(message, field_path='', context=None):
        mock_result.warnings.append(message)
        
    def mock_add_info(message, field_path='', context=None):
        mock_result.info.append(message)
    
    mock_result.add_error = mock_add_error
    mock_result.add_warning = mock_add_warning  
    mock_result.add_info = mock_add_info
