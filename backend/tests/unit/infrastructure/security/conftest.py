...
class FakeSettings:
    ...
    def __init__(
        ...
        additional_api_keys: Optional[str] = None
    ):
...
def fake_settings_with_primary_key():
    ...
    """
    return FakeSettings(api_key="test-primary-key-123")
...
def mock_environment_detection():
    ...
    """
    # Create a mock environment info object
    class MockEnvironmentInfo:
        def __init__(self, environment=Environment.DEVELOPMENT, confidence=0.90, reasoning="Test environment"):
            self.environment = environment
            self.confidence = confidence
            self.reasoning = reasoning

    mock = Mock()
    mock.return_value = MockEnvironmentInfo()
    return mock
...
def mock_staging_environment(mock_environment_detection):
    ...
    class MockEnvironmentInfo:
        def __init__(self):
            ...
            self.reasoning = "Staging environment indicators found"
...
def mock_http_bearer_credentials():
    ...
    """
    return create_autospec(HTTPAuthorizationCredentials, instance=True)
...
def valid_http_bearer_credentials(mock_http_bearer_credentials):
    ...
    """
    mock_http_bearer_credentials.credentials = "test-valid-key-123"
    return mock_http_bearer_credentials
...
def invalid_http_bearer_credentials(mock_http_bearer_credentials):
    ...
    """
    mock_http_bearer_credentials.credentials = "invalid-test-key"
    return mock_http_bearer_credentials


# Exception classes are real imports - no fixtures needed as they're part of the same application
# These are available as direct imports:
# - AuthenticationError
# - ConfigurationError
# - Environment
# - FeatureContext