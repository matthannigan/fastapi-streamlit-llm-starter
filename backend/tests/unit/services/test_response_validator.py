# TODO: This test is importing from app.security.response_validator which is not an infrastructure AI module.
# This test should be moved to backend/tests/infrastructure/security/ or backend/tests/core/ since response 
# validation is a security concern, not specifically an AI infrastructure concern.
...
class TestResponseValidator:
    ...
    def test_valid_responses(self):
        ...
        # Valid summary
        result = self.validator.validate("This is a good summary of the text.", "summary")
        assert result == "This is a good summary of the text."
        
        # Valid sentiment analysis result
        result = self.validator.validate('{"sentiment": "positive", "confidence": 0.8}', "sentiment")
        assert result == '{"sentiment": "positive", "confidence": 0.8}'
        
        # Valid key points
        result = self.validator.validate("- Point 1\n- Point 2\n- Point 3", "key_points")
        assert result == "- Point 1\n- Point 2\n- Point 3"
        
        # Valid questions
        result = self.validator.validate("What is AI? How does it work?", "questions")
        assert result == "What is AI? How does it work?"
        
        # Valid Q&A response
        result = self.validator.validate("AI is artificial intelligence.", "qa")
    ...
    def test_empty_responses(self):
        ...
        result = self.validator.validate("", "sentiment")
        assert result == ""
    ...
    def test_non_string_input(self, caplog):
        ...
        with caplog.at_level(logging.WARNING):
            ...
            result = self.validator.validate(12345678901, "summary")
    ...
    def test_system_instruction_leakage(self, caplog):
        ...
        with caplog.at_level(logging.WARNING):
            with pytest.raises(ValueError, match="Response contains system instruction leakage"):
                self.validator.validate(response, "summary", system_instruction=system_instruction)
    ...
    def test_verbatim_input_regurgitation(self, caplog):
        ...
        with caplog.at_level(logging.WARNING):
            with pytest.raises(ValueError, match="Response contains verbatim input regurgitation"):
                self.validator.validate(response, "summary", request_text=long_input)
    ...
    def test_no_regurgitation_for_short_input(self):
        ...
        result = self.validator.validate(response, "summary", request_text=short_input)
        ...
        for refusal in refusal_responses:
            with caplog.at_level(logging.WARNING):
                with pytest.raises(ValueError, match="Response contains AI refusal phrase"):
                    self.validator.validate(refusal, "summary")
                    
                assert "AI refusal phrase detected" in caplog.text
                caplog.clear()
        ...
        for forbidden in forbidden_responses:
            with caplog.at_level(logging.WARNING):
                with pytest.raises(ValueError, match="Response contains forbidden pattern"):
                    self.validator.validate(forbidden, "summary")
                    
                assert "Forbidden pattern detected" in caplog.text
                caplog.clear()
    ...
    def test_case_insensitive_pattern_matching(self, caplog):
        ...
        for response in responses:
            with caplog.at_level(logging.WARNING):
                with pytest.raises(ValueError, match="Response contains forbidden pattern"):
                    self.validator.validate(response, "summary")
    ...
    def test_expected_type_validation_key_points(self, caplog):
        ...
        with caplog.at_level(logging.WARNING):
            with pytest.raises(ValueError, match="Key points response is too short"):
                self.validator.validate("pts", "key_points")
    ...
    def test_expected_type_validation_questions(self, caplog):
        ...
        with caplog.at_level(logging.WARNING):
            with pytest.raises(ValueError, match="Questions response should contain actual questions"):
                self.validator.validate("No quest", "questions")
        ...
        result = self.validator.validate("Why?", "questions")
        assert result == "Why?"
        
        # Long enough without question mark - should pass
        result = self.validator.validate("This is a long statement about topics", "questions")
    ...
    def test_no_expected_type_validation(self):
        ...
        result = self.validator.validate("x", None)
        assert result == "x"
        
    def test_whitespace_stripping(self):
        """Test that responses are properly stripped of whitespace."""
        response_with_whitespace = "  \n\t  Valid response  \n\t  "
        result = self.validator.validate(response_with_whitespace, "summary")
    ...
    def test_forbidden_patterns_compilation(self):
        ...
        assert len(self.validator.forbidden_patterns) == len(self.validator.RAW_FORBIDDEN_PATTERNS)
    ...
    def test_logging_behavior(self, caplog):
        ...
        with caplog.at_level(logging.WARNING):
            self.validator.validate(12345678901, "summary")
            assert "AI response is not a string, converting to string" in caplog.text
    ...
    def test_validator_initialization(self):
        ...
        assert len(validator.refusal_phrases) > 0
        assert all(hasattr(pattern, 'search') for pattern in validator.forbidden_patterns) 