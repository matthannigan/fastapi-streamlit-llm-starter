...
class TestCacheKeyGeneratorInitialization:
    ...
    def test_generator_maintains_thread_safety(self):
        ...
        def generate_key(thread_id):
            ...
            key = generator.generate_cache_key(text, "summarize", {"thread_id": thread_id})
            with lock:
    ...
    def test_generator_provides_consistent_behavior(self):
        ...
        different_key = generator.generate_cache_key(text + " modified", operation, options)
...
class TestCacheKeyGeneration:
    ...
    def test_generate_cache_key_creates_properly_formatted_keys(self, sample_text, sample_options):
        ...
        for operation in operations:
            op_key = generator.generate_cache_key(sample_text, operation, sample_options)
            assert f"op:{operation}" in op_key
            assert op_key.startswith("ai_cache:op:")
        
        # Test with different options
        different_options = {"temperature": 0.8, "max_tokens": 150}
        key_with_diff_options = generator.generate_cache_key(sample_text, "summarize", different_options)
    ...
    def test_generate_cache_key_handles_text_below_hash_threshold(self, sample_short_text, sample_options):
        ...
        key = generator.generate_cache_key(sample_short_text, "summarize", sample_options)
    ...
    def test_generate_cache_key_handles_text_above_hash_threshold(self, sample_long_text, sample_options):
        ...
        key = generator.generate_cache_key(sample_long_text, "summarize", sample_options)
        ...
        key2 = generator.generate_cache_key(sample_long_text, "summarize", sample_options)
        assert key == key2
        
        # Different large texts should produce different hashes
        different_long_text = sample_long_text + " additional content"
        different_key = generator.generate_cache_key(different_long_text, "summarize", sample_options)
    ...
    def test_generate_cache_key_uses_streaming_hash_for_large_text(self, sample_long_text, sample_options):
        ...
        large_text = "This is test content for streaming hash verification. " * 20000  # ~1MB+
        ...
        assert memory_increase < text_size * 5  # Less than 5x text size (very generous for CI)
    ...
    def test_generate_cache_key_extracts_question_for_qa_operations(self, sample_text):
        ...
        non_qa_key = generator.generate_cache_key(sample_text, "summarize", {"question": question})
    ...
    def test_generate_cache_key_handles_various_operation_types(self, sample_text, ai_cache_test_data):
        ...
        for operation, data in operations_data.items():
            if operation == "qa":
                ...
                # Standard operations
                key = generator.generate_cache_key(data["text"], operation, data["options"])
                ...
                if "question" not in data["options"]:
                    assert "|q:" not in key or key.endswith("|q:")  # Empty question component
    ...
    def test_generate_cache_key_maintains_backward_compatibility(self, sample_text, sample_options):
        ...
        for operation in legacy_operations:
            legacy_key = generator.generate_cache_key(sample_text, operation, sample_options)
    ...
    def test_generate_cache_key_maintains_hash_consistency(self, sample_long_text, sample_options):
        ...
        Related Tests:
            ...
            - test_generate_cache_key_maintains_backward_compatibility()
        """
        generator = CacheKeyGenerator(text_hash_threshold=500)  # Ensure long text gets hashed
        ...
        key_different_instance = generator2.generate_cache_key(sample_long_text, "summarize", sample_options)
        assert key_different_instance == keys[0]
        
        # Test hash consistency across different hash algorithms (if configurable)
        generator_sha256 = CacheKeyGenerator(text_hash_threshold=500, hash_algorithm=hashlib.sha256)
        key_sha256 = generator_sha256.generate_cache_key(sample_long_text, "summarize", sample_options)
        ...
        modified_key = generator.generate_cache_key(modified_text, "summarize", sample_options)
        assert modified_key != keys[0]
        assert "hash:" in modified_key
        
        # Test consistency with different operations on same text
        sentiment_key = generator.generate_cache_key(sample_long_text, "sentiment", sample_options)
        # Should be different because operation is different
        assert sentiment_key != keys[0]
        # But should still be consistent
        sentiment_key2 = generator.generate_cache_key(sample_long_text, "sentiment", sample_options)
...
class TestPerformanceMonitoringIntegration:
    ...
    def test_generator_integrates_with_performance_monitor(self):
        ...
        key = generator.generate_cache_key("test text", "summarize", {"max_length": 100})
        ...
        for operation, data in operations_data.items():
            ...
            for i in range(count):
                if operation == "qa":
                    ...
                    generator.generate_cache_key(data["text"], operation, data["options"])
...
class TestKeyGenerationEdgeCases:
    ...
    def test_generator_handles_whitespace_only_text(self, sample_options):
        ...
        for whitespace_text in whitespace_texts:
            key = generator.generate_cache_key(whitespace_text, "summarize", sample_options)
            assert key is not None
            assert "ai_cache:" in key
            assert "op:summarize" in key
        ...
        qa_key = generator.generate_cache_key("   ", "qa", {"question": "What is this whitespace?"})
        assert qa_key is not None
        assert "op:qa" in qa_key
        assert "|q:" in qa_key
    ...
    def test_generator_processes_special_characters_correctly(self, sample_options):
        ...
        for special_text in special_texts:
            key = generator.generate_cache_key(special_text, "summarize", sample_options)
            assert key is not None
            assert "ai_cache:" in key
            assert "op:summarize" in key
    ...
    def test_generator_processes_unicode_text_correctly(self, sample_options):
        ...
        for unicode_text in unicode_texts:
            key = generator.generate_cache_key(unicode_text, "summarize", sample_options)
            assert key is not None
            assert "ai_cache:" in key
            assert "op:summarize" in key
        ...
        assert unicode_length > 1000, f"Unicode text should be > 1000 chars, got {unicode_length}"
    ...
    def test_generator_handles_extremely_long_text(self):
        ...
        assert memory_increase < text_size_bytes * 5, f"Memory usage too high: {memory_increase} bytes (text size: {text_size_bytes} bytes)"
        ...
        different_key = generator.generate_cache_key(different_long_text, "summarize", {})
        assert different_key != key, "Different long texts should produce different keys"
    ...
    def test_generator_handles_extreme_option_combinations(self, sample_text):
        ...
        large_key = generator.generate_cache_key(sample_text, "summarize", large_options)
        ...
        nested_key = generator.generate_cache_key(sample_text, "summarize", nested_options)
        ...
        mixed_key = generator.generate_cache_key(sample_text, "summarize", mixed_type_options)
        assert mixed_key is not None
        ...
        extreme_key = generator.generate_cache_key(sample_text, "summarize", extreme_options)
        ...
        special_key = generator.generate_cache_key(sample_text, "summarize", special_char_options)
        ...
        option_keys = [
            ...
            generator.generate_cache_key(sample_text, "summarize", special_char_options),
        ]
        ...
        qa_extreme_options["question"] = "What is this complex text about?" * 10  # Long question
        qa_extreme_key = generator.generate_cache_key(sample_text, "qa", qa_extreme_options)
    ...
    def test_generator_maintains_performance_under_load(self):
        ...
        assert avg_time_per_key < 0.01, f"Average key generation too slow: {avg_time_per_key:.4f}s per key"
        ...
        def concurrent_key_generation(thread_id, num_keys=20):
            ...
            try:
                ...
                for i in range(num_keys):
                    ...
                    key = generator.generate_cache_key(text, "summarize", {"thread": thread_id})
        ...
        for i in range(200):
            key = generator.generate_cache_key(f"Load test {i}", "summarize", {"load_test": True})
            assert key is not None
