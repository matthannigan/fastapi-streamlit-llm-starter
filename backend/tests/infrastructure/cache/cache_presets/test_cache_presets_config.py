...
class TestCacheConfigDataclassBehavior:
    ...
    def test_cache_config_validates_required_vs_optional_parameters(self):
        ...
        assert secure_config.use_tls is False  # Secure default (explicit config required)
...
class TestCacheConfigValidation:
    ...
    def test_cache_config_validate_returns_valid_result_for_good_configuration(self):
        ...
        assert result.is_valid is True, f"Valid configuration should pass validation. Got errors: {result.messages}"
        assert len(result.messages) == 0, f"No validation errors should be present. Got: {result.messages}"
        assert result.validation_type in ["basic", "configuration"], "Should use known validation type"
        ...
        assert ai_result.is_valid is True, f"Valid AI configuration should pass validation. Got errors: {ai_result.messages}"
        assert len(ai_result.messages) == 0, f"No AI configuration errors should be present. Got: {ai_result.messages}"
    ...
    def test_cache_config_validate_identifies_invalid_redis_connection_parameters(self):
        ...
        invalid_connections_high = CacheConfig(
            strategy=CacheStrategy.BALANCED,
            max_connections=150  # Invalid: > 100
        )
        ...
        invalid_timeout_low = CacheConfig(
            strategy=CacheStrategy.BALANCED,
            connection_timeout=0  # Invalid: < 1
        )
    ...
    def test_cache_config_validate_identifies_invalid_performance_parameters(self):
        ...
        assert result_ttl_low.is_valid is False, "Invalid default_ttl should fail validation"
        assert len(result_ttl_low.errors) > 0, "Should have validation errors"
        assert any("default_ttl" in error for error in result_ttl_low.errors), "Should mention default_ttl in error"
        ...
        assert result_ttl_high.is_valid is False, "Invalid default_ttl should fail validation"
        assert len(result_ttl_high.errors) > 0, "Should have validation errors"
        assert any("default_ttl" in error for error in result_ttl_high.errors), "Should mention default_ttl in error"
        
        # Test invalid compression_level (too low)
        invalid_compression_low = CacheConfig(
            strategy=CacheStrategy.BALANCED,
            compression_level=0  # Invalid: < 1
        )
        result_compression_low = invalid_compression_low.validate()
        
        assert result_compression_low.is_valid is False, "Invalid compression_level should fail validation"
        assert len(result_compression_low.errors) > 0, "Should have validation errors"
        assert any("compression_level" in error for error in result_compression_low.errors), "Should mention compression_level in error"
        ...
        assert result_compression_high.is_valid is False, "Invalid compression_level should fail validation"
        assert len(result_compression_high.errors) > 0, "Should have validation errors"
        assert any("compression_level" in error for error in result_compression_high.errors), "Should mention compression_level in error"
        
        # Test invalid memory_cache_size (too low)
        invalid_cache_size_low = CacheConfig(
            strategy=CacheStrategy.BALANCED,
            memory_cache_size=0  # Invalid: < 1
        )
        result_cache_size_low = invalid_cache_size_low.validate()
        
        assert result_cache_size_low.is_valid is False, "Invalid memory_cache_size should fail validation"
    ...
    def test_cache_config_validate_provides_comprehensive_error_context(self):
        ...
        assert result.is_valid is False, "Configuration with multiple errors should fail validation"
        ...
        assert len(valid_result.errors) == 0, "Valid config should have empty error list"
...
class TestCacheConfigConversion:
    ...
    def test_cache_config_to_dict_handles_optional_parameter_serialization(self):
        ...
        assert "text_hash_threshold" not in minimal_dict or minimal_dict.get("text_hash_threshold") is None
        assert "operation_ttls" not in minimal_dict or minimal_dict.get("operation_ttls") is None
    ...
    def test_cache_config_serialization_preserves_strategy_information(self):
        ...
        for config_dict in [fast_dict, ai_dict, robust_dict]:
            ...
            assert config_dict["cache_strategy"] in ["fast", "balanced", "robust", "ai_optimized"]
    ...
    def test_cache_config_serialization_supports_json_yaml_compatibility(self):
        ...
        try:
            ...
            pytest.fail(f"Configuration should be JSON-serializable, but got error: {e}")
        ...
        try:
            ...
            assert isinstance(deserialized_dict, dict), "Deserialized data should be dict"
            ...
            pytest.fail(f"JSON round-trip should preserve configuration integrity, but got error: {e}")
        ...
        def check_json_compatibility(obj, path=""):
            ...
            if obj is None or isinstance(obj, (bool, int, float, str)):
                ...
                return all(isinstance(k, str) and check_json_compatibility(v, f"{path}.{k}")
                          for k, v in obj.items())
            elif isinstance(obj, list):
                return all(check_json_compatibility(item, f"{path}[{i}]")
        ...
        try:
            ...
            pytest.fail(f"Minimal configuration should be JSON-serializable, but got error: {e}")