...
class TestAIServiceResilience:
    ...
    def test_service_initialization(self, resilience_service):
        ...
        assert ResilienceStrategy.CRITICAL in resilience_service.configurations
    ...
    def test_health_status(self, resilience_service):
        ...
        assert health["half_open_circuit_breakers"] == []
    ...
    async def test_resilience_decorator_with_retries(self, resilience_service):
        ...
        # Check metrics - Note: retry attempts are tracked differently now
        metrics = resilience_service.get_metrics("test_retry")
        assert metrics.total_calls == 1  # Only counts final result
        assert metrics.successful_calls == 1
        # Note: retry_attempts tracking may work differently in new implementation
    ...
    async def test_resilience_decorator_with_circuit_breaker(self, resilience_service):
        ...
        for i in range(5):  # More than failure threshold
            with pytest.raises((TransientAIError, ServiceUnavailableError, RetryError)):
                await test_function()
        ...
        if cb:
            ...
            assert cb._state in ["open", "closed"]
    ...
    async def test_resilience_decorator_with_fallback(self, resilience_service):
        ...
        # With fallback, this should succeed by calling fallback
        result = await test_function()
        assert result == "fallback_result"
        assert fallback_called is True
        assert call_count > 0  # Should have tried the original function
...
class TestFallbackFunctionality:
    ...
    async def test_fallback_called_when_circuit_open(self):
        ...
        # First call should fail and use fallback due to retries exhausting
...
class TestRetryStrategies:
    ...
    async def test_critical_strategy_maximum_retries(self):
        ...
        config = resilience_service.configurations[ResilienceStrategy.CRITICAL]
...
class TestCustomConfiguration:
    ...
    async def test_custom_retry_config(self):
        ...
        with pytest.raises((TransientAIError, RetryError)):
            await failing_function()
...
class TestConcurrentOperations:
    ...
    async def test_concurrent_operations_isolated_metrics(self):
        ...
        # The retry mechanism tries multiple times, so failed_calls will be > 1
...
class TestErrorHandlingEdgeCases:
    ...
    async def test_http_status_code_classification(self):
        ...
        with pytest.raises((httpx.HTTPStatusError, PermanentAIError)):
            await bad_request()
...
class TestMetricsAndMonitoring:
    ...
    def test_comprehensive_health_status(self):
        ...
        assert health["half_open_circuit_breakers"] == []  # Should be empty in this test
...
class TestConvenienceDecorators:
    ...
    async def test_all_convenience_decorators_work(self):
        ...
        assert "aggressive_conv_test" in ai_resilience.operation_metrics
        assert "balanced_conv_test" in ai_resilience.operation_metrics
        assert "conservative_conv_test" in ai_resilience.operation_metrics
        assert "critical_conv_test" in ai_resilience.operation_metrics
...
class TestResilienceEdgeCases:
    ...
    async def test_strategy_string_conversion(self):
        ...
        assert "string_strategy_test" in resilience_service.operation_metrics
    ...
    async def test_metrics_reset_all_operations(self):
        ...
        # Should be empty now
        assert len(resilience_service.operation_metrics) == 0
...
class TestRetryLogic:
    ...
    def test_should_retry_on_exception_function(self):
        ...
        from app.infrastructure.resilience import should_retry_on_exception
...
class TestResilienceEdgeCasesAndErrorPaths:
    ...
    async def test_strategy_enum_instead_of_string(self):
        ...
        # Should have created metrics
        assert "enum_strategy_test" in resilience_service.operation_metrics
    ...
    def test_is_healthy_with_open_circuit_breakers(self):
        ...
        # The is_healthy method only checks if circuit breakers are open, it doesn't log warnings
        is_healthy = resilience_service.is_healthy()
        
        # Should return False when circuit breakers are open
    ...
    async def test_fallback_when_circuit_open_no_retry(self):
        ...
        # First call should fail (without retry, so it will use fallback if available)
    ...
    def test_reset_metrics_for_nonexistent_operation(self):
        ...
        assert "nonexistent_operation" not in resilience_service.operation_metrics
...
class TestLoggingAndMonitoring:
    ...
    async def test_logging_levels_and_messages(self):
        ...
        metrics = resilience_service.get_metrics("logging_test")
        assert metrics.total_calls == 1
        assert metrics.successful_calls == 1
    ...
    async def test_error_logging_includes_timing(self):
        ...
        metrics = resilience_service.get_metrics("error_timing_test")
        assert metrics.total_calls == 1
...
class TestCircuitBreakerIntegrationEdgeCases:
    ...
    def test_circuit_breaker_without_metrics_attribute(self):
        ...
        # The get_all_metrics method should handle missing metrics gracefully
        cb_metrics = all_metrics["circuit_breakers"]["mock_cb"]
        assert "state" in cb_metrics
        assert "failure_threshold" in cb_metrics
        assert "recovery_timeout" in cb_metrics
