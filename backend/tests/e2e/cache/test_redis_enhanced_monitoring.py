...
class TestRedisEnhancedMonitoringWorkflow:
    ...
    async def test_redis_invalidation_updates_real_metrics(self, enhanced_client_with_preset):
        ...
        async with enhanced_client_with_preset("ai-production") as client:
            ...
            initial_stats_response = await client.get("/internal/cache/invalidation-stats", headers=headers)
            ...
            updated_stats_response = await client.get("/internal/cache/invalidation-stats", headers=headers)
            ...
            if updated_stats and initial_stats:
                ...
                assert updated_operations >= initial_operations, "Operations count should not decrease"
    ...
    async def test_redis_performance_monitoring_under_load(self, enhanced_client_with_preset):
        ...
        async with enhanced_client_with_preset("ai-production") as client:
            ...
            try:
                ...
                baseline_response = await client.get("/internal/cache/metrics", headers=headers)
                baseline_duration = time.time() - start_time
                
                if baseline_response.status_code == 500:
                    # Check if it's a performance monitor issue
                    error_data = baseline_response.json()
                    if "Performance monitor not available" in str(error_data.get("detail", "")):
                ...
                assert successful_ops >= 10, f"Expected at least 10 successful operations, got {successful_ops}"
                
                # Test monitoring responsiveness under load
                monitor_start = time.time()
                load_metrics_response = await client.get("/internal/cache/metrics", headers=headers)
                monitor_duration = time.time() - monitor_start
                
                assert load_metrics_response.status_code == 200
                assert monitor_duration < 3.0, f"Monitoring should respond within 3s under load, took {monitor_duration:.2f}s"
                ...
                assert "timestamp" in load_data or "cache_hit_rate" in load_data, f"Unexpected metrics structure: {load_data}"
    ...
    async def test_redis_connection_monitoring_and_recovery(self, enhanced_client_with_preset):
        ...
        async with enhanced_client_with_preset("ai-production") as client:
            ...
            healthy_status_response = await client.get("/internal/cache/status", headers=headers)
            ...
            assert "redis" in healthy_data, f"Missing 'redis' key in response: {healthy_data}"
            redis_status = healthy_data["redis"]
            
            # Should show connected with real Redis
            assert "status" in redis_status, f"Missing 'status' in redis info: {redis_status}"
            ...
            if "connected_clients" in redis_status:
                ...
                assert redis_status["connected_clients"] >= 1  # At least our test connection
            ...
            try:
                post_op_metrics = await client.get("/internal/cache/metrics", headers=headers)
                ...
                # According to the contract, metrics should have timestamp, cache_hit_rate, etc.
                assert "timestamp" in metrics_data or "cache_hit_rate" in metrics_data, f"Unexpected metrics structure: {metrics_data}"
    ...
    async def test_redis_memory_usage_monitoring(self, enhanced_client_with_preset):
        ...
        async with enhanced_client_with_preset("ai-production") as client:
            ...
            memory_test_operations = [
                {"pattern": f"redis_memory_test:large_pattern_{i}:*"}
                for i in range(10)
            ]
            ...
            try:
                metrics_response = await client.get("/internal/cache/metrics", headers=headers)
                
                if metrics_response.status_code == 500:
                    # Check if it's a performance monitor issue
                    error_data = metrics_response.json()
                    if "Performance monitor not available" in str(error_data.get("detail", "")):
                        print("Performance monitor unavailable, skipping memory metrics test")
                        return
                    else:
                        raise AssertionError(f"Unexpected 500 error: {error_data}")
    ...
    async def test_redis_ttl_and_expiration_monitoring(self, enhanced_client_with_preset):
        ...
        async with enhanced_client_with_preset("ai-production") as client:
            ...
            status_response = await client.get("/internal/cache/status", headers=headers)
            ...
            assert "redis" in status_data, f"Missing 'redis' key in response: {status_data}"
            ...
            try:
                post_ttl_metrics = await client.get("/internal/cache/metrics", headers=headers)
                
                if post_ttl_metrics.status_code == 500:
                    # Check if it's a performance monitor issue
                    error_data = post_ttl_metrics.json()
                    if "Performance monitor not available" in str(error_data.get("detail", "")):
                        print("Performance monitor unavailable, TTL test operations completed")
                        return
                    else:
                        raise AssertionError(f"Unexpected 500 error: {error_data}")
                ...
                # According to the contract, metrics should have timestamp, cache_hit_rate, etc.
                assert "timestamp" in ttl_metrics_data or "cache_hit_rate" in ttl_metrics_data, f"Unexpected metrics structure: {ttl_metrics_data}"
    ...
    async def test_redis_monitoring_consistency_across_operations(self, enhanced_client_with_preset):
        ...
        async with enhanced_client_with_preset("ai-production") as client:
            ...
            for i, pattern in enumerate(test_patterns):
                ...
                metrics_response = await client.get("/internal/cache/invalidation-stats", headers=headers)
                assert metrics_response.status_code == 200
            ...
            for i in range(1, len(operation_metrics)):
                ...
                assert current["total_operations"] >= previous["total_operations"], \
                ...
                for field in ["total_operations", "frequency", "patterns"]:
                    if field in previous["metrics"]:
                        assert field in current["metrics"], f"Field '{field}' missing in operation {i+1}"