...
class TestPresetDrivenBehavior:
    ...
    async def test_preset_influences_cache_status(
        self, client_with_preset, preset, expected_redis, expected_memory
    ):
        """
        ...
        async with client_with_preset(preset) as client:
            ...
            if expected_redis == "connected":
                ...
                assert redis_status in [
                    ...
                    "error",
                    "unavailable",
                ], f"Unexpected Redis status for {preset}: {redis_status}"
            else:
                ...
                assert redis_status in [
                    ...
                    "unavailable",
                    "error",
                ], f"Expected disconnected Redis for {preset}: {redis_status}"

            # Memory cache expectations (more reliable in tests)
            if expected_memory == "active":
                # Memory cache should be functional
                assert memory_status in [
                    "active",
                    "normal",
                    "available",
                ], f"Expected active memory for {preset}: {memory_status}"
            elif expected_memory == "disabled":
                # Memory cache should be disabled
                assert memory_status in [
                    "disabled",
                    "unavailable",
                ], f"Expected disabled memory for {preset}: {memory_status}"
    ...
    async def test_preset_configuration_persistence(self, client_with_preset):
        ...
        async with client_with_preset("ai-production") as client:
            ...
            for response in responses[1:]:
                ...
                assert (
                    response["memory"]["status"] == first_response["memory"]["status"]
                )
    ...
    async def test_disabled_preset_prevents_cache_operations(self, client_with_preset):
        ...
        async with client_with_preset("disabled") as client:
            ...
            redis_disabled = redis_status in [
                "disabled",
                "disconnected",
                "unavailable",
                "error",
            ...
            assert (
                redis_disabled or memory_disabled
            ), f"Expected at least one cache backend disabled. Redis: {redis_status}, Memory: {memory_status}"
