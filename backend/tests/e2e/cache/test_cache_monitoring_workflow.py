...
class TestCacheMonitoringWorkflow:
    ...
    async def test_invalidation_action_updates_metrics(self, authenticated_client):
        ...
        initial_stats_response = await authenticated_client.get(
            "/internal/cache/invalidation-stats"
        )
        ...
        invalidate_response = await authenticated_client.post(
            "/internal/cache/invalidate", params={"pattern": invalidation_pattern}
        )
        ...
        updated_stats_response = await authenticated_client.get(
            "/internal/cache/invalidation-stats"
        )
        ...
        if updated_stats:  # Non-empty response indicates some stats tracking
            ...
            if initial_stats and (
                initial_daily_frequency > 0 or initial_total_operations > 0
            ):
                assert (
                    updated_daily_frequency >= initial_daily_frequency
                ), "Daily frequency should not decrease"
                assert (
                    updated_total_operations >= initial_total_operations
    ...
    async def test_metrics_consistency_across_operations(self, authenticated_client):
        ...
        for i, pattern in enumerate(test_patterns, 1):
            ...
            if current_stats and initial_stats:  # Both responses have data
                ...
                ), f"Operation {i}: Total should not decrease from {initial_total} to {current_total}"
                ...
                if current_total > initial_total:
                    ...
                    ), f"Operation {i}: Expected at least {expected_total}, got {current_total}"
    ...
    async def test_monitoring_performance_under_load(self, authenticated_client):
        ...
        ), f"Monitoring endpoint took {monitor_duration:.2f}s, should be under 5s"
        assert stats_response.status_code in [
            200,
            503,
        ], "Monitoring should respond even under load"
    ...
    async def test_monitoring_metrics_precise_increments(self, authenticated_client):
        ...
        auth_test_response = await authenticated_client.post(
            "/internal/cache/invalidate", params={"pattern": "auth_test"}
        )
        if auth_test_response.status_code == 401:
            pytest.skip(
                "Authentication not working in test environment - skipping precise metrics test"
            )

        # Step 1: Get baseline metrics to establish measurement foundation
        initial_metrics_response = await authenticated_client.get(
        ...
        if initial_metrics_response.status_code == 500:
            ...
            try:
                ...
                if (
                    "performance monitor" in error_detail.lower()
                    or "not available" in error_detail.lower()
                    or "InMemoryCache" in error_detail
                ):
                    pytest.skip(
                        "Performance monitor not available in test environment - skipping precise metrics test"
                    )
                else:
                ...
                pytest.skip(
                    "Metrics endpoint unavailable - skipping precise metrics test"
                )

        assert (
            initial_metrics_response.status_code == 200
        ...
        available_fields = [
            field for field in required_fields if field in initial_metrics
        ]

        if len(available_fields) < 2:
            pytest.skip(
                "Insufficient metrics fields available for precise increment testing"
            )
        ...
        if executed_operations < 2:
            pytest.skip(
                "Unable to execute sufficient cache operations for precise metrics test - may need AI API key"
            )
        ...
        final_metrics_response = await authenticated_client.get(
            "/internal/cache/metrics"
        )
        assert (
            final_metrics_response.status_code == 200
        ), "Metrics endpoint should remain available"
        ...
        print(
            f"  Cache hits: {final_cache_hits} (change: +{final_cache_hits - initial_cache_hits})"
        )
        print(
        ...
        assert (
            total_ops_increase >= executed_operations
        ), f"Total operations should increase by at least {executed_operations}, got {total_ops_increase}"
        ...
        assert (
            total_cache_activity > 0
        ), "Cache activity metrics should show some change"
        ...
        if total_ops_increase > 0:
            assert (
                total_cache_activity <= total_ops_increase + 10
            ), "Cache metrics should be consistent with total operations"  # Allow small buffer for concurrent operations
        ...
        if "cache_hit_rate" in final_metrics:
            ...
            assert (
                0 <= hit_rate <= 100
            ), f"Cache hit rate should be percentage between 0-100, got {hit_rate}"
            ...
            if final_total_ops > initial_total_ops and final_total_ops > 10:
                ...
                assert (
                    abs(hit_rate - expected_hit_rate) < 5
                ), f"Hit rate {hit_rate}% should approximate expected {expected_hit_rate:.1f}%"
