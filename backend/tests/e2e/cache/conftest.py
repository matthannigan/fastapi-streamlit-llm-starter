...
try:
    ...
    from .conftest_redis import (enhanced_cache_preset_app,
                                 enhanced_client_with_preset, redis_config,
                                 redis_container, redis_preset_scenarios)
except ImportError:
...
async def client(cache_preset_app):
    ...
    app_instance = cache_preset_app(
        "development"
    )  # Any preset works, rate limiting is disabled in cache_preset_app
    ...
    async with AsyncClient(
        ...
        try:
            ...
            if test_response.status_code != 200:
                ...
                logger.warning(
                    "Authentication may not be working properly in test environment"
                )
...
PRESET_SCENARIOS = [
    (
        ...
        "disconnected",
        "active",
    ),  # Redis unavailable in test env, falls back to InMemoryCache
    (
        "disabled",
        "disconnected",
        "active",
    ),  # "Disabled" still uses InMemoryCache fallback, reports "active"
    (
        "development",
        "disconnected",
        "active",
    ...
    (
        ...
        "disconnected",
        "active",
    ),  # Memory-only configuration, Redis disconnected
]
