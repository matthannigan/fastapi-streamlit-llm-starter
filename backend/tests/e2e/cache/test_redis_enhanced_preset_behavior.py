...
class TestRedisEnhancedPresetBehavior:
    ...
    async def test_ai_production_preset_shows_connected_redis_status(self, enhanced_client_with_preset):
        ...
        async with enhanced_client_with_preset("ai-production") as client:
            ...
            assert "status" in redis_info, f"Missing 'status' in redis info: {redis_info}"
    ...
    async def test_redis_pattern_invalidation_with_real_connectivity(self, enhanced_client_with_preset):
        ...
        async with enhanced_client_with_preset("ai-production") as client:
            ...
            test_requests = [
                {"text": "Test content for Redis invalidation", "operation": "key_points"},
            ...
            for request_data in test_requests:
                try:
                    await client.post("/v1/text_processing/process", json=request_data, headers=auth_headers)
                except Exception:
            ...
            if invalidate_response.status_code == 401:
                ...
                print(f"Authentication failed for invalidation test: {invalidate_response.json()}")
            ...
            assert invalidate_response.status_code == 200, f"Expected 200, got {invalidate_response.status_code}: {invalidate_response.json()}"
    ...
    async def test_redis_performance_metrics_with_real_operations(self, enhanced_client_with_preset):
        ...
        async with enhanced_client_with_preset("ai-production") as client:
            ...
            try:
                initial_metrics = await client.get("/internal/cache/metrics", headers=auth_headers)
                ...
                # Get updated metrics
                updated_metrics = await client.get("/internal/cache/metrics", headers=auth_headers)
                ...
                assert "timestamp" in updated_data or "cache_hit_rate" in updated_data, f"Unexpected metrics structure: {updated_data}"
    ...
    async def test_redis_security_features_with_testcontainers(self, redis_container, enhanced_cache_preset_app):
        ...
        async with AsyncClient(
            ...
            assert "status" in redis_config, f"Missing 'status' in redis config: {redis_config}"
            ...
            if "connected_clients" in redis_config:
                ...
                assert redis_config["connected_clients"] >= 1  # At least our test connection
    ...
    async def test_redis_preset_scenarios_comprehensive(self, enhanced_client_with_preset, redis_preset_scenarios):
        ...
        """
        for preset, expected_redis_status, expected_memory_status in redis_preset_scenarios:
            async with enhanced_client_with_preset(preset) as client:
                ...
                # Validate response structure
                assert "redis" in data, f"Missing 'redis' key in response for preset {preset}: {data}"
                assert "memory" in data, f"Missing 'memory' key in response for preset {preset}: {data}"
                
                # Test expectations based on real Redis availability
                redis_info = data["redis"]
                memory_info = data["memory"]
                
                assert "status" in redis_info, f"Missing 'status' in redis info for preset {preset}: {redis_info}"
