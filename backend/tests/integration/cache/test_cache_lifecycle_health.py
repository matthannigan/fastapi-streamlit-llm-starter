...
class TestCacheLifecycleHealth:
    ...
    async def test_health_check_dependency_with_healthy_cache(self, cache_factory):
        ...
        assert retrieved_value == "test_value", "Cache should be functional for health test"
        ...
        assert health_status["status"] == "healthy", "Connected cache should report healthy status"
        ...
        assert "ping_available" in health_status, "Health status should include ping availability"
        assert "operation_test" in health_status, "Health status should include operation test result"
    ...
    async def test_health_check_dependency_with_degraded_cache(self):
        ...
        assert isinstance(health_status, dict), "Health status should be dictionary"
        assert "status" in health_status, "Health status should include status field"
        assert health_status["status"] in ["degraded", "unhealthy"], "Broken cache should report degraded/unhealthy status"
        
        # Verify diagnostic information about the problem
        assert "errors" in health_status, "Health status should include errors list"
        assert len(health_status.get("errors", [])) > 0 or len(health_status.get("warnings", [])) > 0, "Health status should include error or warning details"
        assert "cache_type" in health_status, "Health status should include cache type"
    ...
    async def test_cleanup_dependency_empties_cache_registry(self, cache_factory):
        ...
        assert isinstance(cleanup_result, dict), "Cleanup should return statistics dictionary"
        assert "total_entries" in cleanup_result, "Cleanup should report total entries"
        assert "active_caches" in cleanup_result, "Cleanup should report active caches count"
        assert "dead_references" in cleanup_result, "Cleanup should report dead references count"
        assert "disconnected_caches" in cleanup_result, "Cleanup should report disconnected caches count"
        assert "errors" in cleanup_result, "Cleanup should report errors list"
        
        # Verify counts are reasonable integers
        for field in ["total_entries", "active_caches", "dead_references", "disconnected_caches"]:
    ...
    async def test_dependency_manager_registry_cleanup(self):
        ...
        assert isinstance(cleanup_result, dict), "Manager cleanup should return dictionary"
        
        # The cleanup should have the same structure as the main cleanup function
        expected_fields = ["total_entries", "active_caches", "dead_references", "disconnected_caches", "errors"]
    ...
    async def test_cache_service_dependency_integration(self):
        ...
        try:
            ...
            assert retrieved == "dependency_value", "Cache service should provide functional cache"
            ...
            pytest.skip(f"Cache service dependency not available in test environment: {e}")
    ...
    async def test_health_status_comprehensive_reporting(self, cache_factory):
        ...
        expected_fields = ["status", "cache_type", "ping_available", "operation_test", "errors", "warnings", "statistics"]
        for field in expected_fields:
            assert field in health_status, f"Health status should include {field}"
        
        # Verify status field has appropriate value
        assert health_status["status"] in ["healthy", "degraded", "unhealthy"], "Status should be valid value"
        
        # Verify cache type information is meaningful
        assert isinstance(health_status["cache_type"], str), "Cache type should be string"
        ...
        assert isinstance(health_status["statistics"], dict), "Statistics should be dictionary"
    ...
    async def test_lifecycle_integration_robustness(self, cache_factory):
        ...
        assert health1["status"] == health2["status"], "Health status should be consistent"
        assert health1["cache_type"] == health2["cache_type"], "Cache type should be consistent"
