...
class TestCachePresetBehavior:
    ...
    async def test_factory_method_behavioral_differences(self, cache_factory):
        ...
        ), "AI factory should create InMemoryCache when Redis unavailable"
        assert isinstance(
            web_cache, InMemoryCache
        ), "Web factory should create InMemoryCache when Redis unavailable"
        ...
        assert ai_value == {
            "data": "ai_value"
        }, "AI cache should store and retrieve data"
        assert web_value == {
            "data": "web_value"
        }, "Web cache should store and retrieve data"

        # Verify different cache instances (different configurations)
        assert (
            ai_cache is not web_cache
        ), "Factory should create different cache instances"
    ...
    async def test_testing_cache_isolation_behavior(self, cache_factory):
        ...
        test_cache1 = await cache_factory.for_testing(
            use_memory_cache=True, default_ttl=60
        )

        test_cache2 = await cache_factory.for_testing(
            use_memory_cache=True, default_ttl=120
        )

        # Both should be InMemoryCache for isolated testing
        assert isinstance(
            test_cache1, InMemoryCache
        ), "Testing factory should create InMemoryCache"
        assert isinstance(
            test_cache2, InMemoryCache
        ), "Testing factory should create InMemoryCache"
        ...
        assert (
            test_cache1 is not test_cache2
        ), "Factory should create different cache instances"
    ...
    async def test_cache_configuration_parameter_acceptance(self, cache_factory):
        ...
        web_cache = await cache_factory.for_web_app(
            default_ttl=1800, l1_cache_size=200, compression_threshold=2000
        )

        # Test testing factory with testing-specific parameters
        test_cache = await cache_factory.for_testing(
            default_ttl=60, use_memory_cache=True, l1_cache_size=50
        )

        # All should create working cache instances (InMemoryCache due to Redis fallback)
        assert isinstance(
            ai_cache, InMemoryCache
        ), "AI factory should create working cache"
        assert isinstance(
            web_cache, InMemoryCache
        ), "Web factory should create working cache"
        assert isinstance(
            test_cache, InMemoryCache
        ), "Testing factory should create working cache"
    ...
    async def test_graceful_degradation_behavior_differences(self, cache_factory):
        ...
        test_cache = await cache_factory.for_testing(
            use_memory_cache=True, default_ttl=60  # Explicit memory cache
        )

        # All should be InMemoryCache due to fallback
        assert isinstance(
            ai_cache, InMemoryCache
        ), "AI factory should fallback to InMemoryCache"
        assert isinstance(
            web_cache, InMemoryCache
        ), "Web factory should fallback to InMemoryCache"
        assert isinstance(
            test_cache, InMemoryCache
        ), "Testing factory should create InMemoryCache"
    ...
    async def test_preset_configuration_behavioral_differences(self):
        ...
        assert (
            dev_ttl != prod_ttl
        ), "Development and production should have different TTLs"
        assert (
            dev_ttl < prod_ttl
        ), "Development TTL should be shorter than production for faster feedback"
        ...
        assert (
            "text_hash_threshold" in ai_specific
        ), "AI preset should include text_hash_threshold in ai_config"
        assert (
            "operation_ttls" in ai_specific
        ), "AI preset should include operation_ttls in ai_config"
        assert isinstance(
            ai_specific["operation_ttls"], dict
        ), "Operation TTLs should be dictionary"
        assert (
            len(ai_specific["operation_ttls"]) > 0
        ), "Should have configured operation TTLs"

        # Verify generic preset excludes AI configuration (ai_config is None or missing)
        assert (
            dev_config.get("ai_config") is None
        ), "Generic preset should have ai_config as None"
        assert not dev_config.get(
            "enable_ai_cache", False
        ), "Generic preset should not enable AI cache"
