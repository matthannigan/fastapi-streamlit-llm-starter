...
class TestCacheSecurityIntegration:
    ...
    async def test_security_config_creation_and_validation(self):
        ...
        assert (
            auth_config.has_authentication
        ), "AUTH config should show authentication enabled"
        assert auth_config.security_level in [
            "LOW",
            "MEDIUM",
            "HIGH",
        ], "Should provide security level"

        # Test ACL username/password configuration
        acl_config = SecurityConfig(
            acl_username="cache_user", acl_password="acl_password_456"
        )
        assert (
            acl_config.has_authentication
        ), "ACL config should show authentication enabled"
        assert acl_config.security_level in [
            "LOW",
            "MEDIUM",
            "HIGH",
        ], "Should provide security level"
        ...
        assert (
            tls_config.has_authentication
        ), "TLS config should show authentication enabled"
        assert tls_config.security_level in [
            "MEDIUM",
            "HIGH",
        ], "TLS should provide higher security level"
        ...
        assert (
            combined_config.has_authentication
        ), "Combined config should show authentication enabled"
        assert (
            combined_config.security_level == "HIGH"
        ), "Combined security should be high level"
    ...
    async def test_security_manager_creation_and_basic_functionality(self):
        ...
        assert (
            "security_level" in status or "config" in status
        ), "Status should include security information"
    ...
    async def test_cache_factory_with_security_config_integration(self, cache_factory):
        ...
        security_config = SecurityConfig(
            redis_auth="factory_test_password", connection_timeout=5, max_retries=2
        )
        ...
        assert (
            test_cache is not None
        ), "Test cache should be created with security config"
    ...
    async def test_security_validation_with_mock_redis_success(self, cache_factory):
        ...
        # Create security configuration
        security_config = SecurityConfig(
            redis_auth="correct_password", connection_timeout=5
        )
        ...
        with patch("app.infrastructure.cache.security.aioredis") as mock_redis:
            ...
            try:
                ...
                assert (
                    "connection_successful" in test_result or "status" in test_result
    ...
    async def test_security_configuration_error_handling(self, cache_factory):
        ...
        assert (
            incomplete_tls_config.use_tls
        ), "TLS should be enabled even with incomplete config"
    ...
    async def test_security_config_environment_integration(self, monkeypatch):
        ...
        with patch.dict(
            ...
            if env_config is not None:
                assert (
                    env_config.redis_auth == "env_test_password"
                ), "Should read auth from environment"
                assert (
                    env_config.use_tls is True
                ), "Should parse TLS boolean from environment"
                assert (
                    env_config.connection_timeout == 15
                ), "Should parse timeout from environment"
            else:
    ...
    async def test_security_validation_comprehensive_reporting(self):
        ...
        assert any(
            keyword in report_lower for keyword in security_keywords
        ), "Report should contain security information"
    ...
    async def test_cache_factory_config_based_security_integration(self, cache_factory):
        ...
        config_security = SecurityConfig(
            redis_auth="config_test_password", connection_timeout=8, max_retries=3
        )
        ...
        config_cache = await cache_factory.create_cache_from_config(
            cache_config, fail_on_connection_error=False  # Allow fallback for testing
        )

        # Verify cache was created successfully
        assert (
            config_cache is not None
        ), "Config-based cache should be created with security"

        # Test cache functionality
        await config_cache.set("config_test_key", "config_test_value")
        retrieved = await config_cache.get("config_test_key")
        assert (
            retrieved == "config_test_value"
        ), "Config-based cache should be functional"
    ...
    async def test_security_integration_across_cache_types(self, cache_factory):
        ...
        uniform_security = SecurityConfig(
            redis_auth="uniform_test_password", connection_timeout=6
        )
        ...
        # Web cache
        web_cache = await cache_factory.for_web_app(
            security_config=uniform_security, fail_on_connection_error=False
        )
        ...
        for cache_type, cache in cache_types:
            assert (
                cache is not None
            ), f"{cache_type} cache should be created with security"
            ...
            ), f"{cache_type} cache should be functional with security"
