...
class TestEncryptionKeyValidationIntegration:
    ...
    @pytest.mark.skipif(
        ...
        reason="Test requires cryptography library to be unavailable",
    )
    def test_encryption_key_validation_fails_without_cryptography(
        ...
        Integration Scope:
            validate_encryption_key() → import cryptography.fernet → ImportError
            → error handling → validation failure with helpful message

        Business Impact:
            Ensures missing dependencies are detected at startup with clear
            error messages, preventing confusing runtime failures.

        Test Strategy:
            - Provide valid-length key (44 characters)
            - Validate in environment where cryptography is unavailable
            - Verify validation fails with ImportError-related message

        Success Criteria:
            - result["valid"] is False
            - result["errors"] mentions cryptography library unavailability
            - Error provides actionable information for fixing the issue

        Fixtures Used:
            - redis_security_validator: Real validator instance

        Special Execution Requirements:
            This test ONLY runs when cryptography library is NOT available.
            To test this scenario:
            1. Create isolated Python environment without cryptography
            2. Run this specific test: pytest -v -k "without_cryptography"
            3. See backend/tests/integration/startup/TEST_PLAN_cryptography_unavailable.md

        Note:
            This test is marked with skipif to prevent execution in standard
            test environments where cryptography IS available. The test plan
            documents special execution procedures for this scenario.
        """
        # Arrange: Valid length key that cannot be validated without cryptography
        valid_length_key = "x" * 44  # Correct length but can't verify format

        # Act: Validate key in environment without cryptography
        validator = redis_security_validator
        result = validator.validate_encryption_key(valid_length_key)

        # Assert: Validation fails
        assert (
            result["valid"] is False
        ), "Validation must fail without cryptography library"
        assert len(result["errors"]) > 0, "Must generate error for missing library"

        # Assert: Error message mentions cryptography unavailability
        error_message = " ".join(result["errors"]).lower()
        assert (
            "cryptography library not available" in error_message
            or "cryptography" in error_message
        ), "Error should mention cryptography library unavailability"
        assert (
            "cannot be validated" in error_message or "not available" in error_message
        ), "Error should indicate validation cannot proceed"
