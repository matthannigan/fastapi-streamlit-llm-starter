...
class TestConfigEnvironmentIntegration:
    ...
    def test_environment_auto_detection_identifies_development_environment(
        ...
        recommendation = (
            preset_manager_with_env_detection.recommend_preset_with_details()
        )
        ...
        assert any(
            env in detected_env_lower
            for env in ["development", "dev", "environment.development"]
        ), f"Expected development environment detection, got: {recommendation.environment_detected}"
        assert (
            recommendation.confidence > 0.8
        ), f"Low confidence {recommendation.confidence} for clear development environment"
        assert recommendation.preset_name in [
            "development",
            "simple",
        ], f"Unexpected preset recommendation: {recommendation.preset_name}"
        ...
        assert any(
            ...
            for indicator in ["development", "debug", "dev", "testing"]
    ...
    def test_environment_auto_detection_identifies_production_environment(
        ...
        assert any(
            env in detected_env_lower
            for env in ["production", "prod", "environment.production"]
        ), f"Expected production environment detection, got: {recommendation.environment_detected}"
        assert (
            recommendation.confidence > 0.8
        ), f"Low confidence {recommendation.confidence} for clear production environment"
        assert (
            recommendation.preset_name == "production"
        ), f"Production environment should recommend production preset, got: {recommendation.preset_name}"

        # Verify detection reasoning includes production indicators
        reasoning_lower = recommendation.reasoning.lower()
        assert any(
            indicator in reasoning_lower
            for indicator in ["production", "prod", "release", "live"]
    ...
    def test_preset_recommendation_api_returns_production_recommendations_for_production_env(
        self, resilience_test_client, monkeypatch: pytest.MonkeyPatch
    ):
        """
        ...
        assert (
            "environment_detected" in response_data
        )  # Changed from "environment_analysis"

        # Verify production-specific recommendation
        assert (
            response_data["recommended_preset"] == "production"
        ...
        assert any(
            ...
            for term in ["production", "high-reliability", "conservative", "stable"]
    ...
    def test_preset_recommendation_api_auto_detects_environment(
        ...
        Test Strategy:
            - Set specific environment indicators using App Factory Pattern
            - Create app directly to avoid fixture interference
        ...
        App Factory Pattern Usage:
            ...
            - Use create_app() directly instead of fixtures
            - Ensure test isolation with fresh app instance
        """
        # Arrange
        # Set staging environment indicators (using recognized staging value)
        ...
        # Create fresh app directly using App Factory Pattern
        # This avoids fixture interference with environment detection
        ...
        response = test_client.get(
            ...
            headers={"Authorization": "Bearer staging-key-98765"},
        )
        ...
        ), f"Auto-detection API failed: {response.text}"
        ...
        # Verify environment detection (should detect staging)
        detected_env = response_data["environment_detected"].lower()
        assert (
            "staging" in detected_env
        ), f"Expected staging detection, got: {response_data['environment_detected']}"

        # Verify appropriate preset for staging
        recommended_preset = response_data["recommended_preset"]
        assert recommended_preset in [
            "production",
            "simple",
        ...
        assert (
            "pattern" in response_data["detection_method"].lower()
            or "variable" in response_data["detection_method"].lower()
    ...
    async def test_orchestrator_loads_correct_configuration_and_applies_to_resilience_strategies(
        ...
        @production_orchestrator.with_resilience(
            "test_operation", strategy=ResilienceStrategy.CONSERVATIVE
        )
        ...
        circuit_breaker = production_orchestrator.get_or_create_circuit_breaker(
            "test_operation", config.circuit_breaker_config
        )
        ...
        assert (
            "test_operation" in metrics["operations"]
    ...
    async def test_configuration_changes_propagate_to_all_resilience_components(
        ...
        assert (
            prod_config.circuit_breaker_config.failure_threshold
    ...
    async def test_environment_specific_configurations_work_across_multiple_strategy_types(
        ...
        for strategy in [
            ...
            @production_orchestrator.with_resilience(
                f"{strategy.value}_operation", strategy=strategy
            )
            ...
            config = production_orchestrator.get_operation_config(
                f"{strategy.value}_operation"
            )
        ...
        for strategy_name, result in strategy_results.items():
            ...
            ), f"Strategy {strategy_name} failed to execute"
        ...
        assert (
            aggressive_config.retry_config.max_attempts
            >= conservative_config.retry_config.max_attempts
        ...
        assert (
            ...
            >= aggressive_config.circuit_breaker_config.failure_threshold
            and critical_config.circuit_breaker_config.failure_threshold
        ...
        for strategy_name, config in strategy_configs.items():
            ...
            assert (
                config.circuit_breaker_config.recovery_timeout >= 30
        ...
        for strategy_name in strategy_configs:
            ...
            assert (
                operation_name in all_metrics["operations"]
    ...
    def test_preset_manager_validation_with_environment_context(
        ...
        recommended_preset = preset_manager_with_env_detection.get_preset(
    ...
    async def test_end_to_end_configuration_flow_with_real_environment_variables(
        ...
        assert (
            "end_to_end_test" in metrics["operations"]
        ...
        assert operation_metrics["total_calls"] > 0, "Operation should have been called"
        assert (
            operation_metrics["successful_calls"] > 0
    ...
    def test_configuration_loading_with_custom_resilience_preset_environment_variable(
        ...
        )  # Would normally suggest 'development' preset
        monkeypatch.setenv(
            "RESILIENCE_PRESET", "production"
        )  # Explicit override to production
        ...
        assert (
            "dev" in detected_env or "development" in detected_env
