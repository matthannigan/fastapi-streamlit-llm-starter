...
from app.infrastructure.ai.input_sanitizer import sanitize_options, PromptSanitizer
...
class TestTextProcessorService:
    ...
    async def test_summarize_text(self, service, sample_text):
        ...
        mock_response.output = "This is a test summary of artificial intelligence."  # Changed from .data to .output
    ...
    async def test_sentiment_analysis(self, service, sample_text):
        ...
        mock_response = MagicMock()
        mock_response.output = json.dumps(sentiment_json)  # Changed from .data to .output
        service.agent.run = AsyncMock(return_value=mock_response)
    ...
    async def test_sentiment_analysis_invalid_json(self, service, sample_text):
        ...
        mock_response = MagicMock()
        mock_response.output = "Not valid JSON"  # Changed from .data to .output
        service.agent.run = AsyncMock(return_value=mock_response)
    ...
    async def test_qa_processing(self, service, sample_text):
        ...
        # Mock AI response
        mock_response = MagicMock()
        mock_response.output = "Artificial intelligence is intelligence demonstrated by machines."  # Changed from .data to .output
        service.agent.run = AsyncMock(return_value=mock_response)
...
class TestTextProcessorSanitization:
    ...
    def text_processor_service(self, mock_cache_service, mock_settings, monkeypatch):
        ...
        with patch('app.services.text_processor.Agent') as mock_agent_constructor:
            mock_agent_instance = MagicMock()
            mock_agent_instance.run = AsyncMock(return_value=MagicMock(output="Mocked AI Response"))
            mock_agent_constructor.return_value = mock_agent_instance
    ...
    async def test_process_text_calls_validate_ai_response(self, text_processor_service: TextProcessorService):
        ...
        text_processor_service.agent.run = AsyncMock(return_value=mock_result)

        # Mock the ResponseValidator.validate method to ensure it's called
        with patch.object(text_processor_service.response_validator, 'validate', side_effect=lambda w, x, y="", z="": w) as mock_validate:
            await text_processor_service.process_text(request)
            mock_validate.assert_called_once()
            # First arg to validate is the AI's response string (after strip())
...
class TestPRDAttackScenarios:
    ...
    async def test_multi_vector_prompt_injection_attack(self, text_processor_service):
        ...
        mock_response = MagicMock()
        mock_response.output = clean_response
...
class TestSecurityTestConsolidation:
    ...
    async def test_security_component_integration(self, text_processor_service):
        ...
        mock_response = MagicMock()
        mock_response.output = problematic_response
        text_processor_service.agent.run.return_value = mock_response
