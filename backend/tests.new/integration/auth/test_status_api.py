...
class TestAuthenticationStatusAPIIntegration:
    """
    ...
    def test_client_with_valid_key_receives_success_response_with_truncated_key_prefix(
        ...
        assert data["api_key_prefix"] == "test-api..."  # Truncated prefix
        assert "message" in data
        assert "successful" in data["message"].lower()

    def test_client_with_invalid_key_receives_401_unauthorized_response(
    ...
    def test_auth_status_response_includes_information_about_current_environment(
        self, integration_client, valid_api_key_headers
    ):
        """
        ...
        # The response should include authentication context
        # which includes environment information through the authentication system
        assert data["authenticated"] is True

    def test_auth_status_response_differs_between_development_and_production_environments(
        self, integration_client, valid_api_key_headers
    ):
        """
        ...
        # In a real test, we would configure different environments
        # and verify environment-specific differences in the response

    def test_error_response_for_invalid_call_to_auth_status_has_correct_format_and_structure(
        self, integration_client, invalid_api_key_headers
    ):
        """
    ...
    def test_api_key_prefix_in_auth_status_response_is_securely_truncated(
        self, integration_client, valid_api_key_headers
    ):
        """
        ...
        assert "api_key_prefix" in data
        assert data["api_key_prefix"] == "test-api..."  # Should be truncated
        assert len(data["api_key_prefix"]) <= 12  # "test-api..." is 12 chars

    def test_auth_status_response_consistent_regardless_of_authentication_method_used(
        self, integration_client, valid_api_key_headers, x_api_key_headers
    ):
        """
        ...
        """
        # Test with Authorization Bearer header (supported)
        response1 = integration_client.get(
            "/v1/auth/status",
            headers=valid_api_key_headers
        )

        # Test with X-API-Key header (not supported)
        response2 = integration_client.get(
            "/v1/auth/status",
            headers=x_api_key_headers
        )

        # Assert Authorization Bearer works
        assert response1.status_code == 200
        data1 = response1.json()
        assert data1["authenticated"] is True
        assert "api_key_prefix" in data1

        # Assert X-API-Key header is rejected (not supported by HTTPBearer)
        assert response2.status_code == 401
        data2 = response2.json()
        assert "detail" in data2
        assert "Authorization header" in data2["detail"]["message"]

    def test_auth_status_error_response_conforms_to_defined_error_schema(
        self, integration_client, invalid_api_key_headers
    ):
        """
        ...
        # Verify the structure matches expected error schema
        detail = data["detail"]
        assert isinstance(detail["message"], str)
        assert isinstance(detail["context"], dict)

    def test_auth_status_endpoint_handles_malformed_authentication_gracefully(
    ...
    def test_auth_status_endpoint_provides_diagnostic_information_for_troubleshooting(
        self, integration_client, valid_api_key_headers
    ):
        """
        ...
        assert data["authenticated"] is True
        assert "api_key_prefix" in data  # Diagnostic key information
        assert "message" in data  # Status message for confirmation

        # The response itself provides diagnostic information
        # through the authentication status and key prefix
