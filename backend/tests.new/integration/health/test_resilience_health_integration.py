...
class TestResilienceSystemHealthIntegration:
    """
    Integration tests for resilience system health monitoring.
    ...
    def test_resilience_health_integration_with_healthy_circuit_breakers(
        self, health_checker, mock_resilience_service
    ):
        """
        ...
        """
        with patch('app.infrastructure.monitoring.health.ai_resilience', mock_resilience_service):
            health_checker.register_check("resilience", check_resilience_health)

            result = await health_checker.check_component("resilience")

            # Verify healthy resilience monitoring
            ...
            assert "half_open_circuit_breakers" in result.metadata
    ...
    def test_resilience_health_integration_with_open_circuit_breakers(
        self, health_checker, mock_unhealthy_resilience_service
    ):
        """
        ...
        """
        with patch('app.infrastructure.monitoring.health.ai_resilience', mock_unhealthy_resilience_service):
            health_checker.register_check("resilience", check_resilience_health)

            result = await health_checker.check_component("resilience")

            # Verify degraded resilience monitoring
            assert result.name == "resilience"
            assert result.status == HealthStatus.DEGRADED
            assert "circuit breakers detected" in result.message.lower()
            assert result.response_time_ms > 0

            # Verify open circuit breaker information
            assert result.metadata is not None
            assert "open_circuit_breakers" in result.metadata
            assert len(result.metadata["open_circuit_breakers"]) == 1
            assert "test_cb" in result.metadata["open_circuit_breakers"]

    def test_resilience_health_integration_with_metrics_collection(
        self, health_checker, mock_resilience_service
    ):
        """
        ...
        mock_resilience_service.get_all_metrics.return_value = {
            "circuit_breakers": {
                "ai_service_cb": {"state": "closed", "failures": 2, "successes": 100},
                "cache_cb": {"state": "closed", "failures": 0, "successes": 50},
                "external_api_cb": {"state": "half_open", "failures": 5, "successes": 20}
            },
            "operations": {
                "retry": {"success": 100, "failure": 5, "avg_response_time": 0.15},
                "timeout": {"success": 50, "failure": 10, "avg_response_time": 2.5}
            }
        }
        ...
        with patch('app.infrastructure.monitoring.health.ai_resilience', mock_resilience_service):
            ...
            assert result.response_time_ms > 0
            assert result.response_time_ms < 150  # Metrics collection adds some overhead

    def test_resilience_health_integration_with_resilience_service_unavailable(
        self, health_checker
    ):
        """
        ...
        """
        # Mock import failure
        with patch('app.infrastructure.monitoring.health.ai_resilience', side_effect=ImportError("Resilience module unavailable")):
            health_checker.register_check("resilience", check_resilience_health)

            result = await health_checker.check_component("resilience")

            # Verify graceful handling of resilience service failure
            assert result.name == "resilience"
            assert result.status == HealthStatus.UNHEALTHY
            assert "failed" in result.message.lower()
            assert "ImportError" in result.message or "unavailable" in result.message
            assert result.response_time_ms > 0

    def test_resilience_health_integration_with_circuit_breaker_state_transitions(
        self, health_checker
    ):
        """
        ...
        """
        # Mock service with mixed circuit breaker states
        mixed_resilience_service = Mock()
        mixed_resilience_service.get_health_status.return_value = {
            "healthy": False,  # Overall unhealthy due to open breaker
            "open_circuit_breakers": ["failing_service"],
            "half_open_circuit_breakers": ["recovering_service"],
            "total_circuit_breakers": 4,
            "operations": {"retry": {"success": 50, "failure": 25}}
        }

        with patch('app.infrastructure.monitoring.health.ai_resilience', mixed_resilience_service):
            health_checker.register_check("resilience", check_resilience_health)
            ...
            assert result.name == "resilience"
            assert result.status == HealthStatus.DEGRADED  # Due to open breaker
            assert "circuit breakers detected" in result.message.lower()

            # Verify state-specific information
            assert result.metadata is not None
            assert len(result.metadata["open_circuit_breakers"]) == 1
            assert "failing_service" in result.metadata["open_circuit_breakers"]
    ...
    def test_resilience_health_integration_performance_under_load(
        self, health_checker, mock_resilience_service
    ):
        """
        ...
        """
        # Configure large metrics dataset to simulate load
        mock_resilience_service.get_health_status.return_value = {
            "healthy": True,
            "open_circuit_breakers": [],
            "half_open_circuit_breakers": [],
            "total_circuit_breakers": 100,  # Large number of circuit breakers
            "operations": {
                "retry": {"success": 10000, "failure": 500, "avg_response_time": 0.12},
                "timeout": {"success": 5000, "failure": 100, "avg_response_time": 2.1},
                "circuit_breaker": {"success": 15000, "failure": 200, "avg_response_time": 0.08}
            }
        }
        ...
        with patch('app.infrastructure.monitoring.health.ai_resilience', mock_resilience_service):
            ...
            assert result.status == HealthStatus.HEALTHY
            assert result.response_time_ms > 0
            assert result.response_time_ms < 300  # Should handle large datasets efficiently

            # Verify all metrics are still collected accurately
            assert result.metadata is not None
            assert result.metadata["total_circuit_breakers"] == 100
